...
- **Grand Refactoring for Public Release**: To make the `install.sh` universally deployable, the codebase needs a major refactor to remove hardcoded absolute paths (like `/Users/neo/...`) and consolidate the scattered microservice repositories. Following Principle 1 (Council Before Code) and Principle 4 (Human Timelines), this massive architectural change should NOT be done immediately after a major stabilization effort (like fixing the memory kernel panics). This refactoring should be planned for a future date, tackled incrementally on a separate branch, and tested in isolation.
