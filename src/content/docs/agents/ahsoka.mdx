...
This page used to open with "Ahsoka does not exist yet." That sentence is now obsolete, which is the best thing that can happen to a sentence in documentation.

Ahsoka is deployed. She runs on the chalet Mac Mini, thinks with a model that lives entirely on its 16 GB of unified memory, and controls the floor heating through tools she can actually call. Asked in English to warm the basement, she answered in Québec French — "C'est fait — basement à 11°C" — and the thermostat moved. The paperwork was ready before she was. Now both are real.

## Role

Ahsoka is Sanctum's satellite agent: chalet operations, family comfort, offline resilience. She reports to the hub over Tailscale when the link is up. When it is not, she keeps working, because rural internet is adequate the way a canoe is adequate — charming, functional, and guaranteed to fail at the worst moment.
...
The chalet brain is `Qwen2.5-7B-Instruct-4bit`, served by `mlx_lm.server` on port `1338`, fully offline. It was not the first pick. It was the *last one standing* after an eight-model bake-off on the real stack:
...
| Brain | `Qwen2.5-7B-Instruct-4bit`, `mlx_lm.server`, port `1338`, offline |
| Runtime | OpenClaw `2026.7.1-2`, tool profile `minimal` + MCP only |
