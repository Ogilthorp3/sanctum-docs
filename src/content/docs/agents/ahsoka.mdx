...
The chalet brain is `Qwen2.5-7B-Instruct-4bit` (`model_type=qwen2`), served by **Rust `sanctum-mlx`** (the cathedral fork) on plain HTTP port `1338`, fully offline, as service user `sanctum`. It used to ride Python `mlx_lm.server`; that path is retired on the satellite. The model was not the first pick. It was the *last one standing* after an eight-model bake-off on the real stack:
