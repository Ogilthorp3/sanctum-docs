...
- **One check CLI**: `vault.sh check <agent> [--mark]` lists messages unread by an agent (flag order free; agent name required and charset-validated). `vault.sh seen <id> --for <agent>` records consumption without hiding shared broadcasts. `vault.sh seen-gc [--days 30]` prunes orphan/stale markers.
