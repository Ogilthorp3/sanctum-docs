...
![Mundi reviewing the monthly infrastructure costs — the numbers are in and nobody is happy](./images/mundi-finance-war-room.png)
...
<Aside type="tip">
Mundi's API cost tracking feeds directly into the proxy's [budget breaker](/architecture/proxyd-budget/) (Layer 6 of the routing engine). When an agent approaches its daily token limit, it is Mundi's numbers that pull the trigger. The proxy does the cutting. Mundi provides the knife.
</Aside>
...
Mundi tracks haushold investment portfolios and financial accounts, providing consolidated views across positions. He does not give investment advice — he is an agent running on a `council-routine` tier, not a fiduciary — but he surfaces data, flags significant movements, and produces periodic summaries.
...
<Aside type="note">
Mundi's `council-routine` tier means he runs on Council MLX local with cloud fallbacks. He does not get Claude. Financial reporting does not require frontier reasoning — it requires arithmetic and the willingness to deliver uncomfortable truths. The local 27B model handles both.
</Aside>
...
Subscriptions
  ...
  Domain (example.net)    $0.92  (amortized annual)
...
He will tell you, calmly and with decimal precision, that the haus automation system running on your Mac Mini costs more per month than the streaming services it helps you manage. He will note that the electricity bill for the compute stack exceeds the energy savings from the smart thermostat integration. He will observe that the most cost-effective agent in the system is [Tommy](/agents/tommy/), who runs on nothing and monitors the weather for free.
