...
<Aside type="note">
Mundi's `council-routine` tier means he runs on Council 27B local with cloud fallbacks. He does not get Claude. Financial reporting does not require frontier reasoning — it requires arithmetic and the willingness to deliver uncomfortable truths. The local 27B model handles both.
</Aside>
