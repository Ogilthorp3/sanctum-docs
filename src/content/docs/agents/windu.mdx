...
![Windu — the security agent who treats every packet like a potential threat and every open port like a personal insult](../../../assets/illustrations/windu-security-perimeter.svg)
...
| Fallback chain | None. Claude Sonnet or nothing. |
| Workspace | `~/.openclaw/workspace-windu/` |
| Skills | `firewalla-toolkit`, `security-policy` |

<Aside type="danger">
Windu runs on the `council-secure` tier, which means the Sanctum Proxy will **never** reroute his requests to a cheaper model. Not to Gemini. Not to Qwen. Not to the local 27B. Security analysis does not get budget-optimized. If you change this, you deserve whatever Windu finds that a cheaper model would have missed.
</Aside>
...
| Bridge LaunchAgent | `com.openclaw.firewalla-bridge` |
| Bridge port | 18094 (bound to 0.0.0.0) |
| Bridge code | `/Users/neo/.openclaw/firewalla-bridge.js` |
| VM access | `http://10.10.10.1:18094` |
