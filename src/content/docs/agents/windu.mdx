...
<Aside type="danger">
Windu runs on the `council-secure` tier, which means the Sanctum Proxy will **never** reroute his requests to a cheaper model. He uses **Gemini 3.1 Pro** exclusively to leverage its multimodal spatial reasoning abilities for analyzing blueprints and security footage. He will never be automatically downgraded to Gemini Flash, Qwen, or the local 27B. Security analysis does not get budget-optimized. If you change this, you deserve whatever Windu finds that a cheaper model would have missed.
</Aside>
