...
A new MAC address joins the network at 2:47 in the morning. Nobody bought a device. Nobody is awake. By sunrise Windu has fingerprinted it, checked its vendor prefix against the known-hardware table, filed it under *unidentified*, and pinned it to the top of the morning briefing with a note. It turns out to be a neighbour's robot vacuum that wandered onto the wrong SSID. Windu does not apologize for the alarm. Windu apologizes for nothing.

This is the security agent. He monitors the network via the Firewalla Gold Pro, enforces security policies, and generates daily threat briefings — maintaining a level of vigilance that would be clinically concerning in a person and is exactly correct in a firewall agent. He is the one who knocks when an unfamiliar MAC address knocks first. He sees threats everywhere. This is occasionally annoying and will one day save everything.
...
- Network monitoring through the Firewalla Gold Pro P2P API
...
- Incident response escalation to [Yoda](/agents/yoda/) when human intervention is required
...
<Aside type="danger">
Windu runs on the `council-secure` tier, which means the [Sanctum Proxy's dynamic model routing](/architecture/dynamic-model-routing/) will **never** reroute his requests to a cheaper model. He uses **Gemini 3.1 Pro** exclusively to leverage its multimodal spatial reasoning abilities for analyzing blueprints and security footage. He will never be automatically downgraded to Gemini Flash or Qwen to save budget. His only safety net is the local Council MLX model, which is strictly a disaster recovery fallback in case the internet goes down. Security analysis does not get budget-optimized. If you change this, you deserve whatever Windu finds that a cheaper model would have missed.
</Aside>
...
Windu reaches the Firewalla Gold Pro through a bridge running on the Mac. The full topology of that bridge — and why the firewall lives one cable away but three hops out — is documented in [Sanctum Firewalla](/architecture/sanctum-firewalla/):

| Component | Details |
|-----------|---------|
| Firewalla IP | 10.0.0.1 (LAN gateway) |
