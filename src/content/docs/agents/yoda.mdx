...
| Local fallbacks | `council-mlx` (`sanctum-mlx` serving Qwen3.6-35B-A3B-4bit on mTLS `:1337`) and `council-code` (`sanctum-mlx-codestral` serving Codestral-22B-v0.1-4bit on mTLS `:3301`) |
...
<Aside type="caution">
Yoda's primary model is `sanctum/council-brain`, which `proxyd` on port 4040 resolves to Claude Opus (Max subscription — the unpinned `claude-opus-4` alias resolves to the latest Opus, 4.8 today) with content-based smart routing layered on top. Security and tool sessions stay on the latest Opus. Code drops to the local Codestral-22B on `sanctum-mlx-codestral` (mTLS `:3301`) for latency and volume. General chat drops to the local `council-mlx` (`sanctum-mlx` serving Qwen3.6-35B-A3B-4bit on mTLS `:1337`) so small talk never leaves the haus. Reasoning-heavy prompts are promoted to the same Opus with `--effort max` thinking via the Claude Max proxy on `:3456`. Images go to Gemini 3.1 Pro. If Yoda starts giving you suspiciously vague answers about infrastructure, check `~/.sanctum/sanctum-proxy/usage.jsonl` — the `model_used` field tells the truth even when the agent doesn't know it got rerouted.
</Aside>
...
| Code (`implement` / `refactor` / `debug` / fenced block) | `council-code` (`sanctum-mlx-codestral`, Codestral-22B-v0.1-4bit on mTLS `:3301`) | infrastructure work is high-volume — local wins on latency |
