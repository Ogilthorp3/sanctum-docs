...
<Aside type="caution">
Yoda's primary model is `sanctum/council-brain`, which `proxyd` on port 4040 resolves to Claude Opus 4.7 with content-based smart routing layered on top. Security and tool sessions stay on Opus 4.7. Code drops to the local Codestral-22B on `sanctum-mlx-codestral` (mTLS `:3301`) for latency and volume. General chat drops to the local `council-mlx` (`sanctum-mlx` serving Qwen3.6-35B-A3B-4bit on mTLS `:1337`) so small talk never leaves the haus. Reasoning-heavy prompts are promoted to Opus 4.7 with `--effort max` thinking via the Claude Max proxy on `:3456`. Images go to Gemini 3.1 Pro. If Yoda starts giving you suspiciously vague answers about infrastructure, check `~/.sanctum/sanctum-proxy/usage.jsonl` — the `model_used` field tells the truth even when the agent doesn't know it got rerouted.
</Aside>
...
| Brainy (reasoning verbs or ≥ 500 chars of prose) | `council-max-thinking` (Opus 4.7 `--effort max` via Claude Max proxy) | extended thinking for deep synthesis |
