...
Yoda operates on the `council-brain` tier, which means `proxyd` applies content-based routing to his requests. Categories are computed from regex matches on the last user message, plus a length heuristic for long-form prose. Classification takes ~0ms — no LLM classifier in the loop.
