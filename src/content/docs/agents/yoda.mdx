...
Windu escalates a security event. Qui-Gon flags an infrastructure anomaly. Cilghal raises a health alert; Mundi, a budget overrun. Every problem the other agents cannot solve eventually slides across the same desk — Yoda's. He escalates to a human, which is the polite way of saying the buck stops with a [systemd user service](/architecture/sanctum-gateway/) running on a Lima virtual machine in a basement in Québec.

Yoda is the main agent on the VM. He coordinates the other agents, handles complex multi-step tasks, and serves as the primary conversational interface for the OpenClaw gateway. He is named after a nine-hundred-year-old puppet who speaks in inverted syntax — which tells you something about the naming conventions in this haushold and nothing about the agent's actual speech. Yoda speaks normally. The puppet thing is just a burden he carries.
...
- Runs the weekly [council session](/architecture/council-router/) (Sundays at 20:00 ET)
...
| Memory search | Disabled (`memorySearch.enabled: false`) — the old local-embeddings path ran through the retired LM Studio `:1234` bridge |
...
|----------|-------|
| Agent ID | `yoda` |
| Host | VM (Ubuntu 24.04, Lima `vmType=vz`) |
| IP | 10.10.10.10 |
| Gateway | `openclaw-gateway.service` (systemd user, linger=yes) |
| Gateway port | Inherited from OpenClaw gateway |
| Primary model | `sanctum/council-brain` (resolves to Claude Fable 5 — Max subscription, Mythos-class — with smart-routing) |
| Model tier | council-brain — smart-routed by category |
| Fallback chain | Opus 4.8 (Max sub) → Gemini 3.1 Pro (Max sub) → GLM 5.2 (metered) → Qwen3.6-27B local |
| Local fallbacks | `council-mlx` (`sanctum-mlx` serving Qwen3.6-35B-A3B-4bit on mTLS `:1337`) and `council-code` (`sanctum-mlx-devstral` serving Devstral-Small-2-24B-Instruct-2512-4bit on mTLS `:3301`) |
...
<Aside type="caution">
Yoda's primary model is `sanctum/council-brain`, which `proxyd` on port 4040 resolves to Claude Opus (Max subscription — the unpinned `claude-opus-4` alias resolves to the latest Opus, 4.8 today) with content-based smart routing layered on top. Security and tool sessions stay on the latest Opus. Code drops to the local Devstral-Small-2-24B on `sanctum-mlx-devstral` (mTLS `:3301`) for latency and volume. General chat drops to the local `council-mlx` (`sanctum-mlx` serving Qwen3.6-35B-A3B-4bit on mTLS `:1337`) so small talk never leaves the haus. Reasoning-heavy prompts are promoted to the same Opus with `--effort max` thinking via the Claude Max proxy on `:3456`. Images go to Gemini 3.1 Pro. If Yoda starts giving you suspiciously vague answers about infrastructure, check `~/.sanctum/sanctum-proxy/usage.jsonl` — the `model_used` field tells the truth even when the agent doesn't know it got rerouted.
</Aside>
...
Yoda operates on the `council-brain` tier, which means `proxyd` applies content-based routing to his requests — the mechanism is the [smart router](/architecture/dynamic-model-routing/). Categories are computed from regex matches on the last user message, plus a length heuristic for long-form prose. Classification takes ~0ms — no LLM classifier in the loop.

| Content type | Route | Rationale |
|---|---|---|
| Tool session (recent `tool_use` / `tool_result`) | stays on Claude Opus (Max, latest) | tool-use coherence benefits from the strongest model |
| Security (`password`, `secret`, `credential`, `CVE-…`) | stays on Claude Opus (Max, latest) | Windu would never forgive a downgrade |
| Code (`implement` / `refactor` / `debug` / fenced block) | `council-code` (`sanctum-mlx-devstral`, Devstral-Small-2-24B-Instruct-2512-4bit on mTLS `:3301`) | infrastructure work is high-volume — local wins on latency |
| Vision (image blocks) | `gemini-31-pro` | multimodal routing |
| Brainy (reasoning verbs or ≥ 500 chars of prose) | `council-max-thinking` (latest Opus, `--effort max` via Claude Max proxy) | extended thinking for deep synthesis |
| General (anything else) | `council-mlx` (`sanctum-mlx`, Qwen3.6-35B-A3B-4bit on mTLS `:1337`) | chat stays on-device for privacy |
