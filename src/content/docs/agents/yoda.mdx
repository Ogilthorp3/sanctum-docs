...
| Local fallbacks | `council-mlx` (`sanctum-mlx` serving Qwen3.6-35B-A3B-4bit on mTLS `:1337`) and `council-code` (`sanctum-mlx-coder` serving Qwen2.5-Coder-14B-Instruct-4bit on mTLS `:1338`, with a plain `:1234` socat bridge for legacy callers) |
...
<Aside type="caution">
Yoda's primary model is `sanctum/council-brain`, which `proxyd` on port 4040 resolves to Claude Opus 4.7 with content-based smart routing layered on top. Security and tool sessions stay on Opus 4.7. Code drops to the local Coder-14B on `sanctum-mlx-coder` (mTLS `:1338`) for latency and volume. General chat drops to the local `council-mlx` (`sanctum-mlx` serving Qwen3.6-35B-A3B-4bit on mTLS `:1337`) so small talk never leaves the haus. Reasoning-heavy prompts are promoted to Opus 4.7 with `--effort max` thinking via the Claude Max proxy on `:3456`. Images go to Gemini 3.1 Pro. If Yoda starts giving you suspiciously vague answers about infrastructure, check `~/.sanctum/sanctum-proxy/usage.jsonl` — the `model_used` field tells the truth even when the agent doesn't know it got rerouted.
</Aside>
...
| Code (`implement` / `refactor` / `debug` / fenced block) | `council-code` (`sanctum-mlx-coder`, Qwen2.5-Coder-14B-Instruct-4bit on mTLS `:1338`) | infrastructure work is high-volume — local wins on latency |
