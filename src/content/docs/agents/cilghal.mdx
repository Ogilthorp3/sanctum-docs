...
<Aside type="note">
  Cilghal runs on the `council-routine` tier — Council 27B local with cloud fallbacks. She does not get Claude. Health analysis at 3am does not require the most expensive model in the fleet. It requires the most reliable one, and reliability in this system means "will still work when the internet goes out and all you have left is a quantized model on the same machine." Which, given that this is a haus in Quebec, happens more often than the cloud providers would like to admit.
</Aside>
