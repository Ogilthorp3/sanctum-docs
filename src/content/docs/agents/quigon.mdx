...
![Qui-Gon surveying the garden — calculating the optimal number of flowers per square meter](../../../assets/illustrations/quigon-efficiency-garden.svg)
...
Every 30 minutes, the healer diagnoses the full Home Assistant stack: 55 Tuya smart lights, 4 Ecobee sensors, 4 Ring cameras, 10 Sonos speakers, Alarmo, and 24 automations. It assigns severity levels from 0 (everything is fine) to 4 (the house is confused about whether its own lights exist), then works through a three-tier remediation pipeline:
...
| Model tier | `council-routine` (Qwen 3.5 Plus primary, council-27b fallback) |
| Skills | `mac-mini-ops`, `service-doctor`, `ha-self-healer` |
...
| Schedule | Weekly performance reviews, on-demand infrastructure checks, HA self-healer every 30 min |
