...
| **Purpose** | Launch QEMU headless, start the VM, and set the bridge100 IP to the configured `mac_bridge_ip` |
| **Required Service** | `vm` |
| **KeepAlive** | No |
| **RunAtLoad** | Yes |

Runs a startup script that launches QEMU headless, waits for the VM to boot, and configures the bridge interface IP via `sudo ifconfig`. Requires the `vmnet-bridge` sudoers entry at `/etc/sudoers.d/vmnet-bridge`.
...
The agents that give the haus its opinions. One serves a 27-billion-parameter model. One synthesizes speech. One listens for a wake word and responds as a fictional Jedi. Totally standard residential infrastructure.
