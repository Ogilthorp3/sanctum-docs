...
Runs two processes: the guardrail injector (Starlette/uvicorn on port 4000) which handles request sanitization, content-based routing, prompt caching, PII scrubbing, and assistant prefill stripping; and LiteLLM (port 4001) which handles model resolution, tiered fallback chains, and analytics. All external traffic enters through port 4000. The bouncer and the bartender, working the same door.
