...
The first domino. Everything else assumes the VM is running and the bridge exists.
...
Manages voice capture, wake-word detection, and Yoda personality interactions. The part of the system that listens for someone to say the magic words, then responds in the cadence of a small green Jedi. A completely normal thing for a house to do.
...
Runs the `cloudflared` tunnel daemon for the configured tunnel name (e.g., `haus`). Routes external traffic to internal services like Home Assistant and the health ingester.
...
Watches iCloud Drive directories and automatically files documents into organized folder structures. Digital Marie Kondo, but for PDFs.
...
| **KeepAlive** | Yes |
| **RunAtLoad** | Yes |
| **Ports** | `4000` (guardrail injector, all interfaces), `4001` (LiteLLM, localhost only) |

Runs two processes: the guardrail injector (Starlette/uvicorn on port 4000) which handles request sanitization, content-based routing, prompt caching, PII scrubbing, and assistant prefill stripping; and LiteLLM (port 4001) which handles model resolution, tiered fallback chains, and analytics. All external traffic enters through port 4000.
...
Periodically checks the health of all enabled services and auto-heals failures via `service-doctor`. Not KeepAlive — uses launchd's `StartInterval` for periodic execution. Every ten minutes, it wakes up, looks around, makes sure nothing is on fire, and goes back to sleep.
...
LaunchDaemons run as root and load at system boot (before user login). There is exactly one, and it exists because of the number 80.
