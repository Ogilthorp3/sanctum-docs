...
![A pencil-sketched apothecary cabinet at night, dozens of labelled drawers in neat rows, a keeper holding a lantern reading one label; each drawer glows faintly teal, a few amber; one tiny drawer ajar showing a curled wisp.](./images/hero-service-inventory.png)

This is the ground-truth registry of what actually runs on the Mini: every `com.sanctum.*` launchd job and every listening TCP port, each with a one-line purpose. It exists because the weekly docs-drift audit kept finding services that were running but undocumented — this page closes that gap and is the canonical place a new daemon or port earns its row.

<Aside type="note">
Two audit blind spots were fixed alongside this page: the drift check now reads port numbers from this inventory (and `port-summary.mdx`), not just the three files it scanned before, and it skips the volatile dynamic-port range (49152+) so transient third-party sockets (Lima, OrbStack, Rapport helpers) no longer read as drift. It still only enumerates user LaunchAgents, so a handful of system LaunchDaemons (proxyd, watchdog, castellan) can show as "phantom" — documented but not in its scan — which is a scan limitation, not a real absence.
</Aside>

## Daemons (launchd jobs)

Every entry is loaded on the Mini. "Domain" is user-agent (`~/Library/LaunchAgents`) unless noted; "cadence" is how it is triggered — a long-running daemon, a fixed interval, a calendar schedule, or a one-shot at boot.
...
| `com.sanctum.mlx-devstral-rust` | user-agent | daemon (KeepAlive SuccessfulExit:false + RunAtLoad) | Runs the sanctum-mlx Rust inference server serving Devstral-Small-2-24B-4bit over mTLS on loopback:3301 (no-plain). |
...
| `6669` | sanctum-service | sanctum-mlx - ~/Projects/sanctum-rs-cathedral/targ | Cathedral sanctum-mlx inference server (MoE Qwen seat), localhost + ::1 |
...
| `8009` | sanctum-service | Python /opt/homebrew/.../Python - -m workers.stt_server (PID | Sanctum STT (speech-to-text) worker server on loopback |
...
| `8765` | sanctum-service | Python /opt/homebrew/.../Python - -m orchestrator.server (PI | Sanctum orchestrator server on loopback |
