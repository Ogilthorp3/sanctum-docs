...
| `haus.sanctum.pq-terminator.1111` | user-agent | daemon (KeepAlive/RunAtLoad) | Runs the sanctum-pq-terminator post-quantum TLS terminating proxy listening on 0.0.0.0:1111 and forwarding to local backend unix://~/.sanctum/run/command-center.sock (le-manoir cert). |
| `haus.sanctum.pq-terminator.2222` | user-agent | daemon (KeepAlive/RunAtLoad) | Runs the sanctum-pq-terminator PQ-TLS proxy on 0.0.0.0:2222 forwarding to local backend unix://~/.sanctum/run/health-center.sock (term cert). |
...
| `2222` | sanctum-service | sanctum-pq-terminator - ~/.sanctum/pq-terminator/sanctum-pq-terminator | Post-quantum TLS terminator: listens on 0.0.0.0:2222 with PQC cert/key, proxies to backend unix://~/.sanctum/run/health-center.sock |
