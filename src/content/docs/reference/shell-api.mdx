...
Two `source` lines at the top of any script and the entire `sanctum_*` namespace is yours. The first gives you the config reader. The second gives you a way to make the haus say something out loud.
...
The second one is optional unless you plan on raising your voice; the first one is not.
...
sanctum_get instance.slug  # sanctum-hub
...
echo "$expanded"  # User is operator at 192.168.1.10
```
...
These functions query the `nodes` section of the instance config. They accept a `<node_id>` argument (e.g., `hub`, `satellite`). Useful when your haus automation hobby has metastasized into a multi-site distributed system.
...
```bash
sanctum_node_type hub        # hub
sanctum_node_type satellite  # satellite
```
...
```bash
sanctum_node_host hub        # 192.168.1.10
sanctum_node_host satellite  # satellite.local
```
...
```bash
sanctum_node_ts_ip hub  # 100.0.0.20
```
...
```bash
sanctum_node_ts_name hub  # hub-mac-mini
```
...
```bash
sanctum_node_user hub  # operator
```
...
```bash
if sanctum_node_online satellite; then
...
```bash
# Interactive shell
sanctum_node_ssh satellite

# Run a remote command
sanctum_node_ssh satellite 'uptime'
```
...
```bash
sanctum_node_ssh_ts satellite 'df -h'
```
...
```bash
sanctum_node_vm_ssh hub 'systemctl --user status openclaw-gateway'
```
...
```bash
if sanctum_node_enabled satellite home_assistant; then
  echo "Chalet runs HA"
fi
```
...
```bash
sanctum_node_service hub gateway port  # 1977
```
...
Source `~/.sanctum/lib/notify.sh` for notification support. Because when your haus has something to say, it should be able to say it.
