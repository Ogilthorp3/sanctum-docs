...
description: Complete reference for the sanctum_* shell functions provided by config.sh and notify.sh.
---
...
All functions are also available under the legacy `fos_*` prefix. For example, `fos_get` is an alias for `sanctum_get`, `fos_enabled` for `sanctum_enabled`, and so on. The `fos_*` aliases will remain indefinitely but new code should use `sanctum_*`.
</Aside>
