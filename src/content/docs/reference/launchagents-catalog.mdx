...
### com.sanctum.lmstudio-bridge (retired 2026-06-07)

| Property | Value |
|----------|-------|
| **Label** | `com.sanctum.lmstudio-bridge` |
| **Purpose** | Expose the Mac-local LM Studio listener to the VM bridge address |
| **Status** | Retired 2026-06-07, alongside the LM Studio coder seat |

This small bridge LaunchAgent forwarded the VM-facing `10.0.0.1:1234` listener to the Mac-local LM Studio process bound on `127.0.0.1:1234`. The VM did not need the whole desktop. It needed one reliable door — until the local coder seat moved onto a Rust MLX server (`com.sanctum.mlx-devstral-rust` on `:3301`) and the door led nowhere.
...
| **KeepAlive** | Yes |
| **RunAtLoad** | Yes |
| **Bridge Port** | `1984` (loopback-bound) |
| **Discovery Port** | `8833` (Firewalla P2P) |

The original one-shot `com.sanctum.firewalla` plist is retired (`.pre-w7.disabled`); the bridge is now babysat by this watchdog instead. It supervises `firewalla-bridge.js`, which discovers the Firewalla box over the P2P port `8833` and exposes a bearer-token HTTP API on `1984`, bound on loopback.
...
| **Purpose** | `sanctum-tts` (Rust) TTS front on `:8008`, forwarding to the Chatterbox `cbx-yoda` engine on `:8011` |
| **Required Service** | `tts` |
| **KeepAlive** | Yes |
| **RunAtLoad** | Yes |
| **Port** | `8008` |

Provides TTS for the voice agent. Replaced `com.sanctum.xtts-server` on 2026-04-19 once Qwen3-TTS proved equal quality with lower memory pressure. Old XTTS plist retained as `.retired` for archaeology, not load. The engine has since turned over again: today `sanctum-tts` (Rust) fronts the port, forwarding to Chatterbox `cbx-yoda` on `:8011`.
...
| **RunAtLoad** | Yes |
| **Port** | `127.0.0.1:8900` |
| **Backends** | `council-secure` → `:1337` Qwen3.6-35B-A3B (mTLS) · `coder` → `:3301` Devstral-Small-2-24B · `cloud` → `:3456` Claude Opus via the Max bridge · `spatial` → Gemini 3.1 Pro |

The `com.sanctum.server` label is the **routing** MLX server — `sanctum-server`, launched via `~/.sanctum/bin/sanctum-server-launch` with `--router-config instance.yaml`, on loopback `:8900` (`/health` reports `mode: routed`, four backends usable). It loads no resident model of its own — pure router. It does not own port 4040, and it is not `proxyd` — a long-standing doc conflation that this entry exists to kill.

**`proxyd` is a separate binary** (`sanctum-rs`) on `*:4040`: the bouncer-and-bartender doing sanitization, content routing, prompt caching, PII scrubbing, prefill stripping, model resolution, and tiered fallback. It runs as the system LaunchDaemon `com.sanctum.proxyd` — single instance, HTTPS with mTLS on `:4040`, keys injected from `~/.sanctum/secrets/`. It earned its plist in the end: `com.sanctum.proxyd` — never `com.sanctum.server`.
...
Forwards VM traffic from `10.10.10.1:18080` to the Orbi web UI (`:80`) and `:18085` to the Orbi API (`:5000`), because the VM has no direct LAN access. The Orbi's LAN IP is **not hardcoded** — `orbi-bridge.sh` resolves it from `endpoints.env` (`SANCTUM_ORBI_LAN`) or `instance.yaml` (`network.orbi_lan`), per the no-hardcoded-endpoints doctrine that earned a memory entry the day the old Bell-subnet literal went stale.
...
| **Status** | Loaded — system LaunchDaemon (`com.sanctum.watchdog-rust.plist`), status API on `127.0.0.1:2187` |

For a long time this was the most relatable agent in the fleet: every so often it woke up, looked around, made sure nothing was on fire, and went back to sleep. The Rust binary `sanctumd` does the sweep and auto-heals through `service-doctor`, with an intentional file-vs-label drift — `com.sanctum.watchdog-rust.plist` on disk, `com.sanctum.watchdog` as the label inside.

It took one retirement (`.pre-w6.disabled`) while the generic sweep-and-heal role was carved up among specialists — `council-guardian` heals the MLX seat, `launchd-health-sentinel` surfaces stuck jobs, `r2d2` runs the classify-and-fix loop — and then came back as a live system LaunchDaemon. The most relatable agent in the fleet turned out to be the hardest to replace after all.
...
Retired 2026-06-07 alongside the LM Studio coder seat itself — the local coder moved onto a Rust MLX seat on `:3301` (Codestral-22B at the time; today `com.sanctum.mlx-devstral-rust` serving Devstral-Small-2-24B-Instruct-2512-4bit), which needs no SIGCONT nursemaid. The guardian's old habit of re-loading an `EXPECTED_MODEL` every 60s kept fighting manual `sanctum-admit` releases anyway. The triage daemon now covers what's left of the memory-pressure unloads.
...
Runs on a calendar schedule, not at boot. Rotates the Keychain- and SOPS-held secrets (gateway tokens, API keys) — 1Password stays the human-edited source of truth; automation never touches it. The only agent that doesn't start at login — it waits for its appointed hour like a well-mannered assassin.
...
### com.sanctum.world-model

| Property | Value |
|----------|-------|
| **Label** | `com.sanctum.world-model` |
| **Purpose** | HA Green z-score predictor — history, Thalamus surprises, Tommy top-3 |
| **KeepAlive** | No |
| **RunAtLoad** | Yes |
| **StartInterval** | 900 |

The July 2026 script lived in `~/.sanctum/scripts/` and vanished. This plist points at `~/Projects/sanctum-emergence/world_model/world_model.py`. Kill switch: `~/.sanctum/state/world-model-disabled`. Doctrine: [The World Model](/architecture/world-model/).

---

## Work lane (Triptyq)

These two run on the MBP. They write the **work** CRM (Affinity), not the haus DuckDB archive. The Affinity key is work-lane. See [Affinity filing](/architecture/affinity-filing/).

### com.triptyq.file-mail

| Property | Value |
|----------|-------|
| **Label** | `com.triptyq.file-mail` |
| **Purpose** | File portco mail attachments + investor-update / Granola notes onto Affinity |
| **KeepAlive** | No |
| **RunAtLoad** | No |
| **StartInterval** | 7200 |
| **Session** | Aqua |

Wrapper and plist are symlinks into `~/Projects/triptyq-cli/launchd/`. The wrapper sources `tq-repo.sh` so a shared venv whose editable `.pth` points at another worktree cannot silently run last week's CLI. Scar: 2026-08-15, thirteen failures, [the venv that ran main](/operations/2026-08-15-the-venv-that-ran-main/).

### com.triptyq.weekly-brief

| Property | Value |
|----------|-------|
| **Label** | `com.triptyq.weekly-brief` |
| **Purpose** | Friday digest for the Monday partners meeting; ensures founder reminders first |
| **KeepAlive** | No |
| **RunAtLoad** | No |
| **StartCalendarInterval** | Friday 16:07 |
| **Session** | Aqua |

Read-only briefing after an idempotent `reminders --execute`. Counts-only macOS notification; full markdown stays in `~/.triptyq/digests/`.
