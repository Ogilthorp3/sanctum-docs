...
The `com.sanctum.server` label is the **routing** MLX server — `sanctum-server`, launched via `~/.sanctum/bin/sanctum-server-launch` with `--router-config instance.yaml`, on loopback `:8055` (`/health` reports `mode: routed`, four backends usable). It loads no resident model of its own — pure router. It does not own port 4040, and it is not `proxyd` — a long-standing doc conflation that this entry exists to kill.
