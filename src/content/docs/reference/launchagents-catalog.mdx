...
This is the per-agent rundown. The parent [LaunchAgents & LaunchDaemons](/reference/launchagents/) page covers the boot chain, daemon configuration, plist generation, and `launchctl` management. This page is the catalog: every agent grouped by role, what it does, and the gotchas that earned a paragraph the hard way.
