...
This is the per-agent rundown — the bestiary. The parent [LaunchAgents & LaunchDaemons](/reference/launchagents/) page covers the boot chain, plist generation, and the choreography of `launchctl bootstrap`. This page is every agent grouped by role, what it's responsible for, and the specific 3 AM incident that earned each one its paragraph. Twenty-three entries, all of them running right now if everything is working, all of them silent if everything is working. Notice them at your peril.
