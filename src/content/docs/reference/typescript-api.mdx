...
| Parameter | Type | Description |
|-----------|------|-------------|
| `nodeId` | `string` | Node identifier (e.g., `"hub"`, `"satellite"`) |
| `keyPath` | `string` | Dot-delimited path within the node config |

```typescript
nodeGet('hub', 'tailscale_ip');
// "100.0.0.20"

nodeGet('satellite', 'type');
// "satellite"
...
```typescript
const online = await isNodeOnline('satellite');
if (online) {
...
```typescript
isNodeServiceEnabled('hub', 'gateway');
// true

isNodeServiceEnabled('satellite', 'vm');
// false
```
