...
The TypeScript config module at `command-center/lib/config.ts` provides typed access to the Sanctum instance configuration. It reads from the same [`~/.sanctum/instance.yaml`](/reference/instance-yaml/) file as the [shell library](/reference/shell-api/) and is used by the [command center dashboard](/guides/dashboard/) and any Node.js tooling.
...
These functions read the [node topology](/architecture/node-topology/) — hubs, satellites, and the sync arrows between them — straight from the config. No probes, no network; just what the YAML declares.
