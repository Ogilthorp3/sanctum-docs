...
Here's where it gets real. Every service your haus runs — the agent gateway, the voice engine, the haus automation hub, the offline library — each one gets an `enabled` boolean and, if it listens on a port, a `port` key. Flip the boolean, regenerate plists, and the service appears or vanishes. Configuration as incantation.
...
| Dashboard | `dashboard` | `3333`, `1111` | Command center web UI (`3333` Web, `1111` API) |
| Firewalla Bridge | `firewalla` | `1984` | Firewalla P2P bridge |
| VM | `vm` | — | QEMU virtual machine |
| Voice Agent | `voice_agent` | `1138` | Yoda voice interaction agent |
| Qwen3-TTS | `qwen3-tts` | `8008` | Text-to-speech server (mlx-audio backend; XTTS-v2 retired 2026-04-19) |
| MLX Server | `mlx_server` | `1337` | Council MLX model server |
...
Seventeen core services shown above. The full catalogue is 38 services. On a Mac Mini. Under a desk. Running a haushold.
...
| `members[].role` | `string` | Role within the haushold |
...
| Key | Type | Description |
|-----|------|-------------|
| `<node_id>` | `map` | Node identifier (e.g., `hub`, `satellite`) |
...
nodes:
  ...
  satellite:
    ...
    tailscale_name: satellite-mac-mini
    user: operator
    services:
      gateway:
        enabled: true
