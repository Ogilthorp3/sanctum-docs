...
![The command line — where every keystroke is a conversation with a system that remembers everything](../../../assets/illustrations/cli-terminal.png)
...
Today that executes the workspace audit and runtime audit suites directly, so the operator entry point and the test wall stay coupled.
