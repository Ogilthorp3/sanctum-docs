---
title: CLI Reference
description: Command-line tools and scripts for managing a Sanctum instance.
---
...
```bash
ssh bert@10.10.10.1 \
  'PATH=/Users/bert/.local/share/fnm/node-versions/v22.22.0/installation/bin:/opt/homebrew/bin:$PATH \
...
<Aside type="note">
The VM-to-Mac path requires a PATH prefix because `fnm`-managed Node.js is not in the default SSH `PATH`.
</Aside>
...
<Aside type="caution">
Restore does not automatically reload LaunchAgents. After a restore, you should either reboot or manually reload affected services via `launchctl`.
</Aside>
