...
The bridge URL defaults to `http://127.0.0.1:4078` and is configurable via the `SANCTUM_BRIDGE_URL` environment variable. Port 4078 — the FDA-privileged proxy sitting one door down from Force Flow (4077). No cultural mythology, just collision-free adjacency and the quiet satisfaction of a port that does exactly what it says.
