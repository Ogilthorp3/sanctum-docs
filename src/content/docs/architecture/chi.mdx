...
At 3 a.m. a healing script fires on a service that was never sick. Nothing is wrong. Nothing asked for it. It ran last night, it will run tomorrow, and every gauge in the haus reports green while the current quietly drains into a decision no one remembers making.

The body has organs for substance (bones, flesh), for signaling (nerves, the endocrine system), for defense (the immune system), for awareness ([chitti](/architecture/chitti/)), and for governance (the Council). Each answers a piece of the haus's condition. None of them own the *energy itself* — what is burned, whether it flows, whether the body is rested, whether an action like that 3 a.m. reflex is still *willed*.
...
- **RAM** from [the castellan's](/architecture/the-castellan/) `/status` (it never re-meters or admits — that stays the castellan's door).
- **Tokens and dollars** from [the proxy's](/architecture/proxy/) usage ledger.
...
Chi **feeds, it never doses.** It emits a single burn-rate/runway scalar where the [endocrine gland](/architecture/endocrine-system/) can read it; the existing homeostat — and only it — gates the haus's ambition. Chi stands up no second regulator that can panic.
...
Its named opposite (its [polarity](/architecture/polarity/)) — the force that catches it when it is wrong — is the post-failure ground truth: the breakers, the 503s, the refusals. If chi cannot be paired with that opposite, it has become an authority, and the design is wrong.
