...
models:
  ...
  council-brain:
    ...
    quality_tier: 1
    smart_route: true
    fallback: [minimax-m27, qwen35-plus, council-code, council-27b]
  council-code:
    provider: local
    quality_tier: 1
    smart_route: false
    fallback: [lmstudio-qwen, council-27b, qwen35-plus]
  council-routine:
    provider: openrouter
    quality_tier: 2
    smart_route: true
...
The important practical split is this: `council-brain` stays the generalist tier, while `council-code` is the local coding lane. The proxy does the triage. Code-shaped requests can be rerouted to `council-code` at the routing layer, which means the decision is visible in config and logs instead of buried in agent prompt folklore.

## Test Coverage

70 tests. The original routing coverage is still there, but it now sits beside live checks for local LM Studio reachability, MLX idle-managed routing, fallback resilience, and Claude Code OAuth passthrough. Because if your local tiers are part of the promise, they belong in the test count and in the blame when they break.
