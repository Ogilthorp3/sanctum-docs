...
- the proxy runs as the system LaunchDaemon `com.sanctum.proxyd`, launched through `~/.sanctum/bin/proxyd-launch` -- a wrapper that injects the Gemini and OpenRouter keys from `~/.sanctum/secrets/` and pointedly loads *no* Anthropic key (the latest Opus arrives via the Claude Max bridge on `:3456`, so the proxy never holds a metered Anthropic credential)
