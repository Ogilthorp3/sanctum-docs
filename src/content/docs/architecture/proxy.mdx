...
It listens on port 4000. It knows about 17 models across 4 providers. It maintains 12 fallback chains. And it has a budget system that will cut off an agent mid-sentence if the math says so. The proxy does not negotiate. The proxy does not feel bad about it.
...
![The routing engine — six layers of increasingly opinionated decision-making](../../../assets/illustrations/living-force-router.png)

```
                    ┌─────────────────────┐
  Incoming Request  │                     │
  ─────────────────>│  1. QUALITY TIER    │  Tier 0? NEVER reroute.
                    │     claude-opus-4-6  │  Go directly to Anthropic.
                    │     council-secure   │  Do not pass Go.
                    └────────┬────────────┘
                             │ not tier 0
                    ┌────────▼────────────┐
                    │  2. TASK TYPE        │  Vision → Gemini.
                    │     vision, code,    │  Code/security → strongest
                    │     security         │  available model.
                    └────────┬────────────┘
                             │ no strong match
                    ┌────────▼────────────┐
                    │  3. COST             │  Claude free tokens left?
                    │     free → cheap     │  Use them. Then OpenRouter.
                    │     → paid → local   │  Then Gemini. Then local.
                    └────────┬────────────┘
                             │ routed
                    ┌────────▼────────────┐
                    │  4. LATENCY          │  (Future) Prefer local
                    │     placeholder      │  for health checks and
                    │                      │  heartbeats.
                    └────────┬────────────┘
                             │
                    ┌────────▼────────────┐
                    │  5. PROVIDER HEALTH  │  >30% error rate?
                    │     error tracking   │  Skip that provider.
                    │     exp. decay       │  Try the next one.
                    └────────┬────────────┘
                             │
                    ┌────────▼────────────┐
                    │  6. BUDGET BREAKER   │  Agent over daily limit?
                    │     hard cutoff      │  All traffic → local.
                    │     no exceptions    │  The conversation is over.
                    └─────────────────────┘
```
