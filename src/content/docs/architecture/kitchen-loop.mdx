...
In March 2026, a paper appeared on arxiv (2603.25697) describing a framework that had autonomously merged 1,094 pull requests across two production systems with zero regressions detected by its regression oracle. The framework — called the Kitchen Loop — treats code evolution as a continuous, specification-driven process where AI agents exercise a product as power users at 1,000x human cadence, then fix what breaks, verify the fixes against ground truth the implementer cannot fake, and measure drift to ensure the system never gets worse.

Reading it felt like looking at a mirror. Not because Sanctum already does this — it doesn't, not fully — but because every piece of the framework maps to something the Living Force already attempted, half-built, or wished it had.

This page documents how Kitchen Loop's four pillars integrate with Sanctum's existing architecture, what we gain, and what changes.
...
## What Sanctum Already Has

The Living Force is not starting from zero. Several Kitchen Loop concepts already exist in some form:

| Kitchen Loop | Sanctum Today | Gap |
|---|---|---|
| Regression Oracle | Fire drills (Phase 5) | Monthly and manual. Kitchen Loop runs every iteration. |
| Multi-Model Tribunal | Council Sessions | Sanctum uses the same model family. Kitchen Loop uses heterogeneous models to prevent groupthink. |
| Pause Gates | Crash-Loop Quarantine | Sanctum quarantines broken *services*. Kitchen Loop pauses broken *development velocity*. |
| Spec Surface | `instance.yaml` | Config defines what runs. Spec surface defines what the system claims to do — and tests every claim. |
| Self-Healing | Code-Forge + HA Self-Healer | Close match. Kitchen Loop formalizes the feedback loop with structured experience reports. |
| Night Deployment | Night Window (02:00-05:00 ET) | Identical philosophy. Kitchen Loop adds the concept of drain mode when PR queues back up. |

The gap is not capability — it's formalization. The Living Force heals reactively. The Kitchen Loop tests proactively, measures continuously, and stops itself when quality degrades.
...
The paper found that debates between instances of the same model family converge to groupthink rapidly. Three heterogeneous models (Gemini, GPT/Codex, Claude) produced genuine perspective diversity. Sanctum's Council currently routes through the same proxy chain.

The integration point is surgical: for critical decisions (architectural changes, security policy updates, deployment approvals), route council votes through diverse model backends:
...
│  "Turn on all lights, arm the house, check      │
...
<Aside type="note">
The Kitchen Loop framework is open source (MIT) at `github.com/0xagentkitchen/kitchenloop`. It ships as shell scripts and Python utilities designed to wrap around Claude Code. The integration with Sanctum would use its scaffolding directly, with domain-specific customization for home automation.
</Aside>

The recommended sequence, ordered by value delivered per effort:

1. **Specification Surface** — enumerate Sanctum's testable claims in YAML. This is documentation work with immediate value: it forces every "we support X" to become a testable assertion. No code required.

2. **L4 State Delta Checks** — extend the Living Force watchdog to verify end-to-end outcomes, not just service health. Start with the five most common automations (goodnight scene, morning routine, presence detection, temperature regulation, security arm/disarm).

3. **Regression Oracle** — build a repeatable test suite that answers "is Sanctum at least as good as yesterday?" Run it nightly during the existing night window. The oracle consumes the spec surface and produces a pass/fail matrix.

4. **Anti-Signal Canaries** — inject known-bad inputs into the agent pipeline monthly. Measure catch rates. Track improvement over time. Start with Tier 1 (obviously bad) and graduate to Tier 3 (adversarial) as the system matures.

5. **Heterogeneous Tribunals** — route critical council decisions through diverse model backends. Requires proxy routing changes but no architectural redesign.

6. **Full Kitchen Loop** — once the oracle and spec surface are stable, enable the complete six-phase autonomous loop. The system exercises itself, finds bugs, fixes them, verifies the fixes, and measures drift — all while the household sleeps.
...
Together, they close the gap between "the service is running" and "the house is actually doing what the family expects it to do." Which is, if you think about it, the only question that matters.
