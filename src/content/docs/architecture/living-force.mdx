...
## The Missing Membrane — Chitti

The Living Force has bones (services), muscles (agents), and an immune system (watchdog, sentinel, pressure-valve, canary). What it does not yet have is the **fascia** — the connective tissue that lets bones and muscles coordinate with the immune system in real time, under pressure, everywhere at once.

In yogic philosophy the word is *chitti-shakti* — pure consciousness, the ever-changing awareness-under-pressure that sits between embodiment and mind. In biotensegrity research, the equivalent is the fluid matrix of the fascial network — it is both the medium signals travel through and the tissue that rearranges itself in response to load. Ground fluid and fluid ground. One modulates (slow feedback: learned thresholds). The other regulates (fast feedback: this tick, act or don't).

Sanctum has fragments of this already, but no membrane:

- **Memory sentinel** knows pressure, but only runs at discrete ticks.
- **Pressure valve** knows pressure deeply, but only the memory axis.
- **The memory gate** on the coder-14b loader knows that under pressure, *not acting* is the right remediation — but only for one specific action, in one module.
- **Sanctum-presence** is a shared awareness field, but narrow: it knows who holds which lock, nothing else.
- **Force Flow** is the bus agents talk on, but a courier — not a fluid.

The gap is the **continuous ambient state layer** every subsystem would consult before acting: memory / thermal / swap pressure, mid-flight work on shared resources, attention (which agent has the floor, which service is experimental), recent action history. Today the watchdog has five separate ad-hoc membranes — memory sentinel, circuit breaker, restart budget, dedup, the new memory-gate — each asking a narrow version of the same question: *given what's happening right now, should this action fire?*

A chitti layer collapses those into a single fluid the whole system reads and writes. Not a new agent — a change in how every agent touches the ground. See **[the chitti architecture proposal →](/architecture/chitti/)**.
