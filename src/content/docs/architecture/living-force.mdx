...
3.  **Namespace Unification**: The legacy `com.manoir` prefix was officially retired. All services now use the `com.sanctum` namespace.
4.  **Intelligent Memory Triage**: Updated `qui_gon_triage.py` to allow LM Studio to run permanently, while automatically unloading "large" models (35B+) if system memory drops below 20%.
5.  **Security Hardening**: Password authentication disabled on the VM in favor of Key-Only SSH.
