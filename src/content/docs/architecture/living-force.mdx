...
<CardGrid>
  ...
  <Card title="Phase 8: Centralized Calibration" icon="setting">
    Hardcoded coordinates are being eliminated. `holocron-config.yaml` is the declared single source for the Holocron's ports and paths, and `run_sanctum.sh` reads it via `yq` at ignition to calibrate every sector — UI, command center, health, archives, and the Neural Link (Port 1138). The Sanctum Watchdog (Port 2187 — the daemon `sanctumd` from `sanctum-rs`, label `com.sanctum.watchdog`, which serves the Living Force API) is the holdout: it still carries its own coordinates and is next in line to read the master file. The organism is learning to read its own DNA; it just hasn't memorized every gene yet.
  </Card>
</CardGrid>
...
1. **Diagnose** (`ha-diagnose.sh`) — queries the HA API for every integration, entity, and automation. Assigns a severity: 0 (healthy), 1 (advisory), 2 (attention), 3 (urgent), 4 (critical).
...
<Aside type="note">
The healer respects a cooldown: 60 minutes between Tuya integration reloads, tracked by a `tuya-reload-cooldown` timestamp file. A flapping integration would otherwise trigger an infinite loop of reloads. The Playwright re-auth stage gates on `needs_reauth` rather than its own clock — a per-session Playwright cooldown is the next gap to close. The system learned patience the hard way, which is to say the operator learned patience the hard way, and then taught half of it to the system.
</Aside>
...
**The Apple Mail Healer.** On April 15, 2026, the Living Force extended into user-space. Several desktop apps — Apple Mail among them — keep a local SQLite database that only syncs while the app is running, so an app that quietly dies stops syncing while looking perfectly fine. Mail got the first concrete healer: `check-apple-mail.sh` does a `pgrep -x Mail`, and `heal-apple-mail.sh` parses the last 15 minutes of `Mail-*.ips` crash logs, and on the `NSCalendarDate` signature clears the corrupted `Envelope Index` files before reopening with `open -a Mail`. The sync pipeline stopped falling behind. Generalizing the same pgrep-and-reopen reflex to Messages, WhatsApp, Signal Desktop, and Telegram is the obvious next move — Mail just earned its healer first by crashing first.
...
<Aside type="tip" title="10. (Neuro)diversity is Paramount">
  ...
  **At the model layer** — the Council must be plural. Five reasoning seats, four model families, four sets of training biases. A council of one model wearing identical robes is not a council; it's a feedback loop with extra steps. (See [(Neuro)diversity is Paramount](./neurodiversity-doctrine.mdx) for the live routing layout — every seat on a different model family, rendered from the one JSON file the [Council page](./agents.mdx) owns, never restated by hand.)
