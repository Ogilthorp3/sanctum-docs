...
<Aside type="tip" title="12. Closed-Loop Enforcement — Never Trust the Command, Verify the State">
  A command that returns "success" is a claim, not a fact. Every write to an external control plane (Firewalla, launchd, a router, a thermostat) must be followed by a read of the resulting state, and the two must be compared. `if result:` is not verification. `assert new_state == intended_state` is. When they disagree, log DRIFT — loudly and with the observed value — and retry once before escalating. The April 18 Apple TV incident happened because the code trusted a bridge response that literally said `success: false` on HTTP 200. Five devices kept streaming while the log insisted they were blocked. The system was lying about its own work. Principle 9 says dashboards can't lie; this is its operational twin — commands can't lie either.
</Aside>
