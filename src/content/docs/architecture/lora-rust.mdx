...
<Aside type="note">
The original 27B-dense binary OOM'd on the 64GB Mac Mini — it loaded the whole model at once, and Python's `mlx_lm.server` got to stay there on lazy mmap. The fix wasn't lazy loading in Rust. It was switching the council to a 35B-A3B MoE (only ~3B params active per token) and bolting on explicit Metal caps: `--metal-memory-limit-mb 28672 --metal-wired-limit-mb 18432`, plus `--turboquant`. The Mac Mini now runs the Rust binary in production on port 1337. Python survives as the small Qwen3.5-9B fallback on `:8901` — kept around for the same reason you keep a spare tire: not because you expect to, but because the day you need it you really need it.
</Aside>
