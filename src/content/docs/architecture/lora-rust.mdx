...
Same tok/s because the Metal GPU is the bottleneck — both runtimes are feeding the same silicon. Rust wins on everything around the inference: startup, memory, determinism. The 250MB of saved overhead doesn't matter much on a 128GB machine, but it matters a great deal on a 64GB machine that's also running LM Studio, Home Assistant, Qwen3-TTS, and whatever else decided to be important today.
