...
services:
  mlx_coder:
    enabled: true
    port: 1338                         # mTLS
    model_path: ~/.cache/huggingface/hub/models--mlx-community--Qwen2.5-Coder-14B-Instruct-4bit
  council_mlx:
    enabled: true
    port: 1337                         # mTLS
...
<Aside type="note">
The pre-2026-05-11 catalog had a `lm_studio: port: 1234` block here. LM Studio was retired in favor of `sanctum-mlx-coder` once the pure-Rust path proved equal quality with stricter operational controls (mTLS, manifest verification, signed weights). The `:1234` socket is still reachable — it's now a plain-HTTP `socat` bridge to `sanctum-mlx-coder` on `:1338` for callers that haven't moved to mTLS yet.
</Aside>
