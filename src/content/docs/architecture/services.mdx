...
Agents use a **tiered model strategy** — each agent tier has its own primary model and fallback chain:
...
<Aside>
  LiteLLM (port 4001) handles model resolution and fallback routing. The guardrail injector (port 4000) handles request sanitization and content-based routing. All external traffic enters through port 4000.
</Aside>
