...
| Council-27B MLX | `1337` | leet — the local workhorse | `com.sanctum.server-mlx` | `sanctum-server` front-end with a built-in idle manager. It serves the fine-tuned Qwen 3.5 27B distilled model through a local wrapper on `1337` and wakes the MLX backend on `8900` only when someone actually asks it to think. |
...
| `council-brain` | Claude Opus 4.6 | minimax-m27 → qwen35-plus → council-code → council-27b | Yoda, Mon Mothma, Jocasta |
| `council-code` | Qwen 2.5 Coder 14B (LM Studio) | lmstudio-qwen → council-27b → qwen35-plus | Any agent when the request is code-shaped |
...
| `council-heartbeat` | Council 27B (local) | council-27b → minimax-m25-free | All (heartbeat checks) |
...
0. Smart content-based routing (Gemini-first for non-code tasks, `council-code` for code-shaped work)
...
There are eight of them. Two are SSH tunnels. Two are mDNS broadcasters. One talks to the router by first authenticating through a cloud server three thousand miles away. One talks to Sonos speakers because Docker couldn't. Home networking in 2026 is the kind of helpful that gets people killed in horror movies.
...
| VM Autostart (QEMU headless) | -- | `com.sanctum.utm-autostart` | Launches QEMU headless on login, starts the Ubuntu VM, and sets the bridge100 interface IP to 10.10.10.1. |
...
<Tabs>
  ...
  <TabItem label="Health Check">
    ...
    ```bash
    # Manual health check
    sanctum_enabled gateway && curl -sf http://localhost:1977/health
    ```
  </TabItem>
</Tabs>
...
| 1337 | Council-27B MLX | Mac | LEET | Because nothing says elite hacker like a Metal-accelerated language model running inference in a home office. |
...
| 10101 | Health Ingester | Mac -> VM | Binary 21 | 10101 in binary is 21 — blackjack. The health ingester always hits, never stands. |
...
| 18081 | Signal Bridge | Mac | Orbital-S | One door down from the Orbi Bridge, handling messages instead of mesh nodes. They share a hallway but do not speak. |
