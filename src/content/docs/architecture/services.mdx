...
<Aside type="note">
The pre-2026-05-11 catalog had a `lm_studio: port: 1234` block here. LM Studio was first retired in favor of `sanctum-mlx-coder` once the pure-Rust path proved equal quality with stricter operational controls (mTLS, manifest verification, signed weights), and for a while `:1234` survived as a plain-HTTP `socat` bridge to `sanctum-mlx-coder` on `:1338` for callers that hadn't moved to mTLS. As of 2026-06-07 all of that is gone: LM Studio is fully decommissioned (no `:1234`, no bridge/guardian), the `sanctum-mlx-coder` `:1338` seat is retired, and native MLX seats serve every model — the CODER role is now Codestral-22B on `:3301`.
</Aside>
...
| VM Autostart (Lima/vz) | -- | `com.sanctum.vm-autostart` | Starts the Ubuntu VM via Lima/vz on login (migrated off QEMU/HVF 2026-05-30) and restores the bridge100 interface IP. The old VM-facing `sanctum-mlx-coder` `:1234` socat bridge is gone with the coder seat's 2026-06-07 retirement. |
