...
![Sanctum services catalog — every service, every port, organized by category](../../../assets/illustrations/sanctum-services-catalog.svg)
...
| Council-27B MLX | `1337` | leet — the coding brain | `com.sanctum.council-mlx` | Quantized Qwen 3.5 27B 4-bit with per-agent LoRA adapters. Runs natively on Apple Silicon via MLX. KeepAlive enabled. |
| XTTS Voice | `8008` | BOOB — Yoda’s mouth | `com.yoda.xtts-server` | XTTS-v2 text-to-speech server. Runs on the MPS GPU for real-time voice synthesis. |
...
| Sanctum Proxy | `4040` / 4001 | .40 caliber — the gateway | `com.sanctum.litellm-proxy` | Two-layer LLM proxy: guardrail injector (port 4040) + LiteLLM router (port 4001). Handles tiered model routing, prompt caching, request sanitization, PII scrubbing, and automatic fallback. KeepAlive enabled. |
...
The **guardrail injector** (port 4040) intercepts all requests before they reach LiteLLM and applies a 7-step pipeline. Seven steps between an agent's thought and the outside world. The TSA wishes they were this thorough.
...
<Aside>
  LiteLLM (port 4001) handles model resolution and fallback routing. The guardrail injector (port 4040) handles request sanitization and content-based routing. All external traffic enters through port 4040. Two ports, seven pipeline stages, and a hope that the content blocks aren't empty again.
</Aside>
...
| Service | Port | Codename | LaunchAgent | Description |
|---------|------|----------|-------------|-------------|
| Firewalla Bridge | `1984` | Orwell — Big Brother watches | `com.openclaw.firewalla-bridge` | HTTP bridge to the Firewalla Purple router P2P API (port 8833). Bound to `0.0.0.0` so the VM can reach it. KeepAlive enabled. |
...
| Network Control | `4007` | 007 — licensed to ping | `com.user.ha-ssh-tunnel` | SSH tunnel forwarding port 4007 to the VM for Home Assistant integrations. |
| Health Ingester | `10101` | binary heartbeat | `com.user.health-ssh-tunnel` | SSH tunnel forwarding port 10101 to the VM for the health data ingester. KeepAlive enabled. |
...
Twenty-one ports. Two hosts. One network that exists entirely inside a closet. Every Deadpool port has a codename because if you're going to memorize port numbers at 2 AM in your underwear, they should at least be funny.
...
| 4007 | Network Control | Mac -> VM |
| 1984 | Firewalla Bridge | Mac |
| 10101 | Health Ingester | Mac -> VM |
| 1977 | OpenClaw Gateway | VM |
| 19001 | DenchClaw Gateway | Mac |
| 19001 | Holocron Chat | Mac |
| 21063 | HomeKit Bridge | Mac (Docker) |

<Aside type="note">
  The DenchClaw Gateway (Mac, port 19001) and OpenClaw Gateway (VM, port 1977) use different ports on different hosts. Earlier documentation incorrectly listed both at 1977. They never conflicted in practice because they live on separate hosts (10.10.10.1 and 10.10.10.10), but the port numbers are in fact distinct.
</Aside>
