...
| Voice Agent | 8090 | — | `com.sanctum.voice-agent` | Yoda voice interface. Integrates with Sonos speakers via the Sonos Bridge REST API on port 1969. |
...
| Cloudflare Tunnel | -- | — | `com.sanctum.tunnel` | Named tunnel `haus`. Exposes `health.nepveu.name` and `ha.nepveu.name` via Cloudflare Zero Trust. KeepAlive enabled. |
...
| mDNS Alias (manoir) | -- | `com.sanctum.mdns-alias` | Broadcasts `haus.local` via mDNS for LAN discovery. KeepAlive enabled. |
...
| 1337 | Council-27B MLX | Mac | LEET | Because nothing says elite hacker like a Metal-accelerated language model running inference in a home office in Quebec. |
...
| 4040 | Sanctum Proxy | Mac | Forty Cal | Sits at the intersection of every request that enters the house. Named after the .40-caliber round — the proxy that guards the gate carries accordingly. |
...
| 8090 | Voice Agent | Mac | Understudy | Living in 8008's shadow at a respectable 82-port distance. Handles voice routing without the calculator jokes. |
...
The newest addition to the port registry. Force Flow handles notification triage — deciding what deserves your attention and what gets filed under the house is fine, stop asking. We gave it 4077 because, like the 4077th M\*A\*S\*H, it operates in a permanent state of controlled chaos, sorting the critical from the cosmetic while maintaining a dark sense of humor about the whole situation. Hawkeye Pierce would approve of a system that intercepts alerts about your thermostat with the same gravity as a push notification about your front door.
</Aside>
