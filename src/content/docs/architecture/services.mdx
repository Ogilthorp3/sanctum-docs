...
Every Sanctum port — what it carries, where it lives, and the joke that justifies the number — is catalogued in [Port Summary](/architecture/port-summary/), along with the Deadpool Protocol that governs how new ports earn their numbers. The catalog is on its own page so this architecture overview can stay focused on services rather than network trivia.
