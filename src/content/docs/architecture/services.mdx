...
<Aside type="note">
  Port 1977 appears twice because the DenchClaw Gateway (Mac) and OpenClaw Gateway (VM) run on separate hosts (10.10.10.1 and 10.10.10.10). Same port number, different machines, zero conflict. The networking equivalent of two people named Steve in different departments.
</Aside>
