...
|---------|------|----------|-------------|-------------|
| Coder MLX | `1338` | — | `com.sanctum.mlx-coder` | Pure-Rust `sanctum-mlx-coder` server. Serves `Qwen2.5-Coder-14B-Instruct-4bit` — the operator's long-validated coding model, and Qui-Gon's primary. mTLS-only; a plain-HTTP `socat` bridge on `:1234` (`com.sanctum.coder-plain-bridge`) lets the proxy reach it. Replaced the LM Studio app that used to host this model. |
...
| VM Autostart (QEMU headless) | -- | `com.sanctum.vm-autostart` | Launches QEMU headless on login, starts the Ubuntu VM, restores the bridge100 interface IP, and re-establishes the VM-facing `sanctum-mlx-coder` plain-HTTP bridge on 10.10.10.1:1234 via `socat` (the mTLS surface lives on `:1338`). |
