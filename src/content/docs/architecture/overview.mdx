...
The bones are written in Rust and live in the [`sanctum-rs`](https://github.com/Ogilthorp3) workspace. They are the parts of the system that have stopped redesigning themselves. Once a service crosses from the Python side of the house — where features still sprout new limbs every week — into `sanctum-rs/services/`, it becomes structural: a single deploy binary, compile-time guarantees on its interfaces, and a launchd slot that either holds or doesn't.
...
| `sanctum-proxy` | 4040 | The cloud-tier model router. Classifies requests, applies daily cost caps, falls back four models deep before giving up. |
| `sanctum-server` | varies | Smart Router. Pattern/intent dispatch between agents. |
| `sanctum-mlx` | 1337 | Native Apple-Silicon inference with LoRA adapter merging. The local-secure brain. |
| `sanctum-firewalla` | 1984 | Bridge to the router, Orwell-port. iptables fallback over SSH when the REST API feels creative. |
| `sanctum-xtts` | 8008 | Text-to-speech on MPS. Every Sonos announcement starts here. |
| `sanctum-memory` | 42069 | Jina-v2-reranked memory store. Everything the agents ever learned about the haus. |
| `sanctum-idle` | — | Quiet sentinel. Unloads large models when RAM drops below the red line. |

Bones graduate, they don't grow. Each one spent eight to twelve months as a Python script before it earned its way into the skeleton, and the doctrine for that transition is documented in [Language Maturity](/architecture/language-maturity/) with the `tools/rust_readiness.py` harness that tells us which service is next in line.
...
The flesh is Python. It is how we find out what the service *is* before we carve it in Rust. Every service that currently sits in `~/.sanctum/force-flow/` or `screen-time-build/` is there because its feature shape is still moving — new credit mechanisms, new curfew edge cases, new channels added to the notification fan-out, a homework mode that keeps acquiring exemptions. Writing that in Rust would mean shipping less, slower, with fewer features that our family actually needs.
...
```
Mac Mini (10.10.10.1)  ◀────  bridge100  ────▶  Ubuntu VM (10.10.10.10)
   │                                                 │
   ├── 4040 sanctum-proxy                             ├── openclaw-gateway (systemd user)
   ├── 4077 force-flow                                ├── Yoda, Windu, Qui-Gon,
   ├── 1984 sanctum-firewalla                         │   Cilghal, Mundi, Mothma
   ├── 1337 sanctum-mlx                               │
   ├── 1234 LM Studio                                 └── 10.10.10.1:* (outbound
   ├── 8008 sanctum-xtts                                   only, via the Mac)
...
|-------|------|------------|
| **Yoda** | [Consigliere](/agents/yoda/) — absorbs everyone else's escalations, advises the Don | Local ops (Coder-14B via LM Studio) |
| **Windu** | Security master — Firewalla rules, PF policy, perimeter audits | Cloud (Opus 4.6) |
...
The practical rule is this: **you cannot tell a service's maturity from its function, only from its velocity.** Force Flow is mission-critical and still Python. `sanctum-xtts` is cosmetic voice synthesis and already Rust. What graduates a service is not how important it is — it is how *settled* it is. Hardened and flexible at the same time is the only configuration that survives contact with a real family living in a real haus in Québec for a real decade.
...
<CardGrid>
  ...
  <Card title="Language Maturity" icon="seti:rust">
