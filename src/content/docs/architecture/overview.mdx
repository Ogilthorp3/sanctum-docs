...
| `haus-control` | — | Curfew, wind-down, homework mode, credits, guests. Deployed alongside force-flow. The rules here change whenever a teenager finds a loophole. |
| `yoda-tts-worker` | 8011 | The Python voice backend — Chatterbox `cbx-yoda` — answering on `:8011` under the `com.sanctum.yoda-tts-worker` unit, behind the live Rust `sanctum-tts` front door on `:8008`. |

Flesh hardens. The current `rust_readiness` readout puts `force-flow` alone in the `hardening` stage — feature velocity slowing, but not zero. `haus-control` is still firmly `organic`, the busiest service on the chart with 42 commits in thirty days. It crosses when the guest-approval flow stabilises, and not a day before.
...
```
 ...
 ├─ 3301   sanctum-mlx-devstral (coder)
 ├─ 4040   proxyd
 ├─ 4077   force-flow
 ├─ 8008   sanctum-tts (→ cbx-yoda :8011)
 ├─ 8055   sanctum-server router (BOSS)
...
| **Ahsoka** | Satellite — the agent that will follow you when you leave the haus (reserved — chalet node, not yet deployed) |
