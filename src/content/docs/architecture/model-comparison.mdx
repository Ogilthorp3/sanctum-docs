...
<Aside type="caution">
**Deployment changed under the benchmarks (2026-06-07).** The Coder-14B serving tier was retired and the model is off disk (`com.sanctum.mlx-coder.plist.retired-20260607`). Council *and* code now run on one model, **Qwen3.6-35B-A3B-4bit**, served by `sanctum-mlx` on `:1337` (TLS) and reached by the council via the `:1339` bridge. Codestral-22B (`com.sanctum.mlx-codestral`, `:3301`) is the code fallback. The tables below are the historical bench that justified that consolidation — read them as a record, not a live roster.
</Aside>
