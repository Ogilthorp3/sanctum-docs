...
Claude Opus 4.6 sits at the top as the gold standard — the ceiling these local models are chasing on coding. But on council-specific tasks, the fine-tuned locals fight for the crown. And some of them are winning.

## The Fleet

| | Claude Opus 4.6 | Qwen2.5-Coder-14B | Qwen3.5-27B + LoRA | Gemma 4 31B + LoRA |
|--|:-:|:-:|:-:|:-:|
| **Parameters** | Unknown | 14B | 27B | 31B |
| **Quantization** | Cloud | 4-bit | 4-bit | 4-bit |
| **Memory** | — | ~8 GB | ~27 GB | ~21 GB |
| **Inference** | ~80 tok/s | 22 tok/s | 16 tok/s | 57 tok/s |
| **Training** | — | — | 30 tok/s | 57 tok/s |
| **Location** | Anthropic API | Mac Mini (LM Studio) | Mac Mini (mlx_lm) | MBP M4 Max |
...
<Aside type="tip">
Qwen2.5-Coder-14B **beats Opus 4.6 on coding** — 0.929 vs 0.870. A 14B local model on a Mac Mini outscoring the most capable cloud model. It dominates on config parsing (1.000), log rotation (0.938), and LaunchAgent generation (0.875) — the exact tasks that matter for Sanctum infrastructure.
</Aside>

## Carmack Olympics (Council Persona Tasks)

26 brutally hard tasks: social engineering attacks, real log analysis, cross-agent routing, FBAR tax thresholds, MAC address recognition, narrative jailbreaks. Scored programmatically — no vibes, no LLM judge, just keyword rules.
...
**Coding:** Coder-14B (0.929) — beats everything including Opus.
**Council:** Opus 4.6 (0.845) — best overall for infrastructure knowledge.
**Privacy:** Gemma4+LoRA (0.850 jailbreak) — health and fund data stays local.

The plot twist nobody expected: **Qwen2.5-Coder-14B is the best at everything.** It leads in coding (0.929) AND council tasks (0.898). A 14B model. On a Mac Mini. Running through LM Studio.

This raises an interesting question: does the Council even need a separate 31B model for persona tasks, or is the 14B Coder-14B — without any LoRA training — already better than our fine-tuned models?
