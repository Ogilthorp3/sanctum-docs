...
|---|---|---|
| Council (5 seats) | Four distinct model families — Claude Opus, Codestral, Gemini Pro, sanctum-mlx (Qwen3.6) — agreeing only by triangulation | Same model arguing with itself produces consensus, not signal |
...
The Council bug under [mlx-lm #1185](https://github.com/ml-explore/mlx-lm/issues/1185) is the canonical example. The Jedi voted unanimously on a fix that turned out to be wrong, because every seat was the same Qwen3.6-35B-A3B backbone in a different costume. The fix shipped, the bug got worse, and the second-opinion layer that was supposed to catch it had been a mirror the whole time. That incident is told in full on [(Neuro)diversity is Paramount](/architecture/neurodiversity-doctrine/); the lesson it left is this page.
