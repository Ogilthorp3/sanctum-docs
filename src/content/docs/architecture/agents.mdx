...
Six specialized agents, each routed through `sanctum-server` (Rust proxy on the Mac Mini) and dispatched across a 3-tier model architecture: Opus 4.7 (cloud, via proxy on :4040) for strategic synthesis, Coder-14B (local, LM Studio on :1234) for infrastructure and operations, and Gemma4+LoRA (local, sanctum-mlx on :1337) for security and domain-specific tasks. The Smart Router assigns each agent to its tier based on Carmack Olympics benchmark results. Council sessions are held via structured prompt exchange; the agents do not run as persistent daemons — they are invoked by the infrastructure on demand.
