...
- **sanctum-server** (port 8900): Rust smart router fronting four backends — `council-secure` (mTLS to `sanctum-mlx` on `:1337`), `coder` (plain-HTTP socat bridge `:1234` to `sanctum-mlx-coder` on mTLS `:1338`), `spatial` (Gemini 3.1 Pro), and `cloud` (Claude Opus 4.7 via Max proxy on `:3456`, OpenRouter fallback). Five tiers of routing, observability, breakers, and quality detectors. See [Smart Router Cathedral](/architecture/smart-router-cathedral/) for the full anatomy.
