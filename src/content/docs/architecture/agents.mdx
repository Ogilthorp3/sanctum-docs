...
<table>
  <thead>
    <tr><th>Agent</th><th>Logical model</th><th>Provider</th><th>Resolved model</th></tr>
  </thead>
  <tbody>
    {roster.agents.map((a) => (
      <tr key={a.id}>
...
<div style="font-size: 0.85em; opacity: 0.7; margin-top: -0.5em">
  Generated from openclaw.json + sanctum-proxy/config.yaml at <code>{roster.generated_at}</code>. Every Jedi falls back to the local Qwen tier if their primary path fails.
</div>
