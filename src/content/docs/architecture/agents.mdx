...
<CardGrid>
  <Card title="Yoda" icon="star">
    **Grand Master**
    The senior lead of the Sanctum Council. Yoda provides high-level orchestration, wisdom, and guidance for the entire agent roster. He handles council routing, final synthesis of complex decisions, and the daily "Morning Briefing" digest. Also runs as a dedicated voice agent (Python, port 1138 on Mac Mini) with Sanctum TTS (Qwen3 backend via mlx-audio, port 8008). He remains the ultimate authority on whether a raccoon constitutes a security breach.
  </Card>
  ...
  <Card title="Qui-Gon" icon="setting">
    **Infrastructure**
    The guardian of the machine. Qui-Gon is responsible for system health, container stability (OrbStack), and automated recovery. His triage logic now runs as a native Rust binary (`sanctum-triage`) on a 30-second interval. He is why you don't have to SSH into the machine every time it rains.
  </Card>
