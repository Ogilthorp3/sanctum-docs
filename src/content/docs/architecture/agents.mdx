...
Specialized agents, each routed to the model family best suited to their domain. As of 2026-04-28 the Council is **heterogeneous by default** — Yoda and Mundi on Claude Opus 4.7 (Max subscription bridge `<MINI>:3456`), Qui-Gon on Qwen2.5-Coder-14B (LM Studio `<MINI>:1234`), Windu on Gemini 3.1 Pro (Google AI Studio Ultra), Cilghal / Mothma / Jocasta on local Qwen3.6-35B-A3B (sanctum-mlx, mTLS-only on `<MINI>:1337`). Every Jedi falls back to the local Qwen3.6 if their primary path fails.

A Council of one model wearing five robes is not a Council — see [(Neuro)diversity is Paramount](./neurodiversity-doctrine.mdx). Routing config lives in `~/.openclaw/openclaw.json` (per-agent primaries) and `~/.sanctum/sanctum-proxy/config.yaml` on the Mini (tier resolution via `proxyd` on `<MINI>:4040`).
