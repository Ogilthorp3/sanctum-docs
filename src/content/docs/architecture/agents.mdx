...
- mobile nodes without `~/.sanctum/instance.yaml` fall back to local-only sync, while the Mac Mini remains the cross-machine fanout source
- `sanctumctl doctor --quick` now includes the markdown drift check so the mismatch shows up in the same calibration surface as runtime manifests and LaunchAgents
...
This keeps the VM runtime, the Mac Mini runtime, the MBP runtime, and the overnight fine-tuning source pointed at the same markdown truth instead of slowly inventing four different versions of the same Jedi.
