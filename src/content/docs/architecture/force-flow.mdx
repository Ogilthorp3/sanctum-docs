...
Daily-driver endpoints live under `/screen/*` — `/screen/status`, `/screen/override`, `/screen/homework`, `/screen/credit`. The family PWA lives at `/pwa/`. Full engine, tier model, schedule logic, and resilience layers are documented in [Screen Time](/architecture/screen-time/).
