...
The idle-manager pattern (an async TCP proxy that spawns the inference process on demand, kills it after N seconds of socket silence) was the original Rust-rewrite design. Production has since superseded it with the **cathedral fork's KeepAlive model**: `sanctum-mlx` (Council, `:1337`) and `sanctum-mlx-codestral` (the Codestral-22B coder seat, `:3301`) stay resident behind mTLS endpoints, Metal-cache caps bound their resident set, and the pressure valve handles real OOM pressure with SIGSTOP-then-resume — never SIGKILL on the inference servers, because SIGKILL would corrupt the KV cache. (The original coder seat `sanctum-mlx-coder` on `:1338` was retired 2026-06-07.)
