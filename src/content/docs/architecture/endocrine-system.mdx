...
**Date:** 2026-06-14
**Status:** Doctrine (built, off-by-default, awaiting turn-on)
...
| **Receptors** | `sanctum_cli/endocrine/receptor.py` — translates a panel into a seat's effective knobs; opted in per seat via `SANCTUM_ENDOCRINE=1` |
...
## Safety: additive, off-by-default, subscription-first

<Aside type="caution" title="Nothing is live until you turn it on">
The receptor is wired into the **real** council payload path, but gated behind `SANCTUM_ENDOCRINE`. With no subscription — the default — a seat's request body is **byte-identical** to today: no temperature, no top_p, persona untouched. The gland daemon's launchd plists ship **staged, not loaded**. Turning the organ on is a deliberate, reversible act.
</Aside>
...
- **Off-by-default.** Absent gland or neutral panel → the receptor is a no-op. There is no code path where a failed read makes a seat hotter.
...
- **Real, proven end-to-end:** the temperature + divergent-framing modulation through the real `build_completion_payload`. A settled creative panel raises a subscribed seat's temperature (0.7 → ~0.93) and appends the divergent clause; unsubscribed is byte-identical.
...
Off-by-default means there is a switch, and it is yours to throw:

1. Deploy the staged copies (sentinel → `~/.sanctum/sentinels/`, catalog entry → `~/.sanctum/services/`, both plists → `~/Library/LaunchAgents/`); `sanctum endocrine tick --dry-run` to confirm the gland reads real signals; `launchctl bootstrap` the gland + sentinel.
2. Opt one seat in with `SANCTUM_ENDOCRINE=1` and tune the thresholds on live signals before widening. Subscribe one, then widen — never mutate live seats wholesale.
3. Make subscription-first a *live* invariant by wiring the diversity selector into the fan-out, guarded by the same env gate.
