...
A nerve fires at one synapse. An antibody finds one antigen. A page goes out at 3 AM to exactly one phone. Everything in Sanctum's body is fast, and all of it is aimed — which is exactly what you want, right up until you notice nobody is in charge of the one thing no synapse can set: the haus's **mood**. Whether the council is in a state to execute carefully, or to dream.

**Filed 2026-06-14. Doctrine — live and on by default on manoir since 2026-06-15.**

Sanctum has, for a while now, been built as a body. Not a cute label slapped on afterward — a working division of labor, one organ-system per concern, each with its own metaphor and its own page. By mid-2026 the body had six systems, and exactly one was missing. This page adds the seventh.
