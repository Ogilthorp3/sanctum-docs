...
**Yoda** is the persona; the openclaw agent id is `main`. The runtime workspace is `~/.openclaw/workspace`; the persistent identity sandbox is `~/.openclaw/sandboxes/agent-main-<uuid>`. Bert is the **operator** at `+1-650-265-8350`; Yoda's account is `+1-514-572-2378`. The Mac Mini has codename `manoir` (`haus.local`); the VM is `yoda` at `10.10.10.10`.
</Aside>
