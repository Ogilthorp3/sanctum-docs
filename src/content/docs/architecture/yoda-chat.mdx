...
| 3 | `socat` bridge | Mac → VM | `10.0.0.1:7583` | NAT-friendly TCP forward to VM |
...
The Holocron Bridge — OK
  ...
  ✓  signal-cli TCP 10.0.0.1:7583            27ms  v0.14.4.1
  ✓  signal-proxy :5150                       12ms  healthy
  ✓  mlx bridge http://10.0.0.1:1339         24ms  1 model(s)
...
4. **If `signal-proxy` is red but `signal-cli TCP` is green:** the VM proxy is down. `ssh ubuntu@10.0.0.10 systemctl --user restart yoda-chat-proxy`. The doctor timer will catch this within 10 min anyway.
