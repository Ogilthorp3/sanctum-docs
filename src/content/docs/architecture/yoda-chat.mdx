...
The best interface in Sanctum is the one Bert uses without thinking. He opens Signal, texts a thread named "Yoda," and gets a reply in the Jedi Master's cadence — same UX as texting a friend. Behind that thread sits a 35B-parameter Qwen running locally on the Mac Mini via [sanctum-mlx](/architecture/sanctum-mlx/), an openclaw agent with the persona, and seven hops between the two.
...
| 4 | `yoda-chat-proxy` | VM | `:5150` | HTTP-JSON-RPC ⇄ TCP-JSON-RPC **and SSE** for openclaw |
| 5 | openclaw-gateway Signal channel | VM | `:1977` | Subscribes to proxy SSE `/api/v1/events`, runs agent |
| 6 | `sanctum-mlx` / proxyd | Mac | TLS `:4040` → cathedral | Local (or tiered) inference for the reply |

<Aside type="caution" title="Inbound is SSE, not the CLI subprocess">
Live Bert→Yoda traffic is **openclaw-gateway** on the sanctum-vm, with
`channels.signal.httpUrl = http://127.0.0.1:5150`. The gateway holds a long
SSE connection to the proxy and parses `event: receive` frames. Operator
tooling (`yoda-chat e2e`, honor probes) may still shell `openclaw agent
--deliver`. Both paths must stay green — outbound CLI success does **not**
prove inbound SSE is healthy. See the 2026-07-16 silent-inbound post-mortem.
</Aside>

<Aside type="danger" title="SSE data shape is load-bearing">
OpenClaw does `payload = JSON.parse(event.data); envelope = payload?.envelope`.
The proxy **must** emit `data: {"envelope":…,"account":…}` (signal-cli params),
**not** `data: {"method":"receive","params":{…}}`. Nested params → silent drop
with green health. Goldens + R2D2 `SSE-CONTRACT-BROKEN` guard this.
</Aside>
...
<CardGrid>
  ...
  <Card title="Self-heal on two sides of the airgap" icon="shield">
    **VM:** `yoda-chat-doctor.timer` every 10 min (workspaces, poisoned sessions, `yoda-chat-proxy` restart). **Mac:** launchd KeepAlive for signal-cli/bridge, plus `com.sanctum.signal-link-sentinel` readiness every ~5 min and an [R2D2](/architecture/r2d2/) recipe `heal-yoda-signal-link` every 10 min — including channel-disabled, SSE-contract-broken, and gateway-down (the 2026-07-16 class). Tracked goldens under `~/.sanctum/yoda-chat/`.
  </Card>
</CardGrid>
...
- Proxy down? `systemctl --user restart yoda-chat-proxy` (and bounce openclaw-gateway so it re-subscribes to SSE).
...
7. **Outbound green ≠ inbound healthy (2026-07-16).**

   Agent `--deliver` and proxy `send` can succeed while Bert→Yoda is mute. Causes that stayed green under naive health: `channels.signal.enabled=false` on the **VM** gateway config; yoda-chat SSE wrapping envelopes under `method/params` so OpenClaw never saw `payload.envelope`. Always check inbound readiness:
   `python3 ~/.sanctum/sentinels/signal-link-sentinel.py --check` and
...
6. **If everything is green and you still feel something is off:** run `yoda-chat e2e`. It will send two test messages to the operator's Signal — proof of **outbound** life. Then run the **inbound** probe: `ssh yoda 'python3 ~/.openclaw/yoda-chat/scripts/inbound-readiness.py'` — all five fields must be `true`. If `sse_contract_ok` is false, re-apply goldens: `~/.sanctum/yoda-chat/apply-inbound-goldens.sh`. Full story: [2026-07-16 silent inbound](/operations/2026-07-16-yoda-signal-silent-inbound/).
