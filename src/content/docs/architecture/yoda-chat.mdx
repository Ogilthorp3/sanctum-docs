...
**Yoda** is the persona; the openclaw agent id is `main`. The runtime workspace is `~/.openclaw/workspace`; the persistent identity sandbox is `~/.openclaw/sandboxes/agent-main-<uuid>`. Bert is the **operator** at `+15555550100`; Yoda's account is `+15555550101`. The Mac Mini has codename `manoir` (`haus.local`); the VM is `yoda` at `10.10.10.10`.
</Aside>
