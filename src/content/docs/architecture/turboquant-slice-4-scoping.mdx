...
<Aside type="tip" title="4a landed — 2026-04-19">
Slice 4a shipped across two commits (`21f230f` + `b469c52` on `feat/turboquant-kv-slice-1`). Measured 9.4× speedup for Lloyd-Max d=256 at T=2048 — from 8.0 ms CPU to 0.85 ms GPU per fetch. See `services/sanctum-mlx/bench/ANALYSIS.md` §"Slice 4a-v2" for the full table. This scoping page is preserved below as the planning-time record.
</Aside>
