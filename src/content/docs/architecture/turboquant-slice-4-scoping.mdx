...
The flag works. Slices 1 through 1f dragged TurboQuant from "doesn't compile" to "ships 21× better quality at 4× compression," and every one of those numbers is earned. What the flag cannot yet be is a **default** — and the thing standing in the way is embarrassingly boring. Dequant runs on the CPU. Fine for correctness validation, unshippable for production. This page is where we scoped the kill: Slice 4.
...
Three ways to move dequant onto the GPU, in ascending order of effort and nerve — the hero image up top renders them as a hand-file, a lathe, and a particle-beam etcher, three machinists working the same crystalline part. Qui-Gon, who never met an idle cycle he could forgive, would insist we climb the ladder one rung at a time and measure between each. We meant to.
...
Qui-Gon's ladder was the right ladder. The world just refused to climb it: 4a-as-ported missed the bar, 4b never happened, and the winner turned out to be a 4c-shaped kernel built on a premise the plan never entertained — stop compressing the keys at all. So why keep a planning doc the shipping code now contradicts on nearly every line? Because a plan that survives contact with reality teaches you nothing, and this one did not survive. The escalation still held even when we skipped straight to the particle beam — [The Carmack Optimization](/architecture/carmack-optimization/) is the same instinct pointed at training, and [Sanctum-MLX](/architecture/sanctum-mlx/) is the Rust server all of this fuses into. The [shipped architecture](/architecture/turboquant-kv-compression/) is the destination; this page is the map we drew before we knew the roads had moved.
