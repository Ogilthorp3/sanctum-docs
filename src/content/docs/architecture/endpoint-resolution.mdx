...
- **mTLS `--tls-host` in plists** no longer pins a literal Tailscale IP. The `com.sanctum.mlx` plist invokes `~/.sanctum/bin/sanctum-mlx-launch`, which resolves an `@SANCTUM_MANOIR_TS@` token to the live Tailscale IP at launch; the server cert SAN now carries the stable `manoir.tail7c6d11.ts.net` MagicDNS name, so remote callers verify by name regardless of IP drift. Proven against a real Gold Pro → Orbi Wi-Fi failover: the device-stable Tailscale IP held, and every bind path (loopback, vmnet bridge, Tailscale IP) plus the MagicDNS-name SAN validated over mTLS. Both cert-gen scripts (`mtls-gen-certs.sh`, `mtls-rebuild.sh`) emit the resolved MagicDNS name as `DNS.5`, so a regeneration can't silently drop it.
