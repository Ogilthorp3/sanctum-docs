...
<CardGrid>
  ...
  <Card title="Rule 3: Nothing critical gets suspended" icon="shield">
    the Lima/vz VM, sanctumd, sanctum-mlx, signal-cli, WindowServer are pinned via `evictable: false` in `capacity.yaml`. Castellan programs the kernel's own arbiter: pinned services get jetsam band `critical` (190) and stay there. The active loop will never `SIGSTOP` them; the kernel itself spares them when memory pressure crests.
  </Card>
</CardGrid>
...
```yaml
sanctum-mlx:
  ram_budget_mb: 45000         # Qwen3.6-35B-A3B-4bit, :1337
  exclusive_with: [sanctum-mlx-codestral]
  priority: 100

sanctum-mlx-codestral:
  ram_budget_mb: 14000         # Codestral-22B-v0.1-4bit, :3301
