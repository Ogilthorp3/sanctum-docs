...
The `sanctum-server` smart router on `:8055` (BOSS, [Smart Router Cathedral](/architecture/smart-router-cathedral/)) fronts both binaries with four backends (`council-secure`, `coder`, `spatial`, `cloud`) and five tiers of defense: breaker, budget, latency EMA, quality detectors. Retired plumbing — the old `:1338` coder seat, the socat `:1234` bridges, all of LM Studio — is in the file map below.
