...
After P5: the binary is hardened + auth-gated + bind-restricted + manifest-signed + doubly-watched. The remaining gaps from B+ → A:

| Gap | What it would take |
|-----|--------------------|
| Developer-ID signing + notarization | $99/year Apple cert + build-time notarize step |
...
Grade with P5 in: A−. The honest "military-grade" label starts on the other side of that Developer ID cert and the mTLS rollout.
