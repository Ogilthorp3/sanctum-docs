...
<Aside type="note" title="Current as of 2026-06-07">
...
A 9B Python fallback also stands by on `:8901`. Both MLX binaries run under LaunchAgents (`com.sanctum.mlx`, `com.sanctum.mlx-codestral`) with KeepAlive and 30 s ThrottleInterval, load weights from `~/.cache/huggingface/hub/`, gate the listen socket on a signed manifest (`ed25519` over `shasum -a 256`), and bind only to loopback + Tailscale + bridge100 — never `0.0.0.0`. The earlier coder seat (`sanctum-mlx-coder` / `com.sanctum.mlx-coder` on `:1338`, plain `:1234` via socat, running Qwen2.5-Coder-14B-Instruct-4bit) was retired 2026-06-07 — its GGUF was archived and weights deleted when the role moved to Codestral. LM Studio was fully decommissioned 2026-06-07; there is no LM Studio server and no `:1234` — native MLX seats serve every model now.
...
The cathedral runs PLD by default on both binaries (`SANCTUM_MLX_PLD_MAX_LOOKUP=5`, `NGRAM_SIZE=3`). It self-speculates using the prompt context as a draft model — no second model required, no `flash-attn` wheel-build dance. Branch `feat/cathedral-pld` started this; the path is now mainline.
...
| Mistral / Mixtral | **Not yet supported** | The loader will refuse the checkpoint. Add a `MistralModel` arm to the loader dispatch when needed. |

### Architecture

```
[Client] → mTLS handshake → 127.0.0.1:1337 (or :1338)
    ↓
[sanctum-mlx (Rust/axum)]
  ├─ Manifest gate: ed25519 sig + SHA-256 verify before bind
  ├─ Loader dispatch by model_type → Qwen3.5MoE / Qwen2 / Qwen3.5
...
| `services/sanctum-mlx/src/loaded_model.rs` | Loader dispatch by `model_type` (qwen3_5_moe / qwen2 / qwen3_5); PLD orchestration; LCP cache reuse |
...
| `~/Library/LaunchAgents/com.sanctum.mlx.plist` | Council `:1337` runtime config (`SANCTUM_MLX_FUSED_QKV=1`, `FUSED_MLP=1`, PLD env vars, prewarm file, signed manifest paths) |
| `~/Library/LaunchAgents/com.sanctum.mlx-codestral.plist` | Coder `:3301` runtime config (Codestral-22B-v0.1-4bit) |
