...
The two seats differ in exactly the ways their threat models differ:
...
The whole point is boring: 27 billion 4-bit parameters answering on a 64 GB Mac Mini, zero Python in the request path, and enough hardening around it that nobody has to think about it. Tommy would approve — the most reliable service in the haus is the one that just shows up, on time, and never asks for a restart.
