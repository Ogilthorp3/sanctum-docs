...
<Aside type="note" title="Current as of 2026-05-24">
...
| `sanctum-mlx-coder` | mTLS `:1338` (+ plain `:1234` via socat) | Qwen2.5-Coder-14B-Instruct-4bit (model_type `qwen2`) | Coder seat + Qui-Gon's code path |

Both run under user LaunchAgents (`com.sanctum.mlx`, `com.sanctum.mlx-coder`) with KeepAlive and 30 s ThrottleInterval. Both load weights from `~/.cache/huggingface/hub/`, gate the listen socket on a signed manifest (`ed25519` over `shasum -a 256`), and bind only to loopback + Tailscale + bridge100 — never `0.0.0.0`. LM Studio retirement (2026-05-11) removed the last Python dependency from the inference path.
...
| `qwen2` | Production | Qwen2.5-Coder-14B-Instruct-4bit on `:1338`. With YaRN factor=4 RoPE scaling for long context. |
...
| `~/Library/LaunchAgents/com.sanctum.mlx-coder.plist` | Coder `:1338` runtime config (`YARN` factor=4, fused QKV/MLP, same PLD knobs) |
| `~/Library/LaunchAgents/com.sanctum.coder-plain-bridge.plist` | `socat` plain-HTTP `:1234` → mTLS `:1338` for legacy callers |
