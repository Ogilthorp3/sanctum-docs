...
A 9B Python fallback also stands by on `:8901`. Both MLX binaries run under LaunchAgents (`com.sanctum.mlx`, `com.sanctum.mlx-codestral`) with KeepAlive and load weights from disk, and neither ever binds `0.0.0.0`. The two binaries differ in exactly the ways their threat models differ:
