...
### 4. mTLS (deferred — honestly)

Implemented nothing for mTLS this session. The rollout requires generating a self-signed CA, issuing per-client certs for guardian / canary / drift / sanctum-server / VM / off-box watchers, and updating every client to present its cert. ~2 hours of careful work best done dedicated rather than at the tail of an omnibus push.

What we have today: bearer-token auth over Tailscale-encrypted wire. Bearer is a shared secret — if it leaks, everyone who got the file can call the council. mTLS would replace the shared secret with per-client certs that can be individually revoked. Real win, real complexity. On the queue as P8.

### Grade post-P7: A

Observable (Prometheus). Survivable (HA failover wired, config-deployable). Verifiable (nightly correctness cron). The remaining A+ gap is mTLS + the 3/10 bf16 ULP parity gap — neither is critical path; both are honest horizon items.
