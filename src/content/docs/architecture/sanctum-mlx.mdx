...
A disaster fallback (`com.sanctum.mlx-py-fallback`) sits promote-only on `:8901`, serving the same Qwen3.6-35B-A3B-4bit as `:1337` but via an independent Python mlx-lm stack — so a corrupt or missing Rust binary (which kills both the council and devstral seats, since they share one binary file) leaves a different-stack server able to take over at full parity. It is not always-warm: an always-resident 35B would sit ~20 GB on top of the live Rust 35B and push the 64 GB Mini deep into swap, so `council-integrity-check` promotes it only on unrecoverable Rust-binary failure — booting the broken Rust agent first to free the RAM, then enabling and kickstarting the Python seat. Transient (non-binary) failover is handled separately at the proxyd routing layer (`council-mlx` to `council-code` to `claude-cli-offline`). Both Rust MLX seats run under launchd (`com.sanctum.mlx`, `com.sanctum.mlx-devstral-rust`) with KeepAlive and load weights from disk, and neither ever binds `0.0.0.0`. The two seats differ in exactly the ways their threat models differ:

- **Council `:1337`** gates the listen socket on an **ed25519-signed** manifest (`ed25519` over `shasum -a 256`) and binds three interfaces — loopback, the Mini's Tailscale IP, and bridge100 (`10.10.10.1`) for the Lima VM path. It is mTLS-only: the plist passes `--no-plain`, so there is no plain-HTTP or bearer path left to bypass.
- **Devstral `:3301`** gates on a **plain SHA-256** manifest (no `.sig` is shipped for it yet) and binds **loopback only** — `127.0.0.1:3301`, nothing on the Tailnet, nothing on the bridge. The coder seat is local-only by design; the VM and MBP reach it through the council router, not directly.

The earlier coder seat (`sanctum-mlx-coder` / `com.sanctum.mlx-coder` on `:1338`, plain `:1234` via socat, running Qwen2.5-Coder-14B-Instruct-4bit) was retired 2026-06-07 — its GGUF was archived and weights deleted when the role moved to Codestral. LM Studio itself is decommissioned; native MLX seats serve every model now. The last piece of LM Studio plumbing to stop twitching was `com.sanctum.lmstudio-bridge` — a `socat` shim that kept LISTENing on `10.0.0.1:1234`, forwarding to a `127.0.0.1:1234` that nothing answered anymore. That orphan has since been booted out too: the agent is unloaded and the `:1234` socket is closed.
...
The cathedral runs PLD by default on the council `:1337` binary, where the plist wires it explicitly (`SANCTUM_MLX_PLD_MAX_LOOKUP=5`, `SANCTUM_MLX_PLD_NGRAM_SIZE=3`). It self-speculates using the prompt context as a draft model — no second model required, no `flash-attn` wheel-build dance. Devstral's plist doesn't set the PLD env vars, so the coder seat runs on whatever the code default is rather than an explicit, `ps`-visible override. Branch `feat/cathedral-pld` started this; the path is now mainline.
...
| `mistral` | Production | Devstral-Small-2-24B-Instruct-2512-4bit (Mistral-arch dense, text-only) on `:3301`. The loader's `"mistral"` arm rebuilds the dense `ModelInput` from the shared fields — no linear-attention/SSM slots. |
...
| `~/Library/LaunchAgents/com.sanctum.lmstudio-bridge.plist` | RETIRED — was the orphan `socat` LISTEN `10.0.0.1:1234` → `127.0.0.1:1234` (dead backend) that outlived the LM Studio decommission; now unloaded, and `:1234` is closed |
