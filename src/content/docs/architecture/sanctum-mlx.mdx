---
title: Sanctum-MLX
description: A pure Rust inference server that talks directly to Apple Silicon Metal — zero Python, fused kernels, mTLS, signed weight manifests, and an irrational level of commitment to eliminating overhead.
---
...
So we did what any reasonable person would do: we rewrote the entire inference stack in Rust, implemented a GatedDeltaNet state-space model from scratch, wrote custom Metal GPU kernels, and shipped it as a pair of binaries that start in 3 seconds and decode at 36–48 tokens per second.

This is sanctum-mlx, **cathedral fork**, on Apple Silicon Metal. It has no Python dependency. It has no regrets.

<Aside type="note" title="Current as of 2026-06-09">
This page accumulated through eight months of incremental rewrites. The narrative below is mostly intact but the cathedral fork has changed several load-bearing facts since the v0.2.0 cutover. Read the **Cathedral fork — current state** section first; the rest of the page is the path that got us here, preserved because the bugs and the fixes are still instructive.
</Aside>

## Cathedral fork — current state

The active inference layer is the **cathedral fork** at `~/Projects/sanctum-rs-cathedral/services/sanctum-mlx/`. The original `sanctum-rs/services/sanctum-mlx/` tree is deprecated and kept only for archival reference. Production drives:

| Binary | Port | Model | Role |
|--------|------|-------|------|
| `sanctum-mlx` | mTLS `:1337` | Qwen3.6-35B-A3B-4bit (MoE, model_type `qwen3_5_moe`) | Council seats (yoda, mothma, windu, cilghal, mundi, jocasta, quigon) |
| `sanctum-mlx-codestral` | `:3301` | Codestral-22B-v0.1-4bit | Coder seat + Qui-Gon's code path |

A disaster fallback (`com.sanctum.mlx-py-fallback`) sits promote-only on `:8901`, serving the same Qwen3.6-35B-A3B-4bit as `:1337` but via an independent Python mlx-lm stack — so a corrupt or missing Rust binary (which kills both the council and codestral seats, since they share one binary file) leaves a different-stack server able to take over at full parity. It is not always-warm: an always-resident 35B would sit ~20 GB on top of the live Rust 35B and push the 64 GB Mini deep into swap, so `council-integrity-check` promotes it only on unrecoverable Rust-binary failure — booting the broken Rust agent first to free the RAM, then enabling and kickstarting the Python seat. Transient (non-binary) failover is handled separately at the proxyd routing layer (`council-mlx` to `council-code` to `claude-cli-offline`). Both Rust MLX binaries run under LaunchAgents (`com.sanctum.mlx`, `com.sanctum.mlx-codestral`) with KeepAlive and load weights from disk, and neither ever binds `0.0.0.0`. The two binaries differ in exactly the ways their threat models differ:

- **Council `:1337`** gates the listen socket on an **ed25519-signed** manifest (`ed25519` over `shasum -a 256`) and binds three interfaces — loopback, the Mini's Tailscale IP, and bridge100 (`10.0.0.1`) for the VM path. It is mTLS-only: the plist passes `--no-plain`, so there is no plain-HTTP or bearer path left to bypass.
- **Codestral `:3301`** gates on a **plain SHA-256** manifest (no `.sig` is shipped for it yet) and binds **loopback only** — `127.0.0.1:3301`, nothing on the Tailnet, nothing on the bridge. The coder seat is local-only by design; the VM and MBP reach it through the council router, not directly.

The earlier coder seat (`sanctum-mlx-coder` / `com.sanctum.mlx-coder` on `:1338`, plain `:1234` via socat, running Qwen2.5-Coder-14B-Instruct-4bit) was retired 2026-06-07 — its GGUF was archived and weights deleted when the role moved to Codestral. LM Studio itself is decommissioned; native MLX seats serve every model now. The one piece of LM Studio plumbing still twitching is `com.sanctum.lmstudio-bridge` — a `socat` shim that still LISTENs on `10.0.0.1:1234` and forwards to a `127.0.0.1:1234` that nothing answers anymore. It is a no-op orphan, not a live LM Studio server, but until the bridge is booted out the `:1234` socket is technically still open.

The `sanctum-server` smart router on `:8900` ([Smart Router Cathedral](/architecture/smart-router-cathedral/)) fronts both binaries with four backends (`council-secure`, `coder`, `spatial`, `cloud`) and routes requests with five tiers of defense — breaker, budget, latency EMA, quality detectors.

### Recent shipped work (Phase 1A/1B and the view-aliasing fix)
...
| **Phase 1B — FusedGateUpProj** | Merged 2026-05-14 (PR #16, commit `f92d44e`) | Coder essay throughput reached ~48 tok/s. Same `+ zero` discipline carried forward; `mlx_contiguous` / `mlx_copy` are NOT substitutes. |
| **Phase 2 — Fused RoPE Metal kernel** | Designed, not shipped (branch `feat/cathedral-phase2-fused-rope @ 6378408`) | MSL kernel is correct in isolation but MLX's lazy graph doesn't auto-track custom-kernel outputs; the eval barrier costs more than the two saved launches. Waiting on upstream MLX fix. |
...
### LCP cache reuse + T_q ≥ 2 invariant

Cross-request prompt-cache reuse via longest-common-prefix matching is on. The cathedral caps the LCP at `len - 2` so the suffix is always `T_q ≥ 2` — MLX's decode-kernel vs prefill-kernel SDPA diverge across requests when the suffix is exactly 1 token, producing byte-different responses to byte-identical prompts. The two-token margin makes the cache byte-identical (commit `0123773`).

### Prompt-Lookup Decoding (PLD) as a self-speculation art piece

The cathedral runs PLD by default on the council `:1337` binary, where the plist wires it explicitly (`SANCTUM_MLX_PLD_MAX_LOOKUP=5`, `SANCTUM_MLX_PLD_NGRAM_SIZE=3`). It self-speculates using the prompt context as a draft model — no second model required, no `flash-attn` wheel-build dance. Codestral's plist doesn't set the PLD env vars, so the coder seat runs on whatever the code default is rather than an explicit, `ps`-visible override. Branch `feat/cathedral-pld` started this; the path is now mainline.

### Architecture coverage

`sanctum-mlx` dispatches its loader on the checkpoint's `model_type`:
...
| `qwen3_5` (dense) | Reference | The original v0.2.0 27B-4bit path. Still in the loader; not currently the production champion. |
| `mistral` | Production | Codestral-22B-v0.1-4bit (Llama-arch dense, text-only) on `:3301`. The loader's `"mistral"` arm rebuilds the dense `ModelInput` from the shared fields — no linear-attention/SSM slots. |
| Mixtral (sparse MoE) | Not yet supported | The loader handles dense `mistral` but has no Mixtral expert-grid arm yet. Anything else `model_type` is refused outright with a message that names the four it does load. |
...
[sanctum-mlx (Rust/axum)]
  ...
  └─ /metrics (Prometheus), bearer auth on plain, cert auth on mTLS
```

The server exposes an OpenAI-compatible `/v1/chat/completions` endpoint. It supports LoRA adapter hot-swap (covers quantized + batched-quantized + 3-D `switch_mlp` layers — needed for the MoE expert grids). Models load directly from safetensors files into mlx-rs arrays. The full model graph runs on the Metal GPU without ever touching Python, NumPy, or the existential dread of `pip install`.
...
<Aside type="note" title="35B-era figures differ">
The caps above (`40960`/`24576`) and the 13 GB-wired / 9.2 GB-swap steady state are the **27B-era** numbers from this 2026-04-24 push. The live 35B champion runs tighter caps in its plist — `--metal-memory-limit-mb 28672 --metal-wired-limit-mb 18432` (cache limit still `1024`) — and a bigger model means more resident weight and a hotter swap: at the time of writing the Mini sits at ~28.7 GB swap, ~27.5 GB of it used. The bounding logic is the same win; the constants moved with the model.
</Aside>
...
`--host` is now repeatable. Default is a single `127.0.0.1`. Production plist adds the Mini's Tailscale IP (`100.0.0.25`). LAN devices at `192.0.2.x` cannot connect to `:1337` directly — the socket doesn't exist on that interface. Bridge100 (`10.0.0.1`) is owned by `sanctum-triage`, a proxy that normalizes VM traffic and forwards to loopback, so the VM path is intact.

The `--host`/plain-listener detail here is the P5 shape. The council binary has since moved to mTLS-only (`--tls-host` ×3, `--no-plain`); the bind *interfaces* — loopback, Tailscale, bridge100 — are unchanged, only the listener type is.
...
`GET /metrics` renders Prometheus text format and goes through the same auth middleware as the rest. At the time this shipped that meant loopback-bypass plus a bearer token for cross-machine scrapers. Since the mTLS-only consolidation (see the current-state section), `:1337` has no plain listener: `/metrics` is reachable only over the mTLS socket and a scraper has to present a client cert signed by the Sanctum CA. Operational detail doesn't leak onto the open Tailnet.
...
<Aside type="note" title="Superseded — the migration finished">
What follows is the dual-listener design as it shipped. The gradual rollout it describes is **done**: every client moved to mTLS, the plain `--host` lines came out of the plist, and the second TLS listener folded back onto `:1337`. Live state today is a single mTLS-only listener on `:1337` (`--tls-host` ×3, `--tls-port 1337`, `--no-plain`); there is no plain `:1337` and no `:1338`. Read the rest for the *why*; trust the current-state section for the *what*.
</Aside>

The binary shipped two routers:

- **Plain HTTP (`:1337`)** — bearer-auth enforced with loopback bypass. Everything we shipped in P5 still worked; guardian/canary/drift/sanctum-server/VM-through-triage on loopback kept functioning without config changes.
...
**Rollout was gradual, no flag day.** Plain listeners stayed up alongside TLS while clients migrated one at a time. Once every client had moved, the plain `--host` lines came out of the plist, only `--tls-host` remained, and the mTLS listener moved onto `:1337` — which is where it lives now. The bearer token rode along as a belt-and-suspenders fallback during the transition and has since been retired with the plain listener.
...
All paths below are in the **cathedral fork** at `~/Projects/sanctum-rs-cathedral/`. The original `~/Projects/sanctum-rs/services/sanctum-mlx/` tree is deprecated and kept for archival only — anything new lands in the cathedral.
...
| `~/Library/LaunchAgents/com.sanctum.mlx-codestral.plist` | Coder `:3301` runtime config (Codestral-22B-v0.1-4bit) — `--host 127.0.0.1` loopback-only, plain SHA-256 manifest (no `.sig`), no PLD env vars |
...
| `~/Library/LaunchAgents/com.sanctum.coder-plain-bridge.plist` | RETIRED 2026-06-07 — was the `socat` plain-HTTP `:1234` → mTLS `:1338` shim for legacy callers; gone with LM Studio decommission |
| `~/Library/LaunchAgents/com.sanctum.lmstudio-bridge.plist` | STILL LOADED (orphan) — `socat` LISTEN `10.0.0.1:1234` → `127.0.0.1:1234` (dead backend). A no-op since LM Studio was decommissioned; boot it out to actually close `:1234` |
