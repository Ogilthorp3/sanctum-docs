...
A wrapper that only exists as a one-off file in `~/.local/bin` is not deployed. Docs without a sidebar slug are not in Sanctum-docs. "It worked once on my laptop" is not E2E. EETISMAD is how we say "done" without lying. Local Flash launch on the MBP is the standing example — see [ds4max](/architecture/ds4max/).
