...
![A pencil-sketched pressure gauge mounted on a pipe, its needle well below the marked limit, with a small handwritten card of rates clipped alongside — swappable without touching the pipework. A cat watches the needle.](./images/hero-proxyd-budget.png)
...
CLI-facing Opus/Fable chains prefer **subscription bridges before metered glm-52**:

`claude-cli-offline (Max :3456) → gemini-3-pro → grok-oauth → glm-52 → council-local-think`

So a capped agentic night lands on free subs first, not the cathedral 27B. Cathedral is blackout tail only.
