...
The five seats route to four distinct model families. The current assignment is rendered live from `src/data/council-roster.json`, regenerated from the Mini's `openclaw.json` + `sanctum-proxy/config.yaml` whenever the lineup shifts:
