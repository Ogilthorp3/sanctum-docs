...
The five reasoning seats route to four distinct model families. The current assignment is rendered live from `src/data/council-roster.json`, regenerated from the Mini's `openclaw.json` + `sanctum-proxy/config.yaml` whenever the lineup shifts:

<CouncilRoster />

This page is the **doctrine**. [The Council page](./agents.mdx) is the **single source-of-truth** for the roster — the same component renders there and anywhere else in the docs that needs to say "who runs on what." When a champion changes (new Claude model, local-tier upgrade, training run promoted), run `pnpm refresh:council` once and every page reflects the new assignment on the next deploy. No prose to hunt down, no tables to keep in sync.

Fallback for every Jedi is the local Qwen3.6 backend (visible in the per-agent JSON, omitted from the table above for compactness). Cloud paths can fail; the local model is always there, on the loopback, behind the same mTLS we use for everything else.
...
**Distinct failure modes.** When the Anthropic seat, the Google seat, the French code model, and the local Qwen all reach the same answer, that's signal — four different architectures failed in four different directions and converged anyway. When they disagree, that's also signal — the disagreement is the data. The old Council, being one model in four masks, could only ever return the first signal, and it returned it falsely.
...
<Aside type="caution">
Run the smoke test about once a week. Cloud paths fail quietly — a rate limit, a billing flip, a preview model deprecated overnight — and a quiet failure on a second-opinion layer is the worst kind, because the wrong answers keep arriving on schedule. The script catches the drift before the morning briefing does, and the morning briefing reports it the only way it knows how: "Yoda sounds different today."
</Aside>
...
The outer reading is **diversity** in the AI sense: distinct architectures, distinct training data, distinct failure modes, distinct ways of being wrong. Different brains in the seats means a real disagreement is possible, which is why a Council exists at all.
...
- [Polarity](/architecture/polarity/) — the underlying principle. This page is its Council-specific application.
