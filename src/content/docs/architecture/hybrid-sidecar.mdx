...
**The Container Plane (Docker via OrbStack)** is the Logic Execution Engine. It houses the OpenClaw core, the `claw-gateway` daemon, and a lightweight MLX client that offloads heavy inference back to the host. It operates as a sidecar — it doesn't replace the host, it augments it. (Colima ran this plane through 2026-04-27; OrbStack replaced it for native networking and no `bridge100` contention.)
...
| Coder MLX (`sanctum-mlx-codestral`) | Host | 3301 (mTLS) | `127.0.0.1` + Tailscale | Codestral-22B-v0.1-4bit, the CODER role. Was a `socat` plain-HTTP bridge on `:1234` to `sanctum-mlx-coder` `:1338` (retired 2026-06-07); LM Studio pre-2026-05-11. |
| Sanctum Proxy (`proxyd`) | Host | 4040 | `127.0.0.1` | Rust daemon — traffic cop between external connections and container |
| MLX Runtime (`sanctum-mlx`) | Host | 1337 (mTLS) | `127.0.0.1` + Tailscale | High-performance inference path via Metal. See [Dynamic Model Routing](/architecture/dynamic-model-routing/) |
| ~~Coder MLX (`sanctum-mlx-coder`)~~ | Host | ~~1338~~ | — | Retired 2026-06-07. Was Qwen2.5-Coder-14B; CODER role now served by `sanctum-mlx-codestral` on `:3301` (see row above) |
| Docker Engine (OrbStack) | Host | — | — | Native macOS container runtime for the container plane |
...
services:
  openclaw-core:
    ...
    environment:
      ...
      - CODER_BRIDGE_HOST=host.docker.internal
      - CODER_BRIDGE_PORT=3301   # sanctum-mlx-codestral, mTLS (was socat→:1338, retired 2026-06-07)
      - MLX_HOST=host.docker.internal
      - MLX_PORT=1337            # sanctum-mlx, mTLS
