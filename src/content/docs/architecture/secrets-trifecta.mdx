...
Two LaunchAgents are written to keep this honest. Both plists live in `~/.sanctum/scripts/secret-rotator/` and load under an `Aqua` session — staged, but not currently bootstrapped on the host (`launchctl list | grep secrets-` comes back empty):
