...
4. **P0 secrets must be filesystem-readable, not Keychain-only.** Added 2026-05-08 as doctrine §1.1.H. The 2026-05-07 reboot drill caught a `firewalla-bridge-token` that lived only in the macOS login Keychain — fast and convenient until the Mini rebooted without auto-login, the Keychain stayed locked, and the bridge entered a fatal-restart loop. The trifecta's three tiers are not interchangeable: 1Password is unreachable from any service runtime; SOPS needs the VM, the sops binary, and the age key; Keychain is gated on a user session that may not exist. Filesystem (`~/.sanctum/secrets/<name>`, mode 600) is the only tier with no ambient prerequisites. Rule of thumb: if a service is P0, its secret either writes through to filesystem on rotation or it isn't allowed to be P0.
