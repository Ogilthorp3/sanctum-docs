...
| 1234 | Coder Plain Bridge (orphan) | Mac | Password1 | The port equivalent of leaving your key under the mat. Was LM Studio (retired 2026-05-11), then a `socat` bridge to `sanctum-mlx-coder` on `:1338` — both retired 2026-06-07 when the coder role moved to Codestral on `:3301`. The `com.sanctum.lmstudio-bridge` socat keeps respawning anyway, forwarding the Mac↔VM bridge IP on `:1234` to a `127.0.0.1:1234` backend that no longer exists. An orphan listener with the lights on and nobody home; the live plain bridge to the council seat is `:1339`, not this. Pending bootout. |
...
| 1338 | Coder MLX (retired) | Mac | LEET+1 | The sequel port to LEET. Hosted `sanctum-mlx-coder` (Qwen2.5-Coder-14B-Instruct-4bit) until 2026-06-07, when the seat was retired — GGUF archived, weights reclaimed. The coder role is now Codestral 22B on `:3301`. |
...
| 3301 | Codestral Seat | Mac | Cicada | Cicada 3301 — the internet's most elaborate unsolved recruitment puzzle. The French-built Codestral 22B rides here, the council's on-call code seat, and since the Qwen coder on `:1338` retired 2026-06-07, the haus's sole local coder. Unlike the council seat on `:1337`, this one still answers plaintext on `127.0.0.1` only — mTLS is the plan, not yet the posture. If anything in the haus cracks the next puzzle, bet on the model that dreams in cryptography. |
