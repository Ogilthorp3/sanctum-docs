...
| 3456 | Claude Max Relay | Mac | Sequel to Password1 | 1234 is Password1 (above). 3456 is what you pick when someone says 1234 is too weak and you "fix" it by sliding two keys right. Binds loopback only — the council proxy (proxyd) reaches it on localhost while the other host tunnels in over persistent SSH (local `:3457` forwarded to the Mini's `:3456`), two trusted callers sharing one socket. Relays Claude Max / Opus to the council. |
