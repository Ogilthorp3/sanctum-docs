...
| 3301 | Devstral Seat | Mac | Cicada | Cicada 3301 — the internet's most elaborate unsolved recruitment puzzle. The French-built Devstral Small 2 (24B) rides here, the council's on-call code seat, and since the Qwen coder on `:1338` retired 2026-06-07, the haus's sole local coder. Unlike the council seat on `:1337`, this one still answers plaintext on `127.0.0.1` only — mTLS is the plan, not yet the posture. If anything in the haus cracks the next puzzle, bet on the model that dreams in cryptography. |
