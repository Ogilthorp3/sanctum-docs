...
| 1234 | Coder Plain Bridge (retired) | Mac | Password1 | The port equivalent of leaving your key under the mat. Was LM Studio (retired 2026-05-11), then a `socat` bridge to `sanctum-mlx-coder` on `:1338`. Both retired 2026-06-07 — the coder role moved to Codestral on `:3301` and this socket is gone. |
...
| 2189 | Sanctum Castellan | Mac | Gollum | One cell over from 2187. The Capacity Doctrine controller decides who gets through the gate — admission control for heavy services before they ever touch the cellblock. The Rust root daemon `sanctum-castellan` (codename Gollum) holds the keys now; it superseded the older `sanctum-admit` on this socket 2026-05-30. My precious memory pool, it guards. |
...
| 3301 | Codestral Seat | Mac | Cicada | Cicada 3301 — the internet's most elaborate unsolved recruitment puzzle. The French-built Codestral 22B rides here (mTLS-only), the council's on-call code seat — and since the Qwen coder on `:1338` retired 2026-06-07, the haus's sole local coder. If anything in the haus cracks the next puzzle, bet on the model that dreams in cryptography. |
