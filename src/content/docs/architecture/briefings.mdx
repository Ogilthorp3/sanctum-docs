...
**Intel** briefings — Jocasta (calendar) and Mundi (dealflow) — exist to put information in front of you. Their content _is_ the thing you want on Signal, so they page when there is something to act on and stay silent otherwise. Jocasta runs on a forward-looking schedule; Mundi is purely event-driven (below).
...
| `com.sanctum.mundi-briefing` | `mundi-watch.py` (sanctum-crm) | every 30 min, 07:00-22:00 | intel, event-driven | Fires only on a deal-related email or a due Affinity reminder; inline action brief + :2600 dossier |
