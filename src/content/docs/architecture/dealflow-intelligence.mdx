...
|---|---|---|
| Sync | `affinity-sync.py` | Pulls the deal lists from the Affinity **v2** REST API into `workspace.duckdb` (the `v_deal` view). Daily, `com.sanctum.affinity-sync`. On success, re-seeds the [work graph](/architecture/crm-architecture/) so partner sessions stay current. |
