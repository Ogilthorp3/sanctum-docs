...
`sanctum-tts` is the Rust dispatcher on `:8007`. Callers ask for a voice by name; the daemon hands the request to whichever adapter the registry declares. Python never enters the dispatcher's process — adapters are thin HTTP proxies, kept in their own venvs the way you'd keep two cats in separate rooms.
...
- Build the release binary and load `com.sanctum.tts` so the front door on `:8007` actually opens.
