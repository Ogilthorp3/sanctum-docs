...
<Aside type="note" title="Status: built, not yet stood up">
The dispatcher is written, compiles, and passes its tests — but no release binary is deployed and nothing is listening on `:8007` yet. The launchd label `com.sanctum.tts` is reserved, not loaded. **Today the live voice path is the Python worker on `:8008` directly** (`com.sanctum.yoda-tts-worker`, running the Qwen3-TTS model). The front door is hung; we just haven't moved everyone through it yet.
</Aside>
