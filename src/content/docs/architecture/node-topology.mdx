...
1. macOS boots → auto-login (FileVault disabled, `autoLoginUser: bert`)
2. `com.sanctum.utm-autostart` → launches UTM, starts VM, creates bridge100, sets 10.10.10.1
3. `com.sanctum.openclaw.colima` → starts Colima (user-mode networking, no bridge)
4. `apple-post-boot.sh` → loads SSH keys from Keychain, runs health checks
5. Remaining LaunchAgents start services in dependency order
