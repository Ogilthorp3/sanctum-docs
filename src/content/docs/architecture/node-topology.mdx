...
<Aside type="tip">
  Satellites operate independently when the Tailscale connection to the hub is unavailable. Local services (Home Assistant, gateway, local model) continue to function offline.
</Aside>
...
| Dashboard | `http://100.0.0.25:3001` |
| Home Assistant | `https://ha.nepveu.name` (via Cloudflare) |
| Agent query | Via gateway API at `100.0.0.25:18789` |

<Aside>
  The MacBook Pro restricts SSH access to the Thunderbolt interface only. It is reachable via Tailscale IP but does not expose SSH on the general LAN.
</Aside>
