...
2. **Phase 2: Inference + routing** — `sanctum-mlx` (`:1337`, mTLS, Qwen3.6-35B-A3B-4bit) and `sanctum-mlx-devstral` (`:3301`, Devstral-Small-2-24B-Instruct-2512-4bit, the coder seat since the `:1338` Qwen coder + `:1234` LM Studio bridge were retired 2026-06-07) load. `proxyd` on `:4040` and `sanctum-server` on `:8900` come up to front them. Both MLX seats load as **user** LaunchAgents (`com.sanctum.mlx`, `com.sanctum.mlx-devstral-rust`), not system daemons.
