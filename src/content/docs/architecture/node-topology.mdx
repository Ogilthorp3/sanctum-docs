...
- Runs inference servers (`sanctum-mlx`, `sanctum-mlx-codestral`, Sanctum TTS) and the `sanctum-server` router that fronts them
...
2. **Phase 2: Inference + routing** — `sanctum-mlx` (`:1337`, mTLS, Qwen3.6-35B-A3B-4bit) and `sanctum-mlx-codestral` (`:3301`, Codestral-22B-v0.1-4bit, the coder seat since the `:1338` Qwen coder + `:1234` LM Studio bridge were retired 2026-06-07) load. `proxyd` on `:4040` and `sanctum-server` on `:8900` come up to front them.
...
5. Install a local model on the satellite's MLX runtime (e.g., Qwen 3.5 3B 4-bit for limited hardware via `sanctum-mlx`, or route the satellite to the hub's `:3301` Codestral coder seat if the link is reliable).
...
|--------|----------------|----------------------|------|
| `com.sanctum.castellan` (Gollum) | `:2189` | `:2189` | RAM-pool admission + capacity governor (Rust root LaunchDaemon, superseded `sanctum-admit` 2026-05-30). Per-host doctrine. |
