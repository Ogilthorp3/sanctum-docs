...
IP=$(sanctum_node_get "$NODE" tailscale_ip)      # "100.0.0.25"
```
...
**`com.sanctum.bootstrap`** (bert) starts all services in four phases:
...
| Action | Command / URL |
|--------|--------------|
| SSH to hub | `ssh neo@100.0.0.25` |
| SSH to VM | `ssh -J neo@100.0.0.25 ubuntu@10.10.10.10` |
| Dashboard | `http://100.0.0.25:1111` |
| Home Assistant | `https://ha.nepveu.name` (via Cloudflare) |
| Agent query | Via gateway API at `100.0.0.25:1977` |
...
sanctum_node_get chalet tailscale_ip              # "100.0.0.32"
...
const chaletIp = nodeGet('chalet', 'tailscale_ip'); // "100.0.0.32"
```
...
```
    ...
    100.0.0.25               100.0.0.32              100.0.0.55
         |                             |                          |
