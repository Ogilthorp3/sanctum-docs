...
The Mac Mini hub runs one virtual machine — the Ubuntu agent VM on Lima `vmType=vz` — plus an OrbStack container runtime for the few persistent containers (Outline; Home Assistant now lives on its dedicated Green box at `10.0.0.3`). The bridge from the Mac host to the VM is `bridge100` on a private `/24`: the Mac is `10.10.10.1`, the VM is `10.10.10.10`. OrbStack runs on its own private network and does not contest `bridge100`.
...
1. **Phase 1: Containers** — OrbStack starts headlessly via its hidden-autostart LaunchAgent, Docker socket ready, Outline container up.
