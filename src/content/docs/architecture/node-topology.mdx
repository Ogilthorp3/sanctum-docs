...
The multi-site **contract** — isomorphic local design maps, name-only hive plane, no L3 merge — lives in [Hive Topology](/architecture/hive-topology/). Read that when adding a site or fine-tuning models that must share one mental map.
...
nodes:
  manoir:
    ...
    ssh_user: neo
    vm:
      host: 10.10.10.10           # agent VM over the host bridge
      ssh_user: ubuntu

  chalet:
    type: satellite
    host: ""                       # Set during on-site install
    tailscale_ip: 100.0.0.30
    ssh_user: neo
    services:
      home_assistant: { enabled: true, port: 8123 }
      agent:        { enabled: false, name: ahsoka }  # reserved seat, not yet deployed
      local_model:  { enabled: false }                # picked at install for the on-site hardware

  mbp:
    type: mobile
    host: 192.0.2.111              # DHCP on the haus LAN; varies elsewhere
...
| Devstral MLX (`sanctum-mlx-devstral`) | Devstral-Small-2-24B-Instruct-2512-4bit on `:3301`. (Superseded the Qwen2.5-Coder-14B coder seat on `:1338`, retired 2026-06-07 along with the LM Studio `:1234` bridge.) |
...
The agent VM was migrated from QEMU/HVF to Lima `vmType=vz` on 2026-05-10 to end the 370% idle-spin the QEMU/HVF bind was burning ([field note](/operations/2026-05-10-the-vision-probe-lied/)). Colima died 2026-04-27; OrbStack died 2026-08-19 ([The Engine That Wasn't There](/operations/2026-08-19-the-engine-that-wasnt-there/)). Guest docker is Lima's; the host has no container engine.
...
1. **Phase 1: Guest containers** — Lima VM docker brings Outline/Matrix up inside `sanctum-vm`; host socat forwards `:3100` / `:6167`.
2. **Phase 2: Inference + routing** — `sanctum-mlx` (`:1337`, mTLS, Qwen3.6-35B-A3B-4bit) and `sanctum-mlx-devstral` (`:3301`, Devstral-Small-2-24B-Instruct-2512-4bit, the coder seat since the `:1338` Qwen coder + `:1234` LM Studio bridge were retired 2026-06-07) load. `proxyd` on `:4040` and `sanctum-server` on `:8055` (BOSS) come up to front them. Both MLX seats load as **user** LaunchAgents (`com.sanctum.mlx`, `com.sanctum.mlx-devstral-rust`), not system daemons.
3. **Phase 3: Agent VM** — Lima starts the Ubuntu VM; the `vmnet` bridge is set up via `socket_vmnet`; the host IP is pinned to `10.10.10.1`; the boot path waits for VM SSH before declaring success.
...
Five P0 services (`force-flow`, `proxyd`, `watchdog-rust`, `ha-gateway`, `firewalla`) were promoted from user LaunchAgents to system LaunchDaemons in 2026-05-07/08 ([field note](/operations/2026-05-07-the-reboot-reveals/)). They survive any reboot pattern, including a missing `/etc/kcpassword`. The MLX seats stayed user-domain on purpose: a model that can't reach the user's Keychain boots into a brick, so they wait for login state rather than racing it.
...
bash ~/.sanctum/boot/sanctum-bootstrap.sh
```
...
Automation-lane hosts use `IdentityAgent none` with a dedicated key; local VM hosts use `IdentityAgent SSH_AUTH_SOCK`. Everything else falls through to the 1Password agent in `Host *`, which must be the last block, because ssh keeps the first value it reads for each option ([the first line wins](/operations/2026-09-13-the-first-line-wins/)). Verify with `ssh -G <host>`.
...
4. Home Assistant is the Green appliance at `10.0.0.3`, not a Mini container.
5. Install a local model the satellite hardware can hold — the chalet's M1 16 GB fits a ~10 GB 4-bit model for the on-site agent (`ahsoka`, reserved, not yet deployed). Or route to the hub's `:3301` Devstral coder seat when the link is reliable.
...
| SSH to VM | `ssh -J neo@100.0.0.20 ubuntu@10.10.10.10` |
| Dashboard | `http://100.0.0.20:1111` |
| Home Assistant | HA Green box (tailnet node, LAN `10.0.0.3`) via Tailscale |
...
|--------|----------------|----------------------|------|
| `com.sanctum.castellan` (Gollum) | `:2189` | pending activation | RAM-pool admission + capacity governor (Rust root LaunchDaemon). Superseded `sanctum-admit` 2026-05-29 and absorbed the old pressure-valve's shed-and-freeze duty. Per-host doctrine. |
| `com.sanctum.claude-max-proxy` | `:3456` | `:3456` | OpenAI-compatible HTTP wrapper around the local `claude` CLI. Each host has its own Claude Max OAuth session; a persistent SSH tunnel (MBP `:3457` to Mini `:3456`) covers cross-machine fallback. |
...
sanctum_node_get chalet tailscale_ip              # "100.0.0.30"
sanctum_node_get mbp ssh_user                     # "neo"
```
...
const chaletIp = nodeGet('chalet', 'tailscale_ip'); // "100.0.0.30"
```

## Network Diagram

```
                         Tailscale Mesh (your-tailnet.ts.net)
                    _______________________________________________
                   /                       |                       \
    Hub: manoir               Satellite: chalet              Mobile: mbp
    ...
    +-- 5 AI Agents              +-- Home Assistant       +-- castellan (pending)
    +-- Full service catalog     +-- Local model (reserved) +-- claude-max-proxy
    +-- HA gateway
