...
Despite the architectural leaps of the previous session, a single "ghost in the machine" remained: **UTM**. 

The system was capable of booting without a human, but it still required a **GUI login session** to launch the UTM application and its associated VMs. This meant that after a power failure, if the Mac Mini didn't auto-login to a desktop, the Ubuntu VM (and thus the Memory Vault, the OpenClaw gateway, and the 27B coding models) remained dark. Furthermore, the UTM "Sandbox" restricted networking in ways that made automated bridge recovery fragile.

### The Solution: Headless, Bare QEMU
...
1.  **LaunchDaemon Orchestration**: The VM is now managed by a root-level LaunchDaemon (`com.sanctum.bootstrap`) that calls a dedicated bootstrap script (`sanctum-bootstrap.sh`). No user login is required.
2.  **Socket Networking**: We implemented `socket_vmnet` to provide `vmnet-host` networking without requiring Apple Developer ID entitlements or a GUI wrapper. The VM now has a stable, reliable link to the host via `bridge100`.
3.  **Namespace Unification**: The legacy `com.manoir` (Manoir Nepveu) prefix was officially retired. All active services, LaunchAgents, and training datasets were unified under the `com.sanctum` namespace.
4.  **Intelligent Memory Triage**: The `qui_gon_triage.py` script was updated to allow LM Studio to run permanently, while automatically unloading "large" models (35B+) if system memory drops below 20%. Safari was granted "Immunity" and is no longer targeted during emergency purges.
5.  **Security Hardening**: Password authentication was disabled on the VM in favor of Key-Only SSH. Local services were rebound from `0.0.0.0` to `127.0.0.1` or the `10.10.10.1` bridge.

### Results
The "Raccoon Theorem" states that the system's resilience is measured by its ability to recover from a power failure caused by a raccoon at 4 AM without its owner ever knowing it happened. With the removal of UTM and the transition to a headless, bare QEMU architecture, that goal is achieved.

> **Principle 15: The GUI is an artifact of debugging, not a requirement for operation.**
>
> If a service cannot run without a window, it is not a service. True infrastructure is invisible. It lives in the terminal, the daemon, and the socket. By removing the dependency on the GUI login session, we have moved Sanctum from a "desktop application suite" to a true, resilient server architecture.

The machine is now a **lean, mean, headless machine.** UTM has been uninstalled. The prefixes are unified. The doors are locked. The raccoon can do its worst.

---

## Roadmap / Future Considerations

- **Hardware Constraints for Large Models**: The `test-living-force.sh` script has been updated with a memory safeguard. Attempting to load massive models (e.g., 122B parameter models) on the 64GB Mac Mini causes catastrophic overallocation, pushing swap usage beyond 10GB. This previously triggered the Qui-Gon memory triage to aggressively kill the MLX 27B model, resulting in a kernel panic due to a driver race condition. Benchmarking >100B models must be strictly isolated to the 128GB M4 Max machine.
- **Qui-Gon Memory Triage Daemon**: The production triage script at `/Users/neo/Projects/qui-gon/qui_gon_triage.py` has been updated to handle intelligent model unloading via API. Redundant artifacts (`qui_gon_triage_new.py`, `qui_gon_triage_resilient.py`) have been removed to eliminate technical debt.
