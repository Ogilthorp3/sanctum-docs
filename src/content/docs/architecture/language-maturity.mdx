...
`sanctum-firewalla` and `sanctum-tts` appear in the registry with Rust source in the `sanctum-rs` workspace, but neither binary is built or deployed; the live `:1984` listener is the Node bridge from `sanctum-screen-time/deployed/firewalla-bridge.js`, and `:8008` is still served by Python (Qwen3-TTS via mlx-audio, dispatched through the `sanctum-tts` adapter trait once it ships). The registry now reflects this with a `porting` stage rather than the false `graduated` claim it carried previously.
</Aside>
