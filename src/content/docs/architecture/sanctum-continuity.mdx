...
Yes — and the *hardest* layer gets **better**. This is not hypothetical. The OpenClaw side of the Haus already runs on Linux. The architecture is drawn on POSIX primitives with the Apple mechanisms only as inspiration, so most of it ports unchanged.
...
Ship zero-config presence first. It is one turn of work, and every later layer reads its output. Extend the `UserPromptSubmit` hook to write this session's TTL beacon, add a `Stop` hook to stamp last-activity, and have the hook read peers' beacons and inject a relevance-gated roster only when working sets intersect. Retire `vault.sh who`. Zero new infra. It works today over the Tailscale FS, and it degrades correctly at [the chalet](/architecture/node-topology/) because it is outbound-only. Build the radio first — the job board, rules, and governor all compose on it.
