...
`tq` is the only writer. This job runs on the MBP. It is **work-lane**: the Affinity key lives in `triptyq-cli` / `tq secrets`, never in haus SOPS or Power Automate.

**Date:** 2026-08-15 · **Status:** Active · **Host:** MBP · **Repo:** `~/Projects/triptyq-cli`

## What runs unattended

| Job | When | Writes |
|---|---|---|
| `com.triptyq.file-mail` | every 2 hours (Aqua) | `file-mail --all` then `file-notes --all` |
| `com.triptyq.weekly-brief` | Friday 16:07 | `reminders --execute`, then a read-only digest |
...
launchctl print gui/$(id -u)/com.triptyq.file-mail
```
...
| **M**erged | `triptyq-cli` `9842790` on `origin/main`; this chapter on `sanctum-docs` `main` |
| **A**nd **D**eployed | Plist + wrapper are repo symlinks; consumer verified through `~/.triptyq/bin/file-mail` |

Operator detail for partners lives in the work repo: `triptyq-cli/docs/02_workflows/09_affinity-filing.md`.
