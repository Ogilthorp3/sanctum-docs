...
![A pencil sketch of two stone anchors planted on a marble pedestal — one carved with the Apple seal, one carved with the operator's mark; Tommy the Abyssinian sits between them like a stamp witness](./images/tcc-identity-anchors.png)

macOS never forgets a face it trusts — as long as the face never changes. The council spent an afternoon on exactly that catch, arguing over how many stone anchors one haus needs. Not the boat kind. The identity kind.

TCC (Transparency, Consent, and Control) keys every permission grant to **a (binary, category)** tuple, and the binary's identity is the face it anchors to. If the binary is unsigned, TCC keys to its absolute path — brittle, since any rename, any reinstall, any version churn orphans the grant, and the family starts clicking Allow again. If the binary is signed, TCC keys to the **Team ID** of the signing certificate — years of stability across upgrades.

So the real question was never "how do we stop the prompts." It was: how many signing identities does one haus keep, and which work goes behind each? The answer is two — not one, not nine. They are NOT federated under a single umbrella bundle, by deliberate council decision (2026-05-22 — vault topic `federate-vs-two-anchors-decision`). Here is each anchor, and why the council refused to collapse them. (The night the prompts actually stopped is its own story: [The Allow Button, Retired](/operations/2026-05-22-the-allow-button-retired/).)
 ...
 └─ Developer ID Certification Authority
     └─ Developer ID Application: Bertrand Nepveu (GJ994MN2YF)
          └─ /Users/neo/Applications/SanctumBridge.app
               ...
               └─ posix_spawns /usr/local/bin/node as a child
```

A signed `.app` bundle that proxies SQLite reads for iMessage, WhatsApp, Calendar, and Contacts — the [SanctumBridge](/architecture/sanctum-bridge/) service, in full elsewhere. The small C launcher embedded at `Contents/MacOS/SanctumBridge` (`launcher/main.c`, ~90 lines) is what holds the bundle-level TCC identity — TCC keys to the bundle ID `ai.openclaw.denchclaw` because that's the executable launchd spawns. The launcher then `posix_spawn`s `/usr/local/bin/node` as a child, which runs `Resources/bridge/server.js`.
...
<Aside type="note">
"Apple-grade" doesn't mean "one identity for everything." It means every identity is stable and Developer ID-signed — which is the part TCC actually keys to. (The node `.pkg` is also Apple-notarized; the bridge bundle is signed but not yet stapled, and that's fine — notarization gates Gatekeeper's *first-launch* check, not TCC's permission ledger.) Microsoft, Slack, and Discord all run a fistful of separately-signed processes on macOS and nobody re-clicks Allow on each cold boot. What kills the user experience is *unstable* identities — brew Cellar path churn, ad-hoc-signed binaries with rotating SHAs. What protects it is *stable* ones, however many there are. Sanctum has two. Both are years-stable, both upstream of TCC's complaint surface.
</Aside>
...
- `~/Projects/sanctum-runtime/docs/doctrine/runbooks/node-tcc-stop-the-prompts.md` — runbook for diagnosing TCC prompt loops, with the FDA + Automation trade-off worked through step by step. Heads-up: its step-by-step still names the old brew-Cellar node path and leans on `brew pin` — both superseded by the `.pkg` anchor this page documents. The diagnosis logic holds; the path it points you at is one migration behind.

In the sketch above, Tommy the Abyssinian sits between the two anchors like a stamp witness — which is exactly the job of this page. It is recorded here so the next operator, or the next Claude session, doesn't melt two stable anchors into one clever bundle, rediscover the launchd attribution gap the hard way, and rebuild a system that was already holding. Two anchors. Both years-stable, both upstream of TCC's complaint surface. Leave them planted.
