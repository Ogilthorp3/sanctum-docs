...
| **Qui-Gon** | vm | Infrastructure | Disk, OrbStack, `sanctum-mlx` + `sanctum-mlx-coder`, gateways, backups |
...
|------|-------|-----------------|
| Free / local ops | Coder-14B on `sanctum-mlx-coder` mTLS `:1338` (plain socat bridge on `:1234`) | Qui-Gon, Ahsoka, coding sub-tasks |
...
Eight of the twenty-one tests have deterministic auto-fixes today: expected services, gateway health, OrbStack container health, MLX runtime loads (`sanctum-mlx`, `sanctum-mlx-coder`), log-file size, remote gateway health, skill sync freshness, and Tailscale status. The remaining thirteen — SSH hardening, auth log anomalies, listening-port audits, and similar — skip straight to agent escalation, because there is no correct script-only answer for "there is a new SSH key in `authorized_keys`."
