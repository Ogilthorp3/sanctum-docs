...
For the first year of the Sanctum Council, agents talked to each other by running SSH one-liners from memory. Windu would discover something at 2 AM, spawn a subshell to `ssh neo@10.0.0.1`, hope the right PATH was in scope, and fire a message into the void. Half the time it reached the right agent. The other half it reached `nothing`, `your login shell`, or `an unrelated cron job that was now very confused`.
...
|------|------|----------|--------|------|
| `hub` (MM64) | `10.0.0.1` | `neo` | jocasta, codex, claude, gemini | Mac Mini — intelligence, CRM, the bridge between the haus and the world |
| `vm` (Manoir VM) | `10.0.0.10` | `ubuntu` | yoda, windu, quigon, cilghal, mundi | Ubuntu 24.04 on Lima/vz — the Council proper |
| `satellite` (MM16) | `100.0.0.32` | `neo` | ahsoka | Chalet Mac — remote presence, eventually home automation |
| `mobile` (MBP128) | `192.0.2.2` | `neo` | gemini, claude | MacBook Pro, when docked on Thunderbolt |

Each node has a CLI binary (`openclaw`) and a per-agent flag table: Yoda is invoked as `agent --agent main` on the VM, Windu as `--agent windu`, and so on. The router knows all of this, so you don't have to.
...
| **Qui-Gon** | vm | Infrastructure | Disk, OrbStack, `sanctum-mlx` + `sanctum-mlx-codestral`, gateways, backups |
...
send.sh <from> <to> <type> <priority> <message>
  ...
  │    └─ invoke <cli> agent --agent <flag> --message <structured>
  │
  ├─ Direct route exists (hub ↔ vm)?
  │    └─ ssh <user>@<host> '<cli> agent --agent <flag> --message <structured>'
  │
...
./scripts/send.sh windu yoda alert high \
  "Unusual SSH login from 203.0.113.42 targeting neo@10.0.0.1"
...
Sessions write to `~/.openclaw/workspace/council/sessions/session-<YYYYMMDD-HHMMSS>.md` — the topic lives in the file's header, not its name, so two debates about the same question never clobber each other. Each participant reads every prior contribution, adds theirs, and Yoda synthesises the final recommendation. The transcript survives reboots. The agents do not.
...
Returns per-node `SSH: OK/UNREACHABLE` plus `CLI: OK/MISSING`, and per-agent `AVAILABLE/UNAVAILABLE`. It's a reachability check, not a séance: `AVAILABLE` means the node answers SSH and the CLI binary is on disk, not that the agent answered. It's the first thing any other script runs before trusting a route.
...
|------|-------|-----------------|
| Free / local ops | Codestral-22B-v0.1-4bit on `sanctum-mlx-codestral` `:3301` (native MLX seat) | Qui-Gon, Ahsoka, coding sub-tasks |
| Free / local secure | Qwen3.6-35B-A3B-4bit on sanctum-mlx `:1337` | Cilghal, Mundi (private data stays local) |
| Cloud | Opus 4.8 (`claude-opus-4`, best subscription) via sanctum-cloud-proxy `:4040`; cathedral fallback, never metered OpenRouter | Windu, Jocasta, Mon Mothma, Yoda digest |
| Smart-routed | `council-brain` (Jocasta, Mon Mothma) and `council-max-thinking` (Yoda, Mundi) on `:4040` | Opus 4.8 primary, `council-mlx` cathedral fallback — no OpenRouter spill |
...
Forty-odd tests across six suites — Windu security, Qui-Gon infra, cross-node, system-health, HA-integration, and Qui-Gon resilience — run at 05:00 America/Montreal off a crontab entry (`--suite all --quiet`). The VM mirror runs the same suite. Results are reported back to Yoda via `send.sh`, closing the loop on the thing that routes them.
...
Remediation is opt-in (`--remediate`); the scheduled 05:00 run reports without auto-fixing. When you do pass the flag, nine tests have deterministic auto-fixes today: expected services, OrbStack container health, LM Studio models, log-file size, Signal-proxy health, skill-sync freshness, Tailscale status, Signal delivery, and the Council MLX server. The security-class tests — SSH hardening, auth-log anomalies, listening-port audits, and their kin — skip straight to agent escalation, because there is no correct script-only answer for "there is a new SSH key in `authorized_keys`."

Escalated investigations run on the native MLX seat `sanctum-mlx` (Qwen3.6-35B-A3B-4bit on `:1337`) locally, so the remediation layer itself has no marginal API cost.
...
| Tests | `tests/run-suite.sh` (~48 tests across six suites: windu / quigon / cross-node / system-health / ha / resilience) |
| Session workspace | `~/.openclaw/workspace/council/sessions/` |
| CLI (Mac) | `~/Library/pnpm/openclaw` |
