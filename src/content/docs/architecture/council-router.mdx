...
| `vm` (Manoir VM) | `10.10.10.10` | `ubuntu` | yoda, windu, quigon, cilghal, mundi | Ubuntu 24.04 on Lima/vz — the Council proper |
| `satellite` (MM16) | `100.0.0.32` | `neo` | ahsoka | Chalet Mac — reserved; node offline, Ahsoka not yet deployed |
...
| **Qui-Gon** | vm | Infrastructure | Disk, OrbStack, `sanctum-mlx` + `sanctum-mlx-devstral`, gateways, backups |
...
|------|-------|-----------------|
| Free / local ops | Devstral-Small-2-24B-Instruct-2512-4bit on `sanctum-mlx-devstral` `:3301` (native MLX seat) | Qui-Gon, Ahsoka, coding sub-tasks |
| Free / local secure | Qwen3.6-35B-A3B-4bit on sanctum-mlx `:1337` | Cilghal, Mundi (private data stays local) |
| Cloud | Opus 4.8 (`claude-opus-4`, best subscription) via sanctum-cloud-proxy `:4040`; cathedral fallback, never metered OpenRouter | Windu, Jocasta, Yoda digest |
| Smart-routed | `council-brain` (Jocasta, Mon Mothma) and `council-max-thinking` (Yoda, Mundi) on `:4040` | `council-max-thinking` rides Opus 4.8; `council-brain` stays local on the cathedral (`qwen3.6-35b-a3b-4bit`) — no OpenRouter spill |
