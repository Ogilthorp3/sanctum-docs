...
The **Thalamus** (`sanctum-thalamus` on port 1988) plays that exact role. It receives high-salience broadcasts from individual components — a temperature anomaly from the haus World Model, a root-cause from Watchdog — and injects them into the `HEARTBEAT.md` context block of every active agent in the [Jedi Council](/architecture/agents/). No agent has to call another. They simply share a stream of consciousness, the way you overhear a room without asking anyone to repeat themselves.

## The Dream Cycle

A swarm that only reacts never learns. So every night at 05:25 AM, the `council-local-think` seat wakes up and dreams.

This is memory consolidation, modeled on biological REM sleep. The **Dreams** job (`sanctum-dreams.py`) reads the daily diffs from Vault, the anomalies from Home Assistant, and the broadcast history from Thalamus. It compresses those disparate events into a single cohesive narrative digest, stored in `~/.sanctum/dreams/`, and injects the connections it found into the Graphiti knowledge graph. The point is systemic learning, not local. The swarm should wake up understanding a pattern that no single agent lived through — the same instinct that drives the [Living Force](/architecture/living-force/).
