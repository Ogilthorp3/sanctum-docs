...
![Eval Harness — a sensor-wired skeleton in the lab and a balance scale where EVIDENCE outweighs VIBES; the conscience the Carmack Olympics finally grew.](./images/hero-eval-harness.png)
...
The cathedral fork serves the council seat on `:1337` over **mTLS-only** in production (the former coder `:1338` seat retired 2026-06-07; the CODER role now runs Codestral-22B on `:3301`). Every eval pointed at `https://100.0.0.25:1337/v1` (or `manoir.tail7c6d11.ts.net`) must present a client cert signed by the Sanctum CA.

The PKI lives at `~/.sanctum/certs/` on every Sanctum machine. Per-client cert and key pairs are issued for each named consumer (guardian, canary, drift, sanctum-server, parity-smoke, etc.). Two convenience flows:

```bash
# One-off curl probe with the sanctum-server client identity
curl --cacert ~/.sanctum/certs/ca.crt \
     ...
     https://100.0.0.25:1337/v1/models

# Pass through to the eval
.venv/bin/python scripts/carmack_eval_http.py \
  --url https://100.0.0.25:1337/v1 \
  --model mlx-community/Qwen3.6-35B-A3B-4bit \
  --label council-remote \
  --ca-cert ~/.sanctum/certs/ca.crt \
  --client-cert ~/.sanctum/certs/clients/parity-smoke.crt \
  --client-key  ~/.sanctum/certs/clients/parity-smoke.key \
  --output eval/council-remote.json
```

`scripts/test_eval_harness.sh` auto-discovers the cert paths in the same `~/.sanctum/certs/` tree. Localhost probes (the council-guardian running on the Mini itself) connect to the same mTLS endpoint; loopback presents its own client cert from the local PKI — there is no plain-HTTP loopback bypass on the cathedral fork.

<Aside type="caution">
The Rust `sanctum-mlx` backend returns its model ID as a raw snapshot hash (e.g. `38740b847e4cb78f...`) rather than the friendly `mlx-community/Qwen3.6-35B-A3B-4bit`. The harness's model-ID parser handles both, but if you're scripting around `/v1/models` directly, don't assume the friendly name.
</Aside>

<Aside type="note">
Bearer-token auth (the P5 mechanism) survives on the plain `:1337` listener for migration purposes only; production binds `--no-plain` and refuses the plain socket entirely. Any eval still using `COUNCIL_API_KEY` is targeting a non-canonical path.
</Aside>
