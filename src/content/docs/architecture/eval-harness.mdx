...
The goal isn't to produce nicer dashboards. It's to make every routing decision defensible with a number you can audit six months from now, on a clean machine, with `git checkout` as the only setup step. Those decisions feed [dynamic model routing](/architecture/dynamic-model-routing/) and the [champion gate](/architecture/champion-gate/); neither deserves to rest on a scoreboard that flatters itself.
...
<Aside type="caution">
The Rust `sanctum-mlx` backend returns its model ID as a raw snapshot hash (e.g. `38740b847e4cb78f...`) rather than the friendly `mlx-community/Qwen3.6-35B-A3B-4bit`. That hash is the model snapshot directory name — it's stable, just not pretty. The harness's model-ID parser handles both, but if you're scripting around `/v1/models` directly, don't assume the friendly name.
</Aside>

<Aside type="caution">
**The eval CLI hasn't caught up to the server.** `carmack_eval_http.py` still authenticates with a Bearer token (`--api-key-env COUNCIL_API_KEY`) over plain HTTP — it has no `--ca-cert/--client-cert/--client-key`, so it physically cannot present a client cert. Against a `--no-plain` mTLS seat that means the connection dies before the first byte. This is the exact overclaim this page preaches against, so here it is named: the *server* is mTLS-only today; the *client* is the migration debt. Until the eval grows mTLS flags, remote council runs go through the `curl` probe above, or against a seat still speaking plain Bearer. See [the mTLS migration](/operations/2026-04-21-mtls-migration/) for the server side that landed first.
</Aside>
...
- **LLM-as-judge disagreement** — Carmack uses programmatic keyword rubrics, not an LLM judge. This lives in the [Standard Olympics track](/architecture/sanctum-olympics/) if needed.
