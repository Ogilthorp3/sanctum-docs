...
| **Qui-Gon** | **coder — Qwen2.5-Coder-14B** (LM Studio `<MINI>:1234`) | council-secure | Dense code gen, infrastructure pragmatism; 22 tok/s on the Mini |
...
| **Ahsoka** | coder — Qwen2.5-Coder-14B (chalet LM Studio) | — | Runs on a Mac Mini M1 16 GB at the chalet; 35B won't fit |
...
The Olympics (2026-04-23) informed this lineup: Qwen3.6-35B-A3B at 0.957 is still the house default for any seat without a domain-specific upgrade — Mothma/Jocasta/Cilghal stay there because their work is privacy-critical or local-by-mandate, and Coder-14B (0.929) stays on the bench for coding-specific work and the chalet where RAM is the constraint. Opus 4.7 and Gemini 3.1 Pro now sit at the head of two seats each, not as escalations. See [Model Comparison](./model-comparison) for the full per-task score breakdown and [(Neuro)diversity is Paramount](./neurodiversity-doctrine.mdx) for the doctrine that made this routing the default rather than the escalation.
