...
He uses two unprivileged levers:
...
3. **Deadman-timed everything.** Every SIGSTOP he sends gets a tombstone file under `/var/run/castellan/stopped/` named for the PID. A companion process — `castellan-deadman`, a separate launchd job — watches his heartbeat at `/var/run/castellan/heartbeat`. If the heartbeat goes stale for more than sixty seconds, the companion SIGCONTs every process the Castellan left stopped and clears the tombstones.
...
The Castellan inherits port 2189 with a drop-in API surface — `/status`, `/services`, `/load/{svc}`, `/release/{svc}` — plus the new `/ensure-loaded/{svc}` endpoint the proxyd code-seat hook calls. The trio's plists are renamed `.retired-YYYYMMDD` by the cutover script; backups live under `~/.sanctum/backups/castellan-cutover-YYYYMMDD-HHMMSS/`. Rollback is a manual rename and three `launchctl bootstrap` calls.
