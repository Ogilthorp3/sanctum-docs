...
`GET /export` on the bridge (port 18094) dumps the full Firewalla configuration: all hosts, active policies, custom DNS records, and service domain mappings. Combined with the `firewalla-export.sh` and `firewalla-import.sh` scripts, the entire enforcement state can be backed up and restored on a new Firewalla box (hello, Gold Pro) without reconfiguring anything by hand.
...
2. **Bridge running?** `lsof -i :18094`. The bridge handles the actual Firewalla API calls and SSH fallback.
...
4. **Stale DNS rules?** `POST /host/:mac/unrules` on the bridge (port 18094) batch-removes all DNS blocks for a MAC with SSH fallback.
...
| Bridge | `~/.openclaw/firewalla-bridge.js` (port 18094) |
