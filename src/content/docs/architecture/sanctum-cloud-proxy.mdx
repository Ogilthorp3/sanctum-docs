...
2. **Fallback Chain** — If a cloud provider returns a 429 Rate Limit, a 500 Server Error, or times out, the proxy automatically falls back to local MLX seats (first the `Codestral-22B-v0.1-4bit` coder on `:3301`, then the `Qwen3.6-35B-A3B-4bit` council seat on `:1337`) without the client ever knowing the cloud failed.
