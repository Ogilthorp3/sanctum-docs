...
`proxyd` binds `0.0.0.0:4040` — all interfaces, so it answers on both `127.0.0.1` and the `10.10.10.1` Mac↔VM bridge. It intercepts every LLM API request and runs it through five layers of verification before forwarding to the cloud (Anthropic, OpenRouter, Gemini) or routing it to a local MLX seat.
