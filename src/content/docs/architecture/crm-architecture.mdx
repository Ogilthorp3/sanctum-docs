...
![Jocasta pipeline — deal and relationship state flowing into one memory vault](../../../assets/illustrations/jocasta_pipeline.png)

Sanctum CRM is the haus relationship memory. Every conversation that lands on the Mac — mail, iMessage, SMS, Signal, WhatsApp, Telegram, LinkedIn — is ingested into one local DuckDB store. Affinity deal flow and a personal angel ledger sit beside it. The Archives UI is a thin, read-only window on that store: if the UI burns down, zero bytes of intelligence burn with it.
