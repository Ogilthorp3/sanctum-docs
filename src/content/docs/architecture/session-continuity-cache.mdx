...
![The proxy hands a still-lit conversation thread from one AI seat to the next mid-flight — the thread never goes dark, even as the brain behind it changes](./images/hero-session-continuity-cache.png)

<Aside type="caution" title="Design, not shipped">
This page describes a **planned** proxy capability, not a live one. It is on the [Roadmap](/operations/roadmap/) under *Later*. Nothing here runs yet. It exists so the idea is written down honestly instead of living in one operator's memory.
</Aside>

The [Sanctum Proxy](/architecture/proxy/) walks a fallback chain: `Max → Gemini → Grok → glm-52 → cathedral`. When the top seat runs out of quota mid-conversation, the next seat takes over. The routing works. The *feeling* does not: the reply after a switch can read like you handed your notebook to a new person who skimmed it and started talking. This page is about closing that gap.
