...
- Claude itself being prompt-injected into a malicious upload — mitigated
  ...
  (`work-skills/sharepoint-structure.yaml`), the hard-coded
...
- `com.sanctum.bridge-canary` — every 6 h. Writes a tiny payload via
  the public bridge into `01_Fund Admin/_canary/canary-<host>.txt`
...
- `com.sanctum.bridge-manifest-sync` — daily 06:30 local; runs
  `work-skills/scripts/sync-bridge-manifest.py`. Idempotent unless the
...
- `work-skills` (private work repo) — skill helpers, sharepoint-structure.yaml, sync-bridge-manifest.py.
...
2. **Allowlist SoT — `work-skills/sharepoint-structure.yaml`.** A stale
...
5. **Slack auth — per-channel incoming webhooks + channel-ID allowlist.**
   ...
   both point here.
6. **Bridge canary — `01_Fund Admin/_canary` with
