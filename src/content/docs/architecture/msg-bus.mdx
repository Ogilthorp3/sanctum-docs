...
- **Dispatch policy**: per-severity failover lists in `config.yaml`. P0 cascades through every wired transport; P1 stops at HA-push; P2 is iMessage-only. A fourth tier (SMS via voip.ms) and quiet-hours digest batching are written into the policy but not yet wired — see [What's not wired yet](#whats-not-wired-yet).
...
- **Retire**: `launchctl bootout gui/$(id -u)/com.sanctum.msg-bus` + rename the plist `.retired-YYYY-MM-DD`. Notifications are unaffected — Force Flow still dispatches per-channel directly (see [What's not wired yet](#whats-not-wired-yet)), so until `/enqueue` has live producers, retiring the bus is a no-op for delivery.
