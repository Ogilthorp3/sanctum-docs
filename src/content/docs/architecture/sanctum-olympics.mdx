...
![The Sanctum Olympics — three AI models competing on holographic podiums in a Jedi temple arena.](./images/hero-olympics.png)
...
The backend keys are `council-27b` (the local MLX Council seat on `:1337`), `codestral` (the `:3301` coder seat — the key name predates the cutover; that port now serves Devstral-Small-2-24B), and `claude-opus` (cloud, via OpenRouter) — defined in `config.yaml`, not hardcoded in the runner.
