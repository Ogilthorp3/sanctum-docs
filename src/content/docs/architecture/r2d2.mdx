...
Some breakage announces itself. A gateway crashloops, the phone rings, an operator fixes it. The quiet kind is worse: a LaunchAgent respawning a dead VM for nineteen days while nobody happens to look. R2D2 is the droid for the quiet kind.

It sits between the four purpose-built autohealers (OpenRouter key rotation, mlx-drift bootout, SOPS sync, mgmt-key failover) — each solving exactly one problem — and the human escalation path down [Force Flow](/architecture/force-flow/). It is the generalized dispatcher for the long tail: anything novel-but-bounded an operator would otherwise repair by hand at 11pm.
...
Nineteen recipes ship, each a four-tuple of (detector, script, dry-run policy, cooldown). Adding one more is a YAML entry plus a shell script — no code change, no privilege escalation surface. A representative slice:
...
| `snapshot-openclaw-gateway-good` | low | The gateway is active and stably up but no fresh `openclaw.json.bak-good-*` exists for the crashloop heal to roll back to | Snapshot the live config — but only when the gateway is stable and the on-disk mtime predates the gateway's start, so a mid-migration edit can never be frozen as known-good; prune to the last 5 |
...
| `heal-sanctum-mlx-codestral-down` | high | `sanctum-mlx-codestral` in a sustained crash-loop (3+ in 5 min) or its log shows repeated model-load / mTLS / Metal failures | Bootout codestral, preserve the broken plist (`.failed` + timestamp), bootstrap `sanctum-mlx-coder` as the Qui-Gon fallback; vault-announces critical. Written for the pre-Devstral coder seat — its target label no longer loads (the live coder service is `com.sanctum.mlx-devstral-rust`, Devstral on :3301), so this recipe sits dormant until it is rewritten for the new seat |
...
| `heal-claude-max-auth-refresh` | high | `claude auth status` reports `loggedIn: false` — the Claude Max OAuth refresh token is dead (the real signal, not the keychain `expiresAt`, which drifts and false-alarms) | Exercise an authed call to refresh the access token + bounce the `:3456` proxy; escalate the human-only `claude auth login` only if still dead |
| `heal-force-flow-bridge` | high | Force Flow's haus-control reconciler gets HTTP 401 from the Mac Firewalla bridge (`127.0.0.1:1984`) — a Lima VM shadowing the port, or token drift | Wrap `force-flow-bridge-sentinel.py --force`: add the lima 1984-ignore guard and drop the guest listener, or rewrite the canonical token from the live bridge |
...
- **Path two** — Hermes ingest. Tails Force Flow log and [chitti samskara](/architecture/chitti/) past a bookmark, classifies each new line with `nousresearch/hermes-3-llama-3.1-70b` via OpenRouter, returning `{auto:<recipe-id>, escalate:<reason>, info}`. Capped at 5/cycle × ~$0.0001 ≈ $0.07/day under full load.
...
R2D2 is generative help, not load-bearing — the line the [Reliability Doctrine](/architecture/reliability-doctrine-v1/) draws between what must never go dark and what may. If the plist crashes, notices keep flowing to Force Flow and chitti exactly as before — the failure mode is "less helpful," not "broken."
