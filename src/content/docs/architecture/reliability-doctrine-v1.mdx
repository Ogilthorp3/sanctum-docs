...
![Reliability Doctrine — a lighthouse on a rocky shore at dusk, its beam sweeping an empty sea, the keeper invisible inside and the light the only proof anyone is home.](./images/hero-reliability-doctrine.png)
...
<CardGrid>
  ...
  <Card title="Layer 2 — Probe" icon="magnifier">
    Liveness, cadence, and **a contract assertion**. Cadence is tier-proportional: 60s for the haus- and council-critical probes, 5 min for the ops-critical ones, longer for slow canaries that cost real tokens to run. The contract assertion exercises the actual work surface — does the bridge return hosts, does the model serve tokens, does the gateway round-trip a synthetic event. Endpoints that return 200 because the process exists are not probes; they are theater.
  </Card>
  ...
  <Card title="Layer 4 — Runbook" icon="document">
    Both human-readable and machine-executable. The human form is a markdown page in `docs/doctrine/runbooks/<service>.md`; the doctrine merge gate refuses any new always-alive plist that ships without one. The machine form is the consolidated `tools/sanctum-recover` umbrella — `--list`, `--dry-run`, `--json`, or a bare run to actually fix — which auto-detects and heals the known break-glass scenarios rather than scattering one `.sh` per service. Drill each at least once per quarter; a failed drill is a doctrine bug, not a footnote.
  </Card>
</CardGrid>
...
|---|---|---|
| `HEALTHY` | Liveness OK, cadence OK, contract OK | No action |
| `DRIFT` | Liveness OK but readiness or contract divergent | Failsafe (drift correction) |
...
| **P2** | Signal (held to the log during 23:00–07:00 EDT quiet hours) | Hours |

Heal-then-escalate is the intended shape: try to fix it first, widen the channel only when persistence proves the system can't fix itself. Today that's a single flat fuse — the [Force Flow](/architecture/force-flow/) failsafe probes every 60s and, after five consecutive failures (~5 min), fires one P0 iMessage directly, bypassing the very path it's watching. The graduated ladder the doctrine calls for — heal at two minutes, heal again plus a Signal heads-up at five, iMessage and a phone call at ten — is the target, not yet the wiring.

P0 alerts override quiet hours, always. The haus does not wait until morning.

## Why This Class of Doctrine Exists

The doctrine isn't novel — its parts are well-known reliability patterns from SRE practice and military doctrine. What's specific to Sanctum is the *commitment* to apply all four layers to every always-alive service, and to treat silence as a failure mode equal in severity to a crash. Where the [Resilience Doctrine](/architecture/resilience-doctrine/) keeps the haus alive when the grid or the uplink dies, this one keeps the always-alive services honest about whether they're actually alive.
