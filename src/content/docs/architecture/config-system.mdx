...
services:
  ...
  mlx_codestral:
    enabled: true
    port: 3301              # sanctum-mlx-codestral serving Codestral-22B-v0.1-4bit (CODER role)
  council_mlx:
    enabled: true
    port: 1337              # sanctum-mlx serving Qwen3.6-35B-A3B-4bit
  voice_agent:
    enabled: true
    port: 1138
  # ... additional services
...
| `sanctum_slug` | Return the instance slug | `sanctum-hub` |
...
| `sanctum_node_get <node> <path>` | Read a value from a specific node | `sanctum_node_get satellite tailscale_ip` |
...
<Tabs>
  ...
  <TabItem label="VM (SOPS + age)">
    ...
    The `sops-start.sh` wrapper decrypts secrets into environment variables, starts the gateway, and never writes plaintext to disk. The secrets exist in memory only, like a thought you can't quite remember but that still controls your haus.
  </TabItem>
</Tabs>

<Aside type="danger">
  Never commit secrets to Git, never add them to `instance.yaml`, and never pass them as command-line arguments (they appear in `ps` output). Use Keychain on macOS and SOPS on the VM. If you're reading this because you already did one of those things -- rotate immediately. We'll wait.
</Aside>
