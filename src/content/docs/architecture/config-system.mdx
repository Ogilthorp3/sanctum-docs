...
| `sanctum_slug` | Return the instance slug | `haus` |
...
<Tabs>
  ...
  <TabItem label="VM (SOPS + age)">
    ...
    The `sops-start.sh` wrapper decrypts secrets into environment variables, starts the gateway, and never writes plaintext to disk. The secrets exist in memory only, like a thought you can't quite remember but that still controls your house.
  </TabItem>
</Tabs>
