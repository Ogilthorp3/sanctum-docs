...
Every port, every IP, every feature flag in the haus traces back to one file. Change it, and the dashboards, the LaunchAgents, the shell scripts, and the voice agent all follow -- some reading it directly, some through artifacts generated from it. Secrets are the one thing it never holds; those live apart, in the macOS Keychain. This is Sanctum's single source of truth, and it fits in a single YAML file.
...
The central configuration file lives at `~/.sanctum/instance.yaml`. It contains every instance-specific value: network addresses, service ports, [node definitions](/architecture/node-topology/), feature flags, and integration settings.
...
# The coder seat (Devstral at :3301) is NOT a services.* block --
# it's a router backend. The router owns where prompts get sent:
router:
  backends:
    coder:
      url: https://127.0.0.1:3301/v1  # com.sanctum.mlx-devstral-rust, mTLS (no-plain)
...
<Tabs>
  <TabItem label="macOS (Keychain)">
    ...
    Secrets are rotated monthly by the `com.sanctum.rotate-secrets` LaunchAgent, which runs on the 1st of each month at 3:30 AM -- the full cadence lives in [secret rotation](/operations/secret-rotation/). Nothing like waking up to fresh tokens you didn't have to think about.
  </TabItem>
