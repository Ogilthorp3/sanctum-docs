...
The KV cache is where long conversations go to eat memory. Every token a language model generates leaves behind a pair of tensors — the keys and values from every attention head in every layer. For a 27B model at 256-dim heads, that's hundreds of megabytes per thousand tokens. On a Mac Mini that also runs `sanctum-mlx-coder`, Home Assistant, Sanctum TTS, and a dozen other services, the KV cache is the thing that decides whether you get to have a conversation at all.
