...
<Aside type="tip" title="2026-04-24 — production architecture is now Slice 4a-final">
  The shipping `CompressedKVCache` does **not** compress keys. After measuring, the Lloyd-Max + FWHT + QJL key path turned out to be both the throughput tax (CPU round-trip every step) and a negligible memory win at our context lengths (~4 MB per attention layer at T=2048) — nothing on a 64 GB Mini. Keys now live as a plain on-device Array; values stay 4-bit group-affine compressed; a custom Metal kernel (`sdpa_dequant_v`) fuses dequant + attention so V never materializes as a full tensor. The live blended ratio the server logs is **~1.3×** — values shrink ~2×, keys ride at full bf16 width, and the average lands between. The Slice 1 algorithm and protocol below remain accurate as the *measurement-discipline scaffolding* that got us here, but the production storage model has changed. See [Slice 4 Scoping](/architecture/turboquant-slice-4-scoping/) for the pivot.
</Aside>

The KV cache is where long conversations go to eat memory. Every token a language model generates leaves behind a pair of tensors — the keys and values from every attention head in every layer. The production model, Qwen3.6-35B-A3B-4bit, is mercifully hybrid: only 10 of its 40 layers run full attention (the other 30 are linear-attention and hold no standard KV cache), but each of those 10 still hoards keys and values at 256-dim heads. On a Mac Mini that also runs `sanctum-mlx-devstral`, the Home Assistant gateway, Sanctum TTS, and a dozen other services, the KV cache is the thing that decides whether you get to have a conversation at all.
...
Both halves live in [`services/sanctum-mlx/src/turboquant/`](https://github.com/Ogilthorp3/sanctum-rs/tree/main/services/sanctum-mlx/src/turboquant) and hand off to a `CompressedKVCache` that implements `mlx_lm::cache::KeyValueCache` — a drop-in for stock `ConcatKeyValueCache`. Routed via the `KeyValueCache::fused_attention` trait method.

This is not a flag waiting in the wings: the production `com.sanctum.mlx` LaunchAgent passes `--turboquant` against Qwen3.6-35B-A3B-4bit, and the server log emits a `turboquant ... ratio=1.32x` line for each of the 10 full-attention layers (3, 7, 11, … 39) on every request. The `--turboquant` flag and the Slice-1 history below describe how it earned that default seat.

<Aside type="note" title="Why keys stopped being compressed">
  The paper's full recipe (Hadamard rotate → Lloyd-Max → QJL) shipped in Slice 1 and was deleted in commit `37853d8` (2026-04-25). Two findings: at our context lengths the key compression saved only ~4 MB per attention layer — nothing on a 64 GB Mini — and the CPU dequant round-trip every decode step was the dominant throughput tax. Plain bf16 keys on device + group-affine V + the fused kernel keep the V memory savings without that tax. The whole `TurboQuantizer` + `codebook.rs` + `rotation.rs` machinery went with it; `key_bits` and `use_qjl` survive only as vestigial config fields. Receipts and earlier-attempt lessons live in `project_turboquant.md` in agent memory.
</Aside>
...
Never change two variables between runs. The sweep harness supports this through environment variables — the test binary reads `SANCTUM_TQ_KEY_BITS`, `SANCTUM_TQ_VALUE_BITS`, `SANCTUM_TQ_GROUP_SIZE`, `SANCTUM_TQ_USE_QJL`, `SANCTUM_TQ_HEAD_DIM` and installs them into a `OnceLock` before the model is loaded. No recompile between runs.

The shell driver [`bench/tune.sh`](https://github.com/Ogilthorp3/sanctum-rs/blob/feat/turboquant-kv-slice-1/services/sanctum-mlx/bench/tune.sh) orchestrates the staircase. Individual stages (`ablation`, `key`, `value`, `group`, `qjl`) can be run in isolation.

<Aside type="caution" title="This harness lives on a branch">
  The full `SANCTUM_TQ_*` knob harness, `tune.sh`, the `bench/` tree, and the `BypassMode` short-circuits below are all on the `feat/turboquant-kv-slice-1` branch — they were the measurement scaffold. After the Slice 4a-final pivot deleted the CPU key path, `main` kept only the trimmed `turboquant_ppl` regression test, which reads just `SANCTUM_MLX_TEST_MODEL` (and optionally `SANCTUM_MLX_TEST_TEXT_FILE`). The sweep commands here reproduce the original tuning; check out that branch to run them.
</Aside>
...
The gate is `abs_delta ≤ 0.3 AND rel_delta ≤ 5%` on a short eval. That's permissive compared to the TurboQuant paper's reported < 0.1 Δppl, and intentionally so — our implementation omits some of the paper's refinements (learned rotations, full TurboQuant value pipeline) in favor of shipping Slice 1.

A config that merely passes that short-eval gate was, in Slice 1, only good enough to ship behind a flag — not yet good enough to enable by default. Promotion to default required clearing a higher bar:
...
The Bold config cleared that bar in Slice 1b (below), which is why `--turboquant` now ships in the production plist. The section that follows walks the evidence that earned it.
...
We added `BypassMode::{IdentityPassthrough, BridgeOnly}` — short-circuits that skip the quantization pipeline (Slice-1 branch only; the Slice 4a-final pivot deleted the CPU bridge they probed). Result:
...
`TurboQuantConfig::default()` on `main` is still `key_bits=3, value_bits=4, use_qjl=true` — the original scaffold had the right intent, and the protocol gave us **evidence** it was right rather than a hunch. After the Slice 4a-final pivot only `value_bits` and `value_group_size` actually steer storage; `key_bits` and `use_qjl` are now vestigial fields (the key-compression path was deleted in `37853d8`).

### `--turboquant` is on by default in production

Slice 4 landed: the Metal-fused dequant kernel `sdpa_dequant_v` is the production attention path, and the old O(T²) CPU round-trip that justified the opt-in caution is gone with the key pipeline. So the flag did flip — `com.sanctum.mlx` runs Qwen3.6-35B-A3B-4bit with `--turboquant`, and the live ~1.32× blended ratio is the steady state, not an experiment.

What stays narrow is **model coverage**: the 35B and the original Qwen3.5-27B are the validated datapoints. Qwen2.5-Coder, Gemma4-31B, and anything else still deserve their own protocol run before we'd trust the same default — see Slice 1c below for the cross-architecture wall we hit trying. Evidence dictates defaults; evidence is narrower than ambition; keep the knob.
...
Full run log: [`bench/turboquant_sweep.jsonl`](https://github.com/Ogilthorp3/sanctum-rs/blob/feat/turboquant-kv-slice-1/services/sanctum-mlx/bench/turboquant_sweep.jsonl). Decisions recorded in [`bench/ANALYSIS.md`](https://github.com/Ogilthorp3/sanctum-rs/blob/feat/turboquant-kv-slice-1/services/sanctum-mlx/bench/ANALYSIS.md).
