...
{/* TODO: hero image — pencil sketch per CONTRIBUTING, generate with tools/gen_hero_image.py when RAM is free (Flux swap-thrashed on 2026-07-14). Prompt: a rumpled detective at a masquerade where every guest keeps swapping identical blank masks; he ignores the masks and reads the name each guest embroidered on their own collar; a corkboard of crossed-out address labels behind him; Tommy the Abyssinian cat observing from a side table; one teal halo on the guest whose self-given name he is reading. Out: ./images/hero-the-device-that-announces-itself.png */}
