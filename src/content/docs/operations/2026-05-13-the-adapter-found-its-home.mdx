...
The cathedral now carries `f0025e7` — Phase 4 generic `load_and_merge<M: ModuleParameters>`, the [Rust LoRA merge](/architecture/lora-rust/) generalized over the MoE model, riding on top of the fused-qkv branch.
...
Every night the [champion gate](/architecture/champion-gate/) names a winner; until today, nobody carried it to the throne. `council-autoresearch/promote_champion.sh` (commits `a6e639c` + `f3c522d`) is the crown-bearer. 4-Layer Critical Service Pattern:
...
Preflight (refuses to deploy a dense-shaped adapter onto MoE — [the 2026-05-11 trap](/operations/2026-05-11-the-shield-and-its-shadow/), where a champion trained against the wrong base loaded zero pairs):
