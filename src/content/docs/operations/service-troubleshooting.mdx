...
1. The `sanctum-mlx` server (mTLS `:1337`) is down or bound incorrectly.
2. The plain-HTTP `socat` bridge that fronts `sanctum-mlx-coder` on `10.10.10.1:1234` is gone (or `sanctum-mlx-coder` itself on mTLS `:1338` is down behind it).
