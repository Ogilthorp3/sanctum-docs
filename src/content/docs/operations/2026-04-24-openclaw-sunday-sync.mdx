...
description: The council broke at 2 AM with "Unknown agent id yoda" — the cause was version drift. A weekly tarball heartbeat, three tripwires, and a Monday canary keep three openclaw copies in lockstep across the Mini and the airgapped VM.
---

import { Aside, Steps } from '@astrojs/starlight/components';

![Sunday Sync — a sealed-vault VM receiving a weekly tarball from the Mini through a narrow airgap pipeline, with a Monday canary waiting with a checklist to confirm all three copies landed on one version.](./images/hero-openclaw-sunday-sync.png)

The council broke at 2 AM, and the error made no sense: `Unknown agent id "yoda"`. Yoda is the orchestrator — the one agent that is always supposed to be there. Getting told he does not exist is like a courtroom convening to discover it has no judge.

The cause was not Yoda. The haus keeps three copies of openclaw across two machines: the Mac Mini runs the orchestrator and CLI, the OpenClaw VM runs the gateway and council agents, and a third copy — the npm-global CLI shim on the VM — sat a version behind both. Two minor versions of drift between caller and callee silently eats agent config fields; the newer CLI wrote config the older one dropped as stale, including the field that named the agents. So the council forgot its own chairman.
...
1. **LLM backend was down post-reboot.** `sanctum-mlx` on `:1337` listening but not serving — the same [listening-but-silent failure the reasoner threw on April 19th](/operations/2026-04-19-council-mlx-silence/) — and the LM Studio fallback referenced `qwen/qwen3.5-35b-a3b`, retired 2026-04-22. Both paths dead.
...
Fixed in [`openclaw-skills a764146`](https://github.com/Ogilthorp3/openclaw-skills/commit/a764146): Yoda is always orchestrator and synthesizer, never a participant — the script now filters `yoda` out with a `NOTE` when callers include it out of habit. It is the same chairman-not-participant rule the [Council Router](/architecture/council-router/) enforces at the routing layer, arriving one script later at the session layer.
...
None of this is glamorous. It is a tarball, a symlink, and two launchd jobs that mostly do nothing. But drift is the failure that only surfaces at 2 AM, addressed to the one agent who is supposed to always answer — and the entire point of a weekly heartbeat is that nobody has to be awake to read the log where it quietly didn't happen.
