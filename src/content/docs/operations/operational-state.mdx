...
The architecture pages are where Sanctum gets to be beautiful — blueprint, mythology, council sessions in a Québec basement. It is the page that shows up the next morning to check whether any of it actually ran.

Call it the ledger the haus keeps against itself. Less romance, more receipts: what was verified on the actual machine, on an actual Tuesday, by a test run that either passed or didn't. Tommy watches the dashboards from the corner of the ops center, unimpressed, the way he watched everything in life. Nothing below is a claim. Each line is something that returned exit 0.

## The workspace-owned slice

What the workspace repo owns and can prove on its own:
...
- The canonical agent persona markdown now lives in the raw `mlx-finetune` workspaces and is synced mechanically into runtime workspaces via `~/.sanctum/scripts/sync_agent_markdown.py`
...
- `~/.sanctum/scripts/sync_agent_markdown.py --check --audit-live` verifies that reachable `~/.openclaw/workspace*` copies still match the canonical `IDENTITY.md`, `SOUL.md`, `TOOLS.md`, and `HEARTBEAT.md` sources and that the MBP still looks like a healthy fanout target
...
What the live Mac Mini runtime actually reports:
