...
![Tommy the Abyssinian runs a white glove along the shelf of a spotless clean-room install, staged for a stranger who hasn't arrived yet.](./images/hero-clean-for-a-stranger.png)
...
runs is a different animal, and the only way to know is to become the stranger. Tommy the Abyssinian,
the haus's force-ghost, would have run a white glove along the shelf before the guest arrived. That
is the instinct we needed to borrow.
...
and fall back to *nothing* when unknown, never to ours. This is the
[no-hardcoded-endpoints rule](/architecture/beta-portability/) meeting its first real stranger:
the same build has to run on a second haus without smuggling in the first operator's identity.
...
teaching from a new angle, the same one that produced [the installer a friend can trust](/operations/2026-05-22-the-installer-the-friend-can-trust/):
*become the user before you ship to them.* The stranger's machine is where
