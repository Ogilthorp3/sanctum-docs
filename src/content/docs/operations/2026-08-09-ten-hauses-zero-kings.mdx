...
A gate that cannot fail is worse than no gate, because it also spends the credibility. The same project had already shipped a colour check whose material cache was keyed on a resource ID that comes back empty and shared under `--headless` — every surface hashed to the same nothing, and the check passed on all nine armies while telling nobody it had compared a thing to itself.
