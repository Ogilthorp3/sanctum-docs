...
The council runs on a Rust inference binary. Two seats front it — the 35B cathedral on `:1337` and the Devstral coder on `:3301`. For almost every failure, they cover each other and proxyd routes between them. But they share one weakness, and closing it is what this is about.
...
Two voice questions got answers this round.
