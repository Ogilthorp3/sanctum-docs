...
*rotting*. We would open it one morning to find **460 files staged for
deletion**, including `.gitignore`, `CLAUDE.md`, and the submodule pointers. The
next session would find `HEAD` sitting on a commit from **two months ago**, as
if the last sixty days of work had never happened. And through all of it, a
`.git/index.lock` kept reappearing — clear it, and twenty minutes later it was
back, a corpse that would not stay buried.
...
The sharpest save was one the council's adversarial reviewer made. The naive
plan wanted to push a diverged submodule's local commit "to be safe" — a `git
push` that would have been a non-fast-forward and, if forced, would have
**destroyed two commits that existed only on the remote**. A single `git
patch-id` settled it: the local commit was byte-for-byte a *duplicate* of a
commit already upstream. No push. No force. No loss. Falsify before you fortify
— even when the plan is your own.

Once relocated, work lands the one sanctioned way — through
[gitsync](/architecture/gitsync/), which enforces the pathspec discipline so a
sweep from one machine never rides along on another session's commit. This was
not the first time the workspace hid something in plain sight; it once buried
[two stones in a `Cargo.toml`](/operations/2026-06-01-the-workspace-that-hid-two-stones/)
that compiled clean and inferred nothing.

The repo now lives at `~/Projects/Claude_Code` on both machines, non-synced,
verified, with every rescued byte in place. Tommy, the haus's force-ghost,
files his weather report twice a day and has never once needed a rollback; the
git repo can finally say the same. The thing that kept eating it is no longer in
the room.
