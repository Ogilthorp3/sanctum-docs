...
The eval seat is `council-code` through proxyd mTLS (`canary` client cert). Five JSON fields do not get the 27B. A missing work-lane Affinity key (`triptyq-affinity-api-key`) logs and skips CRM writes; it does not open a window. `portco_sync` no longer invents `linkedin.com/company/{slug}` when Exa misses — it skips. One DuckDB row and one Affinity note per company per calendar day. Org and person are find-or-create on exact name.
