...
`com.work.jocastacouncil` is a KeepAlive LaunchAgent on the satellite. Every six hours it asks Exa for gaming and spatial-computing funding news, sends each headline to the local 27B through proxyd, and if the score clears seventy it writes a note to Affinity. She is not the [Jocasta MCP](/architecture/jocasta-mcp/) that reads local SQLite. This job hunts new names.
...
launchctl print gui/$(id -u)/com.work.jocastacouncil
