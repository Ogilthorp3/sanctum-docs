...
- A heartbeat was forced 100 hours stale. The hourly Action ran, went **red**, and opened the issue. The email arrived.
- A real `beat` refreshed the heartbeat. The next run went **green** and auto-closed the issue.
...
A backup that goes quiet now has exactly one place left to hide, and it is a place the haus does not control. That was the entire point.
