...
For eleven days the cathedral gave the same answer to every picture: `HTTP 503 vision_pipeline_not_yet_wired`. The text path had been live since 2026-04-22 — Qwen 3.6 35B-A3B on `:1337` mTLS, answering prompts, holding the council up — but any `image_url` block hit that wall. Multimodal was scaffolded and gated, waiting on the wiring underneath.

Today the gate flipped to 200. The first production multimodal probe came back coherent, the text path didn't move, and the council stayed up the whole time.
...
The cathedral can see now. The first thing it ever looked at was a single pixel — and it answered while the council on the text path never felt the room change around it.
