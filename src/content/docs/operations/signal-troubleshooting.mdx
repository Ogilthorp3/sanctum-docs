...
The Signal stack uses three different services with overlapping names on overlapping ports, and the contention between signal-cli, the Docker REST wrappers, and Signal Desktop on the same account produces three recurring failure modes. This annex collects them so the main [Troubleshooting](/operations/troubleshooting/) page can stay short.
