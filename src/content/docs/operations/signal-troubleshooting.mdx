...
You `curl` a port with "signal" in its name, ask it something reasonable, and it answers `404 Not Found`. The daemon is not down. You reached one of three services, and you spoke the wrong dialect to it.

That collision is not a bug you introduced — it is an inheritance. The signal-cli project never expected anyone to run its CLI beside Signal's Electron app beside a Docker wrapper of its own daemon, three services wearing the word "signal" on three ports speaking three protocols, all fighting over the same account. Three failure modes fall out of that arrangement, and they recur. When Yoda goes quiet on Signal, it is almost always one of these three. This annex collects them so the main [Troubleshooting](/operations/troubleshooting/) page can stay short.
...
None of the three is exotic. They are the standing tax on routing one phone number through a CLI, an Electron app, and a Docker daemon that were never introduced to each other. Learn the three — the 404 that isn't a failure, the stale container reference, the WebSocket that only seats one — and Signal goes quiet only when you mean it to.
