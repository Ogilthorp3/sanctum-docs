...
![A network box broadcasting concentric soundwaves into a dark void while an old brass key dangles disconnected just outside the glow, no longer needed — the box speaks, so the key retires](./images/hero-2026-06-08-the-ssh-key-the-box-no-longer-needs.png)

The box moved. That was all it took. For months the [screen-time enforcement](/operations/screen-time-enforcement/) stack had called itself model-agnostic, and it half was: the bridge finds the box by mDNS, the policy API works on Purple and Gold alike, nothing in the code says `goldpro` out loud. Then the swap put a Gold Pro where the old box used to sit — `mode=router`, `model=goldpro`, firmware `v1.982`. The agnostic stack immediately reached for three things the new box had never heard of. A per-box SSH key it was never told to trust. A hardcoded `pi@192.168.1.1` from a subnet that had since migrated. And a set of protocol assumptions that each failed by returning something shaped like success. "Model-agnostic" had quietly meant "works on any box, as long as it is *this* box, at *this* address, with *this* key on it." The swap was the test the claim had never been made to pass.

## The trio that lied as success

Three bugs sat under the SSH dependency. They survived for the reason they were dangerous: none of them threw. Each returned a value the caller read as fine.
...
The realization that retires the SSH key is small, and a little embarrassing. The box already speaks a full protocol over encipher. Almost everything the SSH paths were reaching for is already in it.
...
<Aside type="note">
The box already speaks a protocol; SSH was us refusing to learn it. Every limb in this stack existed to ask the box something it was already announcing — its model, its mode, its firmware, whether a policy was truly live — or to clean up after a bug that returned success without doing the work. Learn the protocol and the limbs fall off: `FWInitMessage` for identity, `host.policy.monitor` for spoof, and the load-bearing one, `activatedTime` for "did it actually apply." That last field is the whole difference between trusting the ACK and trusting the act. Honest zero-SSH is not the absence of any exception. It is naming your one exception out loud — because the exception you hide is the one that breaks the next time the box moves.
</Aside>
