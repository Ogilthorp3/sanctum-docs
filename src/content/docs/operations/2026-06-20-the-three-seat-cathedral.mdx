...
description: >-
  One Rust binary, one resident MoE crown jewel, three seats. Devstral became
  the all-Rust Qui-Gon code seat — a new mistral3 loader whose long-context NaN
  fell to chunked-prefill and an attn-scale dtype cast, parity-green against
  Python. Cilghal got a refusal-ablated twin on :6669 — one model, two ports,
  ~zero extra memory — after a "moot→works" reversal traced to a mis-aimed r̂.
  And everything consolidated onto one main behind post-quantum mTLS, the
  redundant second 35B retired, ~20 GB handed back.
---

import { Aside } from '@astrojs/starlight/components';

![Three thrones, three doors, one resident crown jewel feeding them all — a code seat, a healer's seat, and the council. Tommy presides.](./images/hero-the-three-seat-cathedral.png)

The Temple has run one resident crown jewel for a while now: Qwen3.6-35B-A3B,
the 4-bit MoE that serves the council on `:1337` behind mTLS. Today's overhaul
did not add a second crown. It added two *seats* — a code seat and a healer's
seat — and arranged for both to draw from infrastructure the haus already pays
for. One Rust binary. One resident model where it could be. Three doors.

## Seat one — Devstral, the all-Rust Qui-Gon

Qui-Gon's code seat used to lean on a heterogeneous patchwork; today it became a
first-class native citizen. **Devstral-Small-2-24B**, 4-bit, now serves on
...
arithmetic that's fine in miniature. Two fixes, both surgical:
...
trial-division, finishes clean, and the NaN is gone where it used to start.
...
where a reflexive refusal is the failure mode, not the safety. That seat needs
a model with its **refusal direction ablated** — the single direction in
activation space that, when a model decides to decline, it moves along. Suppress
that direction and the model stops reflexively refusing while keeping everything
else it knows.
...
a mis-aimed `r̂` points at a direction the *served* model never actually travels,
so projecting it out changes nothing. The reversal came from re-extracting on
the **served 4-bit checkpoint through the production prompt path** — measuring
the direction the deployed model really uses, not the one a different code path
implied.
...
ablation forced on** — one resident 4-bit checkpoint, two ports, served from one
process. Both `/health` endpoints return byte-identical bodies because they are
literally the same model; the ablated seat costs essentially zero additional
resident memory. One model, two doors, two postures.
...
proxyd's post-quantum hop, the ablation, the `:6669` seat. No concurrent feature
branches, no "deployed but uncommitted" limbo — the thing running is the thing
in `main`.
