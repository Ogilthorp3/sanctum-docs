...
description: "Same prompt, warm cache, different answer. Two standalone binaries proved the cache plumbing was byte-perfect at all forty-eight layers, so the bug had to be downstream — in MLX's SDPA kernel, which behaves differently at T_q=1 when the KV state came from a previous request. The fix caps cached-prefix reuse at length minus two, so the suffix always feeds the kernel at least two query tokens. Live on the 14B coder. Cache hits 77–87 percent."
---
...
Same prompt. Warm cache. Different answer. The prompt cache had shipped on a feature branch two evenings earlier, and it was quietly lying — not every turn, but reliably enough that the [cathedral's canary](/architecture/smart-router-cathedral/) drift probe caught it before any operator did. We pushed the branch and left it in the freezer overnight, on the theory that a cleaner head finds the bug in the morning. A cleaner head did. The bug was not where any of us were looking.
