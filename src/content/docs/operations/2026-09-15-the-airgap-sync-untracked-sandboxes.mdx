...
1. **Pull (Step 1)**: The Mac Mini fetches `origin/main` from GitHub in `~/manoir-nepveu`, packages it as a git bundle, SCPs it across the vmnet bridge, and updates the VM's `refs/remotes/origin/main`.
...
1. **Purged Ephemeral Paths from Index**:
   Ran `git rm -r --cached` on `sandboxes/`, `workspace-attestations/`, `sandbox/containers/`, `devices/pending.json`, and `state/openclaw.sqlite*` across both `~/manoir-nepveu` and `~/.openclaw` without altering the live database or disk state.
2. **Comprehensive `.gitignore` Hardening**:
   Updated `.gitignore` in `manoir-nepveu` to permanently ignore:
