...
![A quiet librarian-style controller reads last night's results.tsv at a wooden desk and marks which autoresearch ladder rungs to skip, while the picker robot beside it keeps climbing the ladder in order, unaware anyone is watching](./images/hero-2026-05-23-the-controller-quiet.png)
...
The ladder — [the one that heals itself](/operations/2026-05-10-the-ladder-that-heals-itself/) — is twelve rungs now. Two of them — `recipe-real-only` and `recipe-replay-boost` — vary the *training data* instead of the hyperparameters (Stage 1, yesterday). The picker still walks in order. It does not know that one rung's recipe scored 0.860 last night and another's scored 0.612 the night before. It just walks.
...
Both are deliberate. Neither requires a code change, a plist edit, or a deploy: you `touch` a file to hand the controller the wheel, and you `rm` it to take the wheel back. The controller reads the sentinel state once at the top of its run; toggling the file mid-night cannot flip the decision halfway through.
...
UCB and Thompson sampling are designed-for, not built — [the night the bandit picks first](/operations/2026-05-24-the-bandit-picks-first/) is when that swap leaves the drawing board. When the recipe library grows enough that sample-size confidence matters, the swap is a single subclass.
