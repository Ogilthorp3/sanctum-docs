...
The lesson we keep relearning is small and cruel: a system that heals itself is only as sound as its map of itself. Lose the map and every reflex still fires — it just fires at nothing, politely, for thirteen days, in a language only the log files speak. The manifests are the map made durable, and the doctrine that grew out of them is written up in the [Living Force](/architecture/living-force/) architecture, with the incident receipts in the [case files annex](/operations/living-force-annex/). The rest of us are still catching up. Now, at least, the immune system remembers.
