...
## Swarm Macro Analytics

A decentralized mesh needs collective visibility without surrendering peer privacy. When a node registers or syncs, it emits an anonymized macro pulse:
* **Country of origin**: Two-letter ISO code derived from the host system locale (e.g. `CA`, `US`, `FR`) without exposing physical location or IP.
* **Compute fleet profile**: Apple Silicon family (`M4 Pro`, `M3 Max`) and RAM tier (`64GB`, `32GB`) so the swarm understands aggregate model capacity.
* **Autonomy & Privacy index**: The local vs. cloud inference ratio (% offline) and the local eval baseline.

Any peer runs `sanctum mesh analytics` to read the aggregated swarm state: active node count by nation, fleet compute power, mean offline autonomy, and the catalog's peak benchmark score.
