...
2. **Signature.** The manifest must be signed by its producer's mesh identity.
   ...
   signature the rest of the Sanctum PKI is migrating to, and it drops in behind
   the same three-method interface the day the binding is pinned.
3. **Eval.** The champion must not *regress* yours. The peer's self-reported
   scores are decoration — your haus re-runs its own tiered eval on the actual
   adapter and compares to your baseline. A champion that loses to the one you
   already have is politely declined.
...
- the **sandbox probe** is a VM-pending stub on the happy path, because the real
  `unshare -n` egress check wants the air-gapped VM up.
