...
![mTLS migration — six couriers at the fortress gate, five with glowing wax-sealed scrolls clearing the automated reader, the sixth still empty-handed behind a velvet rope; a shadow-fortress flickers in the distance.](./images/hero-2026-04-21-mtls-migration.png)

Windu asked for the boring thing: copy the cert-detection recipe from the two probes that had it onto the three that didn't. Nothing heroic. Boring is the right shape for security work — boring means the pattern is composing rather than surprising. The morning after the [A+ roadmap push](/operations/2026-04-20-aplus-roadmap-closes/), that is exactly what we did.
...
Five of six probe clients are on mTLS now. The holdout is `sanctum-server`'s `HttpProxyBackend` — the [Rust router](/architecture/council-router/) — which is a real refactor (reqwest client with rustls config, per-backend cert paths read from `instance.yaml`), not a bash one-liner. Moved to its own roadmap slot for a dedicated session. No reason to rush it.
...
## Late Night — Why the Config Waited
...
With the Mini running someone else's turboquant work, steps 2–4 of the sanctum-server mTLS rollout didn't have a `:1338` listener to aim at. So we moved the rollout to the MBP — which already has the shadow on `:8902` (plain+bearer) and `:8903` (mTLS) from the earlier P7.4 work — and exercised the full [primary/fallback matrix](/architecture/fallback-continuity/) against a disposable sanctum-server instance on `:19900`.
...
Five of six probes speak mTLS today. The sixth is a one-line config flip waiting on a `:1338` listener to come home to the signed LaunchAgent — the router's code is proven on the shadow and idle on the Mini, which is exactly the shape we wanted. A migration that never had to become a rollout is the best kind of migration. Windu will notice the bearer curve the day it finally touches zero. No one else will notice anything at all, and for a security change, that silence is the highest praise there is.
