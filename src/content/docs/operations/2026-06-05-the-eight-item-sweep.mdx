...
| 2 | `com.sanctum.holocron-tunnel` (SSH crash-loop) | `ProgramArguments` ended in bare hostname `manoir`; launchd's minimal env has no `~/.ssh/config` short-name resolution → `Could not resolve hostname manoir` | Replaced with FQDN `manoir.tailnet.ts.net`. SSH tunnel forwards `:2187`/`:2189`/`:3344` to Mini |
