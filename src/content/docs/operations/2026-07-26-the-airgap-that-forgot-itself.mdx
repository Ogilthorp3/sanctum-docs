...
This is the [regression doctrine](/operations/2026-07-03-verify-before-you-fortify/) doing its job. When something that worked stops working — or in this case, when something that was *true* stops being true — the answer is in what changed, not in the physics. The break was in the diff: a hypervisor swap.
...
The ruling that followed: seats should declare a capability, fallback should only traverse the same capability class, and a cross-class request should fail closed with an error that cannot be mistaken for a result. That work belongs in [proxyd's fallback path](/architecture/fallback-continuity/), which was under active surgery by another session at the time — so it was handed over rather than duplicated. Two sessions editing one fallback loop is its own kind of incident.
...
The uncomfortable part is the eight days. Not that the airgap broke — platforms change, that's their job — but that a security property everyone believed in had no one checking it. Tommy the Abyssinian has patrolled these grounds at dawn and at dusk for fifteen years, unasked, never missing a day. The airgap had no such habit. For eight days it guarded nothing and told no one.
