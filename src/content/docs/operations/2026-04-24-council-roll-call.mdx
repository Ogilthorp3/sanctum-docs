...
The previous probe discipline had the council's MLX inference server — the 27B / 35B Qwen on `:1337` — covered: guardian, canary, drift-check, off-box watcher. That service got a [full liveness regime](/operations/2026-04-15-living-force-deployment/) after the [sixteen-hour silence on April 19th](/operations/2026-04-19-council-mlx-silence/), when a port that answered `200 OK` was quietly serving nothing. But the *other* inference target — LM Studio on `:1234`, serving Qwen2.5-Coder-14B behind every agent's system prompt — had exactly zero probes. The port was listed in `instance.yaml`. The watchdog checked that the port was open. That was the entire assertion.

An open port is not a working brain. We had learned that lesson once already, on the reasoner. Today we learned it wouldn't stay learned unless we spent it on the coder too.
...
The new steps live inside `run_check()` as Step 2.1, right after the service-graph check and before any remediation. The [Living Force](/architecture/living-force/) results are folded into the same `services` vec that the existing pipeline reads:
