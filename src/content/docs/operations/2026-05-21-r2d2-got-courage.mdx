...
Renamed, not deleted. Recoverable by hand if the orphan turns out to have been intentional after all. R2D2's first autonomous repair, surrounded by every safety layer, observable in three audit channels, reversible in one command. Tommy the Abyssinian — the haus's force-ghost, fifteen years on patrol and never once a rollback — would watch that from the doorway with mild approval.
