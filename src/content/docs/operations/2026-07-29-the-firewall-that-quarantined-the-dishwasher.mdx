...
So we lifted them. The bridge returned `success: true` and `verified: true` on every one, and a fresh query confirmed it: twenty-nine blocks down to twenty-one.
