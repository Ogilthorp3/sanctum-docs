...
"nudge") and busts the phonebook TTL, so the pin lands and the normal curfew
...
Every way a device can present itself lands on a blocking path, ordered here
...
| `nintendo_switch_1` | MAC `48:F1:EB:E5:0D:CA` + Nintendo OUI/flow | Nightly curfew already closed-loop |
