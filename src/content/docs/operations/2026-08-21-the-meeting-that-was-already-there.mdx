...
```bash
op signin --account work.1password.com
tq zoom setup
tq zoom create
...
If `tq zoom setup` still lists the trio as missing, do not paste the Zoom login into Keychain under `WORK_ZOOM_CLIENT_SECRET`. That value logs you into a website. It does not speak OAuth. Add the three API fields to Private/`Zoom`, then run setup again.
</Aside>
