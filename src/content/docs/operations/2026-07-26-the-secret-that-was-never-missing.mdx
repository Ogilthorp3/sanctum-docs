...
**SharePoint was already on disk.** While tracing the bridge, the documented filing destination turned out to be a local path — the OneDrive shared-libraries mount carries `02_Triptyq Capital I SEC/02_Financial/` verbatim. A pipeline that was assumed to need a Graph client may need a file copy. Worth checking before building the API integration nobody asked for.
