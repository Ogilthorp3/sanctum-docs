...
**`screen_time.py` is a library.** We treated it as a daemon for two days before reading line 551. The architectural reality has consequences for every probe, every log path, every plist; the doctrine entry now says so out loud. The probe correction in [`sanctum-family-pass 19c5f6e`](https://github.com/Ogilthorp3/sanctum-family-pass/commit/19c5f6e) is the durable fix. Two log paths, either-passes-means-healthy, no operator-facing config required.
