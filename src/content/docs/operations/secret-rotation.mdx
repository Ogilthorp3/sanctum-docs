...
At 06:00, before anyone in the haus is awake, a scan runs. It greps every git remote and every tracked file for the telltale shapes of a live secret — `ghp_…`, `sk-or-…`, `AKIA…` — and most mornings it prints one line and exits. A credential that has been alive too long is a liability that compounds in silence. So Sanctum rotates on a schedule, and makes the schedule cheap enough that it actually happens.

Every credential here has one of three rotation stories: automatic via a management API, half-automatic through a browser shim, or fully manual in the provider's web UI. One tool covers all three — the rotator, `~/.sanctum/scripts/secret-rotator/rotate.py`. Whichever path a secret lives on, it handles atomic propagation, verification, and the JSONL audit log. The human touches as little as the provider permits.
...
| 06:15 ET | `com.sanctum.secret-age-check` | `rotate.py check-ages --notify` — reads the audit log, posts a [Force Flow](/architecture/force-flow/) `warn` on anything past `rotation_max_age_days`. Never rotates. |
...
Each entry in `~/.sanctum/scripts/secret-rotator/providers.yaml` declares one `auto_rotate.kind`:
...
Those three consumer types — keychain, VM JSON, SOPS — are the same [secrets trifecta](/architecture/secrets-trifecta/) Sanctum keeps in sync everywhere else; rotation just writes a new value into all of them at once. Each consumer writes through a structured dot-path walk that refuses to traverse non-dicts. No `exec`, no shell interpolation, no string templating into shell commands. SOPS edits go through `sops set` so the existing recipient list is preserved; JSON files are rewritten atomically with a timestamped `.bak-<ts>` next to them.
...
python3 ~/.sanctum/scripts/secret-rotator/rotate.py list
python3 ~/.sanctum/scripts/secret-rotator/rotate.py audit | tail -20

# One specific secret, interactive
python3 ~/.sanctum/scripts/secret-rotator/rotate.py rotate <name>

# Batch every secret in providers.yaml, auto-rotatable first
python3 ~/.sanctum/scripts/secret-rotator/rotate.py rotate all --from-clipboard
...
Roll back, confirm the consumer reads the old value, and let the next 06:00 scan tell you the haus is clean again. Most mornings it already is, which is the whole ambition of this subsystem: the best rotation is the one you never had to think about, humming along while everyone in the haus is still asleep.
