...
<CardGrid>
<Card title="Rust Rewrite for Qui-Gon Triage" icon="seti:rust">
The current system relies on Python scripts (`qui_gon_triage.py`, `qui_gon_idle_manager.py`, `qui_gon_docker_sleeper.py`) to manage memory thresholds, swap limits, and idle state unloading. These scripts are critical to system survival but violate the "Rust for long-running services" doctrine.

**Deliverables:** A unified, memory-safe native Rust daemon (`sanctum-triage`) that replaces the Python triage suite, eliminating interpreter overhead and providing deterministic OOM prevention.
</Card>
...
<CardGrid>
<Card title="Rust Rewrite for XTTS Server" icon="seti:rust">
The Python `xtts-server` relies on PyTorch and has required careful memory management to prevent leaks. A pure Rust implementation could eliminate Python overhead, improve performance, and guarantee memory safety for long-running voice agents (like Yoda).

**Why later:** The Python version is currently stable after the recent `torch.no_grad()` memory leak patch, but Rust remains the ultimate target for all core Sanctum infrastructure.
</Card>
