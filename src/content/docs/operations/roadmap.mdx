...
**Memory cost:** ~12 GB additional (Python holds the 9B model warm). Mini swap rose 7.9 → ~20 GB after the always-loaded fallback. Acceptable for now; if it becomes a problem the next move is bounding MLX's buffer cache on the Python side or running it on a cron-restart.
