...
<Card title="Qwen3-TTS — Local Yoda Voice" icon="signal">
Replace XTTS-v2 with Qwen3-TTS via mlx-audio for Yoda's voice agent. XTTS-v2 on MPS is a dead end (Coqui marked it "wontfix"). Qwen3-TTS runs natively on MLX with zero-shot voice cloning from reference audio.

**Status:** mlx-audio installed, 3 model variants downloaded, LiveKit plugin written (`qwen3_tts.py`). Blocked on inference speed — 10s per sentence on 1.7B-bf16. Need either mlx-audio streaming optimization, quantized model improvements, or GPU scheduling (LM Studio contention).

**Why later:** XTTS works as a stopgap. The quality is poor but Yoda answers. This becomes urgent when voice quality is a demo requirement.

**Deliverables:** mlx_audio.server LaunchAgent, Qwen3 TTS plugin in production, sub-2s latency, Yoda voice clone quality parity.
</Card>
...
Replace all cloud dependencies in the voice agent: Deepgram STT → local Whisper via mlx-audio, Claude LLM → local Qwen3.5-27B via sanctum-mlx. Combined with Qwen3-TTS, this yields a zero-cloud voice pipeline. Phone calls to Yoda with no data leaving the haus.

**Why later:** each component needs to be fast enough individually first. Qwen3-TTS speed is the current bottleneck.
</Card>

<Card title="Restore (or formally retire) the council fallback" icon="warning">
The 2026-04-25 failover drill found that `council-guardian.sh::activate_fallback()` is wired to bootstrap a Python `mlx_lm.server` (`com.sanctum.server-mlx`) that has been non-viable since the 2026-04-21 mTLS migration: plist renamed to `.disabled-20260422`, wrapper config points at the retired Qwen3.5-27B path, and Python `mlx_lm.server` only serves plain HTTP while every council client is mTLS-only. A defensive guard now refuses to "activate" a fallback whose plist isn't on disk, so the worst-case (silent council dismantle on a metallib panic) is contained — but the system has no working failover layer.

**Three options, in order of effort:**

- **Restore** — put a TLS-terminating Caddy or tiny Rust shim at `:1337` mTLS that forwards plain HTTP to a Python `mlx_lm.server` on `:1338`. Update `instance.yaml::services.council_mlx.model_path` to the current MoE snapshot. Re-enable the plist. ~1-2 hours including cert + plist wiring. Most faithful restoration.
- **Consolidate** — fold the failover decision into sanctum-server (it already terminates mTLS for inbound traffic). Backend health → route to either sanctum-mlx loopback-plain or Python loopback-plain. Bigger change but it puts the failover in the Smart Router cathedral where it arguably belongs.
- **Retire** — delete `activate_fallback()` and `detect_fatal_pattern` from the guardian. KeepAlive + ThrottleInterval already handle "process crashed"; the auto-fallback was for "process is online but inference is broken," which has not been the failure mode in 2026.

**Why later:** the defensive guard makes the current state safe (loud refusal vs silent dismantle). The actual restoration is a real engineering project, and which option is right depends on whether you trust sanctum-mlx's stability after the 2026-04-24 memory + kernel fixes (overnight 04-24/25 ran 17h59m / 440 requests / 0 errors — encouraging but not 30 days of evidence yet).
