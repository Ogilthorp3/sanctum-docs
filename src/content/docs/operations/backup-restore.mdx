...
description: Automated daily backups, offsite copies, and disaster recovery — the layers that turn a 3 AM disk noise from a catastrophe into an inconvenience.
---

import { Aside } from '@astrojs/starlight/components';

![The stages of backup grief, pencil-sketched — denial, anger, bargaining, testing, acceptance](./images/backup-stages-of-grief.png)

If you're reading this calmly, on a Tuesday afternoon, with a cup of coffee — good. You're doing this right.

If you're reading this at 3 AM because a disk just made a sound disks should never make, we're sorry. But this is exactly why the page exists, and past-you was smart enough to set up backups. Unless past-you didn't, in which case present-you has our deepest condolences.
...
Every byte that hits T9 or leaves your machine for Google Drive is encrypted client-side with AES-256, using a passphrase from macOS Keychain (`sanctum-backup-key`). Walk off with T9 or compromise the Drive folder, and all you get is opaque ciphertext blobs.
...
<Aside type="note">
Earlier iterations used a custom GFS function in bash that ran weekly via a separate LaunchAgent (`com.work.backup-retention`). It worked, but split logic between two scripts and used a different host-naming convention. Both have been retired in favor of restic's built-in policy + a single canonical job.
</Aside>

## Why we moved off iCloud full-copy snapshots

This layer used to be simpler, and simpler was the problem. Until 2026-04-26 the daily backup was an `rsync` of `~/.sanctum` into a new dated directory under `iCloud/Backups/daily/YYYY-MM-DD/`. Each snapshot was a full copy. iCloud Drive doesn't understand hardlinks, so even a `--link-dest`-based incremental wouldn't have worked through the Files-app sync layer. The result, observed empirically:
...
The scripts above are the front door; this is the manual set of keys for when you want one file, not the whole house.
...
The 2026-04-26 incident — internal SSD at 95% capacity from full-copy iCloud snapshots — exposed a missing layer: there was no early signal. The `/status` endpoint of the Navigator Sidecar (`navigator-sidecar.js`, port 3344) now emits a `disk` block, and it surfaces on the [Holocron dashboard](/architecture/holocron-gui/) like every other health signal:
...
The daily backup lives on the Mini. If the Mini itself is compromised — disk failure, filesystem corruption, malicious actor — that backup is gone with it. The weekly snapshot is the second dimension, and it shares the paranoia lineage of the [backup deadman](/operations/backup-deadman/): pulled from manoir to the MBP every Saturday 04:30, SHA256-pinned, rotated to last 8.
...
- `~/.sanctum/secrets/` is **excluded** — secrets live in keychain / 1Password / SOPS (the [secrets trifecta](/architecture/secrets-trifecta/)), not this tarball.
...
Strip away the restic internals and the LaunchAgent plists and the whole architecture reduces to one question, asked at 3 AM when the disk makes the sound: is there a copy that isn't on that disk? T9 answers first. Google Drive answers if T9 is gone. The MBP answers if manoir itself is gone. Run the quarterly drill and the answer stays yes; skip it, and "I have a backup" quietly decays into "I had a backup" — the one sentence this entire page exists to keep you from ever saying out loud.
