...
<Aside type="note">
Earlier iterations used a custom GFS function in bash that ran weekly via a separate LaunchAgent (`vc.work.backup-retention`). It worked, but split logic between two scripts and used a different host-naming convention. Both have been retired in favor of restic's built-in policy + a single canonical job.
</Aside>
