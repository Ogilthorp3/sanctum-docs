...
4. **A declared upstream list** — so [Force Flow](/architecture/force-flow/) can suppress a storm of forty symptoms into one root-cause incident.
</Steps>
...
[Tommy the Abyssinian](/agents/tommy/) watches the gap from the corner of the hero image, and he would recognize the whole exercise. He never applied for his post either. He simply took the terrace, held it for fifteen years, and never once needed to be assigned — dawn patrol and dusk patrol, claimed, not imposed. The council spent a morning arriving at what the cat had known the entire time: the seats that hold are the ones somebody actually wanted.
