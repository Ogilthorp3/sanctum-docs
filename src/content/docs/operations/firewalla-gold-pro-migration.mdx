...
![Firewalla Gold Pro Migration — a technical pencil sketch of a purple router lifted out of a rack while a gold router slides into its slot, teal indicator lights flickering mid-swap.](./images/firewalla-goldpro-swap.png)

The Purple has guarded this haus for a year and a half. It paused kid phones. It blocked curfew-hour YouTube. It ran every screen-time rule we asked of it, without a reboot we didn't schedule ourselves. It also spent those months quietly dropping `flowsummary` bytes — which is why we built an ARP fallback and a [drift sentinel](/operations/drift-sentinel/) to compensate for a router that worked exactly well enough to lie. Tomorrow a Gold Pro arrives with ten times the CPU, four times the RAM, and — if the SDK cooperates — actual per-host flow data. This is the runbook for swapping it in without taking Albert's bedtime offline for longer than a reboot.
...
```bash
~/.sanctum/scripts/firewalla-probe/firewalla-sdk-probe.sh neo@100.0.0.25 \
  > ~/.sanctum/scripts/firewalla-probe/capabilities-purple-baseline.json
```

The baseline lives in the repo as `capabilities-purple-baseline.json`. It is the answer key. Tomorrow's Gold Pro run writes `capabilities-gold-pro.json`, and `diff` tells us which new connectors the [Firewalla bridge](/architecture/sanctum-firewalla/) can finally support.
...
'
~/.sanctum/scripts/firewalla-probe/firewalla-sdk-probe.sh neo@100.0.0.25  # should match the baseline
```

[Screen-time enforcement](/operations/screen-time-enforcement/), drift-sentinel, and activity tracking all keep working on the Purple. Nothing gets worse by trying.
