...
![A vault door etched with a crypto lattice and a fused classical-plus-quantum lock, guarding thirty years of data behind a key that lives only in someone's head — the way the haus's guardian spirit keeps what matters.](./images/hero-the-post-quantum-vault.png)

A patient adversary does not need to break your encryption. He needs to copy it,
and wait.

Most of the stack defends data **in motion** — mTLS on `:1337`, post-quantum key
exchange at the [proxy](/architecture/proxy/), WireGuard between machines. That
is the wire, and the wire is the easy part. The hard part is the **archive**.
Some data stays sensitive *forever*, and forever is long enough for that patient
adversary to harvest the ciphertext today and read it the morning a quantum
computer finally boots. Harvest-now, decrypt-later. A faster classical cipher
does not save you here. Only a vault whose confidentiality does not rest on a
problem a quantum computer makes easy.
...
algorithm identifiers and the recipient fingerprint. Belt and suspenders — and
the suspenders came back from the future to warn us.
...
the part nobody watches until the night it is the only thing that matters.
...
The seal was put through an end-to-end sweep. Real artifacts crossed real
boundaries. Not "the field exists" — the bytes match.
...
This is the [Contracts at the Boundary](/architecture/engineering-discipline/)
doctrine in miniature. Every one of these was a bug caught *before* it shipped,
because the test was written from the consumer's reality instead of the
producer's hope.
...
Two things the sweep found and did **not** paper over. A security page that only
...
The diceware words never touch a disk. They live in exactly one place: your
head. [Tommy](/agents/tommy/), the haus's guardian spirit, would understand the
shape of that — a master key that a stolen laptop cannot cough up, a burned drive
cannot melt, and thirty years cannot rot, because it was never anywhere a machine
could reach. He kept what mattered and forgot nothing. So does the vault.
