...
Triple-train fires only when a candidate's screen-tier score clears the legacy 0.881 threshold. Most nights have zero candidates and incur zero extra cost — Mundi never sees a bill for training that didn't need to run. On the rare night a candidate appears, two additional 50-minute trainings produce two additional adapters; the aggregator rolls them up into one `eval_results_aggregated.json` with mean + standard deviation per tier; the gate reads that file in preference to the single-seed `eval_results_full.json`.
...
A gate is only honest if it is willing to fail the champion that is already wearing the crown. This one is, and does. The cathedral runs tonight at 01:00; in the morning we find out whether it also knows how to say yes.
