...
## Automated Sunday Cadence

In addition to manual upgrades, the Mac Mini executes weekly automated Tier A upgrades every Sunday at 09:30 via `haus.sanctum.sunday-upgrade` (`~/.sanctum/bin/sanctum-sunday-upgrade.sh`):

- **OpenClaw**: Validates 48-hour soak time and zero open crash issues before upgrading Mini CLI, VM CLI, and VM gateway.
- **Homebrew**: Runs `brew update && brew upgrade` with post-upgrade checks for Tailscale skew, pinned dylib linkage, and 1Password CLI authorization.
- **Python & MLX Ecosystem**: Upgrades standalone `uv tool` CLI binaries (`mlx-lm`, `mlx-vlm`, `mlx-whisper`), refreshes `~/.cathedral-venv` and `~/Projects/mlx-finetune/.venv`, and validates import smoke tests.
- **Verification Sentinel**: `haus.sanctum.sunday-upgrade-check` triggers every Monday at 10:00 to verify upgrade logs and surface parity.
