...
3. Added a runtime renderer so `~/.sanctum/services/*.yaml` is generated from `instance.yaml` plus auxiliary launchd-wired services instead of remaining an empty placeholder
...
- `tests/test-sanctum-runtime-audit.sh` reports `14 passed, 0 failed`

That does not mean the runtime is portable. It means the live machine is internally consistent again, and the runtime manifest directory now contains actual graph inputs instead of a directory-shaped lie. The rendered set is now 32 manifests: 27 instance-backed services plus 5 auxiliary runtime services such as the proxy and SSH tunnels.
