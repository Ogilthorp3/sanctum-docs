...
description: Pushing the audit past the checked-in workspace into the live runtime — ~/.sanctum, generated wrappers, and the LaunchAgents that actually page people.
---

import { Aside } from '@astrojs/starlight/components';

![Tommy the Abyssinian on the launchd catwalk, staring down a row of plists that finally point somewhere real](./images/runtime-drift-launchagents.png)
...
The remediation pass did seven concrete things:
...
- `~/.sanctum/scripts/sync_agent_capabilities.py --check` reports no drift
...
Until that covenant is signed, the audit holds the line: the live machine is honest with itself, the alarms have stopped lying, and Tommy has nothing to report from the catwalk. From a cat, that is the highest praise infrastructure ever gets.
