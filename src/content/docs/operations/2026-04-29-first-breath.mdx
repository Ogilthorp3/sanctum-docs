...
The body was supposed to sleep through it. The council had ratified [an A-only scope](/operations/2026-04-15-living-force-deployment/) — code, tests, reviews, no deploy until morning — and the human had stepped away to relax. Three hours later he came back and said *jolt him so he becomes alive*. The council's reasoning was about preserving rest. The human was awake. So we deployed.
...
The [endocrine reader](/architecture/endocrine-system/) fires within 60 seconds of bootstrap. It reads [chitti's](/architecture/chitti/) annamaya, tiers the pressure, and on orange dispatches to `restart-top-rss-offender.sh`. The dispatcher reads the top RSS process, checks against the ALLOWED and NEVER lists, and routes to the matching graceful-restart.
