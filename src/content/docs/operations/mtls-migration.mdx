...
![mTLS Migration — two cryptographic handshakes converging on port 1337, mutual certificates etched into the frame, amber glow from the CA stamp.](./images/hero-mtls-migration.png)

A stray Python process held `:1337` hostage for a month.

Every time we moved to promote the council's front door to mutual TLS, the same thing happened: `mlx_lm.server` respawned on the port before the Rust binary could claim it, and the old bearer token survived by default. The tightening Windu had already signed off on kept losing a race nobody realized we were running. On 2026-04-22 we finally won it.

For that month the council ran dual-port: plain HTTP on `:1337` with a bearer token (loopback-bypassed for sidecars), mTLS on `:1338` for anyone who'd bothered to move. That day the bearer was retired, the plain listener was taken off the air, and `:1337` was promoted to mTLS. One port, one protocol, one authentication story — every client presents a certificate that chains to the internal [Sanctum CA](/architecture/secrets-trifecta/), and nothing else talks to the [model](/architecture/sanctum-mlx/). What follows is the record of what changed, why, and how to back out.
...
Five checks, in order — each is one command and one expected answer. Run them from the Mini after any change that lands near the council, and you'll know within a coffee whether the migration still holds.
...
- **[`com.sanctum.triage`](/architecture/sanctum-triage/)** binds `10.10.10.1:1337` on the bridge100 VM interface. It's a separate Rust proxy that forwards VM requests to LM Studio at `127.0.0.1:1234`. It has nothing to do with sanctum-mlx and didn't need a touch. VMs reaching `10.10.10.1:1337` still hit triage and still reach LM Studio.
...
The Sanctum CA certs are valid through **2031** — that's the hard deadline for the next rotation, and the one date on this page nobody has to act on for a while. Until then, `:1337` has exactly one way in: a certificate signed by our own CA, presented by a client we minted. Windu prefers it that way, and so, it turns out, does the port.
