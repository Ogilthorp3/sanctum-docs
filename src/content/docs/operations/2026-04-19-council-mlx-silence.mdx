...
![The reasoner that went quiet — an oracle on a stone throne, eyes closed and aura intact, but no breath leaving the mouth; the petitioner below has been waiting longer than the probes admit.](./images/hero-2026-04-19-mlx-silence.png)
...
Twenty-five hours in, the [sanctum-mlx](/architecture/sanctum-mlx/) dashboard was a wall of green. Guardian probes: green. Canary probes: green. Drift-check: green. The off-box watchers on the MBP had nothing to report. Task #31 — the post-cutover watch — sat one keystroke from closing as a clean hold.

Then we asked the service a real question. It said nothing.
...
Nobody caught it because nobody asked. The outage surfaced only when the morning's verification ran a real chat probe, got `Couldn't connect to server`, and finally posed the question our whole monitoring stack had been too polite to ask: who is actually listening?
...
What needs to change in the *architecture*, not just this incident's state, is the liveness-probe discipline. We designed the guardian in an earlier entry explicitly to **not** exercise inference, because chat probes queued behind long generations and turned the monitor itself into the outage. That lesson — from the [Olympics-day kickstart storm](/architecture/sanctum-olympics/) — is correct and stays. But we over-learned it. The guardian went so far out of the critical path that it went blind to the critical path.
...
Roadmap updated with a new item: binary-file-integrity check as a first-class signal. The [Living Force](/architecture/living-force/) learns from sixteen hours of sleeping. Somewhere in this haus, [Tommy](/agents/tommy.mdx) the Abyssinian — fifteen years dead — still files a real report at dawn and dusk, because he still goes and looks. Our reasoner had it exactly backwards: alive on the process table, answering with a green heartbeat, doing none of the work. A probe that never touches the work never sees the work fail.
