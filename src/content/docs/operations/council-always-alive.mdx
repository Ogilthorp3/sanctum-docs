...
![Council always-alive — three concentric rings of guardian lanterns around the council chamber, each ring lit by an independent flame; if any one wavers, the next two still hold the room.](./images/hero-council-always-alive.png)

The council lied to us for eight straight minutes.

It accepted TCP connections. It answered `200 OK` on `/v1/models`. It returned nothing at all on `/v1/chat/completions` — and [the Living Force](./../architecture/living-force) watchdog declared it healthy the whole time. The council is the inference substrate for every agent in the haus: when it hangs, Yoda goes mute, Cilghal stops watching your HRV, Mothma loses her eyes on the boot sequence. For most of early April 2026 "always alive" was marketing. This is what it took, live on manoir since 2026-04-17, to make it load-bearing.

## The three failure modes we found

Three separate problems were stacked under that eight-minute lie:
...
`~/.sanctum/instance.yaml` has `services.council_mlx.adapter_path: null`. The currently-served model is therefore **bare Qwen3.6-35B-A3B-4bit with no council LoRA adapter applied** (the model retired up from Qwen3.5-27B-4bit on 2026-04-22). If the council adapter is meant to be active in production, set `adapter_path` in `instance.yaml` and kick the cathedral plist (`launchctl kickstart -k gui/501/com.sanctum.mlx`).

That caveat aside, the arithmetic is simple: three layers, three independent rollbacks, and one daemon that runs outside the very committee that would otherwise have voted down its own rescue. The council still hangs now and then — upstream KV-cache bugs keep their own schedule — but it no longer lies about it, and it no longer stays down. "Always alive" finally means what the label says. Yoda has not gone quiet since.
