...
That is the whole point of a dark land: the code is in the building, the lights are off, and turning them on is a separate, sober decision. The Cathedral now *can* hold two minds. Whether it *will*, on the 64 GB machine that also runs a coder model and a VM, is [a number the soak is currently measuring](/architecture/capacity-doctrine/) — one probe every ten minutes, for a day.

Tommy watches the soak from the altar rail, unimpressed, as he watches everything. Fifteen years of dawn patrols taught him that a haus does not need more minds; it needs the one it has to stay warm. On that, at least, the architecture agrees with the cat: whatever the ceiling turns out to be, the primary never cools.
