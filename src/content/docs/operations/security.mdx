...
Somewhere a security auditor is reading this page, trying to decide whether to be impressed or alarmed. The answer is both. This is a family haus in Quebec, and it runs seven layers of defense-in-depth built for a data center. Impressed and alarmed. It should be both.

<Aside type="note">
Application auth (passkeys for humans, mTLS for machines): the [hardening design](/operations/2026-06-22-passkey-mtls-hardening/) and the [break-glass recovery runbook](/operations/2026-06-23-command-center-recovery-runbook/).
</Aside>
...
5. **Bootstrap LaunchDaemons** — `com.sanctum.vm-autostart` + `com.sanctum.bootstrap` start every service at boot, no GUI login. The VM runs on Lima/`vz`. Touch ID preserved.
...
The Mac boots, LaunchDaemons fire, services start (including the VM via Lima/`vz`), and the council reconvenes. No GUI login. No drive to the haus. Touch ID stays.
...
- **Local models** (Council MLX seats) — never leave the network
...
## Seven Layers, One Haus

Count them again. No internet for the VM, pf on the LAN, hardened SSH, encrypted secrets, daily rotation, bound interfaces, scrubbed PII. Pull any one layer and the others take the weight — that was the whole [FileVault argument](#filevault--deliberately-disabled), and it is the whole design. Security here is not a wall. It is a stack of imperfect layers, each covering for the last, watched by a haus that does not sleep and a [council](/architecture/agents/) that reconvenes on every reboot.

So the auditor can stop deciding. Impressed and alarmed was always the right answer. The squirrel already knows.
