...
Blocked ports include: gateway (19001), dashboard dev (3000, 3004), XTTS (8008), MLX (1337), Firewalla bridge (1984), and others.
...
The guardrail injector proxy (port 4040) sits in front of LiteLLM (port 4001). When a request targets an OpenRouter model — either directly or via fallback routing — the proxy:
...
```
Client → Guardrail Injector (port 4040) → LiteLLM (port 4001) → Provider
              |                                                      |
