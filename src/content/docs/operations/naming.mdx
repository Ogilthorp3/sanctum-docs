...
Steve Jobs walks the length of a Sanctum whiteboard. He reads the names off it, one at a time: *Yoda. Cilghal. Holocron. Sanctum. Manoir. Cathedral. Living Force. DenchClaw. OpenClaw. Force Flow. Bunker. Haus.* Then he turns around and asks which mythology this is. The honest answer used to be "all of them." This page is the honest answer now.
...
## Hive & Tailscale naming (multi-site)

The multi-site nervous system has its own one-name rule. Full contract: [Hive Topology](/architecture/hive-topology/). Cheat sheet: `sanctum node names`.

**Jobs test:** *What do you call it when you talk to it?* Roster key · `.node_id` · MagicDNS stem are **the same string**.

| Kind | Pattern | Yes | No |
|------|---------|-----|-----|
| Site brain | `{place}` | `manoir`, `chalet` | `manoir-mini`, `MM64` |
| Mobile | `{device}` | `mbp` | `berts-mbp`, `MBP128` |
| Site infra | `{place}-{fw\|ha\|orbi}` | `manoir-fw`, `manoir-ha` | `ts-firewalla`, `homeassistant` alone for multi-site |
| Agent | Jedi name | `yoda`, `ahsoka` | Using agent names as Tailscale hosts |

Live MagicDNS stems match the hive names above. Legacy stems may exist only as **recovery matches** in `hive-names.yaml` (re-converge if a device reappears under an old Name) — not as live dual addresses. Place names stay on the **place** layer of this page; they are also the hive addresses on purpose — one string, two layers only when the place *is* the machine.

## VM and host naming

VMs and service hosts run *inside* a physical location (they live on `manoir`), so they take neither a place name nor the "Sanctum" software prefix. Where each one actually runs is drawn in the [node topology](../architecture/node-topology/); this section is only about what we're allowed to call them. Two rules and one prohibition:

- **An agent's VM takes the agent's name.** The production conversation host is `yoda` (`10.10.10.10`) — the roster above already assigns it. Not a product label, not a lifecycle word.
- **A service host takes the service's plain-English name.** The Home Assistant host is `home-assistant` on the LAN / `manoir-ha` on the hive, not `sanctum-vm`. The rule is *no name does two jobs* — and a generic name that does *no* job fails the same test.
- **Never put a lifecycle word — `staging`, `test`, `tmp` — on a production VM.** A host labelled "staging" invites a tired operator to treat the live brain as disposable. If it is production, name it for what it is.
...
| **`sanctum-vm`** (the Home Assistant host) | Decided 2026-06-04 — rename or retire pending | `home-assistant` (LAN) / `manoir-ha` (hive) |
| **`berts-mbp`** (Tailscale machine name) | Renamed on control plane 2026-08-06 | `mbp` (recovery match only in `hive-names.yaml`) |
| **`manoir-firewalla` / `ts-firewalla` / `chalet-firewalla`** | Renamed on control plane 2026-08-06 | `manoir-fw`, `chalet-fw` |
| **`MM64` / `MM16` / `MBP128`** (council display codes) | Display-only if needed; never addresses | hive names `manoir` / `chalet` / `mbp` |
...
6. **The Council is seven.** Five Jedi + Jocasta + Mon Mothma — lucky seven, and the table is full. If a domain is unowned, give it to the closest existing seat before inventing an eighth.
