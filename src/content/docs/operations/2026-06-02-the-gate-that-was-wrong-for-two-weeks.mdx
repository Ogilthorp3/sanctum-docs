...
A nightly autoresearch run scores LoRA adapters against a hundred-and-nine evaluation cases, and a script called `promote_champion.sh` checks the best score against a static gate of `0.881`. Anything above the gate gets rsync'd to the Mini, the cathedral plist gets patched with a fresh `--adapter-path`, a four-case smoke probe fires, and a new champion is live before breakfast. Anything below the gate gets a `NO-OP` breadcrumb and the previous champion keeps its job. This has been the pipeline for weeks.

Tonight, while staring at the 0.69 overall scores coming out of a dropout dose-response sweep that was supposed to give us our first non-overfit champion, the obvious question landed for the first time: **where did `0.881` come from?**
...
The fix is small. Write a script that runs `carmack_eval.py --tier full --adapter-path <production-champion> --seed 42` against the current evaluation set, take whatever `overall` falls out, subtract a `0.005` promotion margin, floor at `0.70` to prevent a catastrophic eval-framework error from accidentally approving every adapter, and write the result back to `~/.sanctum/state/production-champion.json` as a `calibrated_gate` field. Then patch `promote_champion.sh` to read `calibrated_gate` if present, falling back to the legacy `GATE_SCORE` env var if not.
...
The infrastructure was green at six commits. The plan said `Task 5: real-run calibration on May-12 champion`. The natural next step was to run the script.

Three operational facts surfaced at that moment that the spec hadn't accounted for. `carmack_eval.py` lives only on the MBP. The production-champion adapter symlink resolves only on the Mini. The MBP GPU was at that moment fully loaded by tonight's autoresearch nightly, mid-eval on its own adapter. And `~/.sanctum/state/production-champion.json` didn't exist anywhere — the file the calibrator needs to read in order to write doesn't exist yet, which means tonight's run would have had to bootstrap it manually, which means tonight's run would have been racing whatever the nightly's Phase 3 promotion writes if it found a winner.
