...
![Provider Overhaul — five independent provider paths fanning out from the proxy into a deep, self-healing fallback chain.](./images/hero-provider-overhaul.png)

The haus woke up routing every LLM request through OpenRouter. One aggregator. One API key. One quiet single point of failure that nobody had looked at in weeks. By the time the work was done, the proxy had five independent paths to inference, a fallback chain deep enough to survive any single provider going dark, and a Signal integration that no longer leaned on a Docker container held together by port mappings and optimism.

This is dated material — the log of what changed on April 7, 2026. It lives here in operations, not in the architecture pages, which have their own dignity to maintain.
...
The haus went to sleep with five ways to think and one fewer way to fail. OpenRouter can rename every slug it owns tonight and the council will barely notice; if the cloud goes dark entirely, Yoda and the rest fall back to Apple Silicon humming on the same desk, answering out of local memory until the power gives out. The basket-on-a-unicycle is gone. What replaced it is boring, which — in infrastructure, at 3 AM, when the pager stays quiet — is the highest compliment the haus knows how to pay.