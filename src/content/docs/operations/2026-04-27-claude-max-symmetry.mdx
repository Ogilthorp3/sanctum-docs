...
Two Macs. One Claude Max subscription apiece. Until this morning they disagreed about how to spend it.

The [capacity doctrine](/architecture/capacity-doctrine/) that landed 2026-04-24 said *each host answers for its own air supply*. Three daemons enforce it: `sanctum-admit` for the RAM pool, `pressure-valve` for shedding offenders, and the Claude proxy for cloud-backend routing. The first two were already symmetric across the hub and the mobile node. The third was the holdout. Today it fell into line.
...
The mobile ran `com.sanctum.claude-max-proxy` on `:3456` — a long-lived HTTP server from the `claude-max-api-proxy` npm package, wrapped by `claude-max-api-tailscale.js` to bind the Tailscale IP. Persistent process, fast warm path, and the [smart-router cathedral's](/architecture/smart-router-cathedral/) `cloud` backend talked to it without complaint.
...
A new launchd plist `com.sanctum.claude-max-proxy` replaces `com.sanctum.claude-cli-proxy`. We keep the retired plist and its script at `~/Library/LaunchAgents/retired-2026-04-27/` and `~/.sanctum/scripts/retired-2026-04-27.claude-cli-proxy.js` — kept, not deleted, because forgotten work shouldn't disappear.
...
After the orchestrator reload, `launchctl print` confirms `YODA_MAX_URL => http://127.0.0.1:3456` — Yoda now points at the local listener by default. We asked the fresh proxy the only question that matters: a live `curl` to `:3456/v1/chat/completions` returned `Yes — alive and ready.`
...
The cathedral's `cloud` backend gains a hub-side fallback that didn't exist before: if the mobile is asleep or rebooting, the hub's proxy can serve its own traffic. The [node topology](/architecture/node-topology/) now lists this proxy among the services that stand on both hosts rather than leaning on one.
