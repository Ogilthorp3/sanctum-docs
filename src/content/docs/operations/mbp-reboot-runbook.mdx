...
The Mini's `:1337` is, as of this writing, served by a non-canonical `sanctum-mlx` invocation (a parallel session's Qwen3.6-35B build without TLS args). The shadow on this MBP is fully Dev-ID-signed, Apple-notarized, and mTLS-gated on `:8903` — but the production router on the Mini can't flip to mTLS until the Mini-side `sanctum-mlx` reconciles to the signed `com.sanctum.mlx` LaunchAgent. That's not a reboot problem; it's a topology problem. See the sanctum-mlx roadmap for the 10-second `instance.yaml` edit that closes it whenever the topology is ready.
