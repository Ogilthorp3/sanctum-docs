...
Albert had Crunchyroll on the TV and Fortnite on his phone at the same time. The old [screen-time stack](/architecture/screen-time/) could tell you he'd been on the network for four hours — never _what_ he'd been doing for four hours. That is half of a screen-time system. [Force Flow](/architecture/force-flow/) now has the other half, and its first job — because we are not building a generic SaaS, we are building something that has to notice the two-screens-at-once move Albert just pulled — is divided-attention detection.
...
synthetic observation inject; real data will start flowing when the
[Gold Pro arrives](/operations/firewalla-gold-pro-migration/), which is on a truck.
</Aside>
