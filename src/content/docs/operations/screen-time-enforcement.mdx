...
not-study (PC, Apple TVs, Nintendo Switch), mirror the same
`study_mode` block at `screens.<name>.schedule.study_mode` so
each fires at the same boundary. Apple TVs in legacy
