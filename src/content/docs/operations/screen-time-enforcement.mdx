...
The log reported five blocked devices. Netflix said otherwise. Both were telling the truth about different things — which is the worst class of bug there is: the kind that files a clean report while the thing it reports on quietly does nothing.

A screen-time system that says "blocked" but streams Netflix is worse than one that admits it's broken. For two nights in April, ours did exactly that — the [Force Flow](/architecture/force-flow/) daemon on manoir logged `BLOCKED` against seven MACs while five of them stayed happily online, and the only people who noticed were the parents, who were supposed to be the ones the curfew let sleep. This page is the autopsy of April 16–18, 2026, and the closed-loop contract that now governs every enforcement action. The fix went live on manoir on 2026-04-18, E2E-tested and regression-gated.
...
This is [Living Force](/architecture/living-force/) principle 12: *commands can't lie either*.
...
Albert's phone curfew is unaffected — the phone keeps its 23:00 weekend bedtime so he can read or play chess while falling asleep. Only the consoles get bumped.
...
`devices.yaml` — one more layer on the [screen-time
architecture](/architecture/screen-time/)'s stack of who-is-curfewed-when:
...
The whole affair cost two nights of quiet Netflix and one confident, wrong "RED tout est bloqué." What it bought is a rule the haus now applies to everything that writes to hardware: the acknowledgement is a claim, the re-read is the truth, and a curfew you cannot verify is one the kids have already beaten. The daemon no longer gets to tell the parents it did its job. It has to show them.

The same honesty rule applies to the notify path. When the bridge circuit opens CRITICAL and later closes itself, Signal must hear the restore — not only the dashboard. See [The Restore That Never Rang](/operations/2026-08-05-the-restore-that-never-rang/).
