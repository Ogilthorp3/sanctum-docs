...
## The one we didn't fix — and why that's the honest call
...
its fix is a **safety decision**, not a mechanical one.

`classify_posture` checks the Fever breaker (acute pressure ≥ 0.95) *before* the
"be quiet" override, so a quiet request at Fever-level pressure still returns
Fever. The test expects Conserving, and a comment one layer up says quiet "wins
regardless of what the lower koshas say." Those genuinely conflict — and the two
fixes are opposites: correct the test's input to the Alert band it clearly meant
(quiet keeps winning over Alert, Fever keeps paging), *or* reorder the code so a
"be quiet" can silence even a Fever breaker on a live, thermally-stressed box.
One of those changes a running safety behavior. So it went to the domain owner
as a question, not a commit. A red test you understand and flagged is pristine;
a green test you got there by guessing at a safety rule is not.

## Where it landed

The council and server flakes are fixed and merged; the First Hello test is
hardened; the full CLI suite is green across ten runs and both Rust suites are
green. The estate's tests now fail for reasons, or not at all.
