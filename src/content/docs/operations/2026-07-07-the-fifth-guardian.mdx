...
The runtime service layer renders from catalog truth:
...
The same checkup flagged three manifests referencing launchd plists that
...
None of this had paged anyone, because the instrument that would have
reported it was broken in the most quiet way possible: `sanctumctl
doctor` crashed on an `AttributeError` — a bare `Namespace()` handed to
...
service, the enumeration isn't done at the last process with authority
to restart it — it ends at the last **source of truth** that can teach a
process to. A hand-edit to a rendered file doesn't remove the spell from
the book; it just hides the book. And keep the doctor honest first: a
checkup that crashes halfway reports health by omission.
</Aside>

## What remains

The catalog now says what the haus means: the Green is the only Home
...
The twin still rests. Now the book that knew how to wake it has
forgotten, and the physician who would have noticed is back on rounds.
