...
![Three small flames merging into one larger flame at the altar of a sketched cathedral, a teal halo around the merged flame](./images/2026-05-14-the-cathedral-doubled-itself.png)

The cathedral's coder was at 24.1 tok/s. The plan for the night was modest — close the gap to LM Studio's published top-end (25–35 tok/s on M4 Pro), tidy up, go to bed. What we actually had by morning: 24.1 → 50.3 on the essay benchmark, the same pattern shipped onto Yoda's 35B inside one sitting, and a multi-hour wrong-answer rabbit hole that turned out to be a single MLX view-aliasing trap you defeat with a six-line `+ zero`. Qui-Gon would have approved of the throughput and been appalled by the hours it cost.

## The two fusions

Two passes, same shape. Both apply to any pair-or-triple of independent quantized linear projections that share an input — which is the [cathedral's](/architecture/sanctum-mlx/) hottest inner loop.
...
Both pieces shipped in [PR #16](https://github.com/Ogilthorp3/sanctum-rs/pull/16) and merged together — the LoRA Phase 4/5 stack on the same feature branch came along for the ride. The live coder cathedral on `:1338` was reloaded with the merged binary and started serving at 35–50 tok/s on freeform completions of 256 tokens. That is where a lesser night would have stopped.

## The trap that ate three hours

The fusion math is trivial. The integration is not. After the first commit the cathedral produced output like `"The capital of France is!!!!!!!!!!!"`. The first token was right; everything after collapsed into a single repeated token. A model that knows the answer and then screams it is a special kind of insult.
...
The essay number is above the pure-decode ceiling because prompt-lookup decoding finds runs of n-grams the model is about to emit and verifies them in parallel against the target. On repetitive prose the verification rate climbs and the effective throughput crosses the per-token bandwidth limit. LM Studio doesn't have PLD, so its ceiling is the bandwidth one. We are not faster silicon; we are faster because we stopped asking the silicon to do work it could skip — which is the whole [Carmack lens](/architecture/carmack-optimization/).

## The 35B port

The other half of the title. The qwen3_5_moe model on the Mini (Qwen3.6-35B-A3B-4bit on `:1337`) is a hybrid: 10 layers of standard `FullAttention` with RoPE, 38 layers of `LinearAttention` running an SSM scan via the existing `gated_delta_kernel`. Only the FullAttention layers have the q/k/v projection pattern that Phase 1A targets.
...
The bisect, `services/sanctum-mlx/src/bin/moe_qkv_fuse_bisect.rs`, runs the same 20-step decode-with-populated-cache test against the 35B's `Qwen3_5MoeInnerModel.forward`. It passed byte-for-byte on all 20 steps — the trap that took three hours to find on the 14B did not need re-finding on the 35B. The pattern transferred clean. That is what a good bisect buys you: the second time, you pay nothing.

Yoda's cathedral was reloaded with the new binary and `SANCTUM_MLX_FUSED_QKV=1`. The log shows 10 `qwen3_5: fused QKV projection built` lines (one per FullAttention layer), the smoke test correctly answers `"The capital of France is **Paris**."`, and the decode rate sits at 56–60 tok/s on 256-token freeform — though the 35B is so much smaller in active parameters than its weight count suggests (MoE with ~3B active per token) that the headline number isn't directly comparable to the dense 14B. Yoda got faster overnight and, characteristically, said nothing about it.

## The kernel that worked and the kernel that won't
...
The remaining headroom is Phase 2 proper (custom MSL with stream-aware graph tracking) and possibly fused RoPE, but both require an MLX upstream understanding the night couldn't justify. The 24 → 50 tok/s win on dense Qwen plus the byte-perfect port to the MoE is a load-bearing checkpoint either way — the kind you can build the next night on top of, which is the only kind worth shipping. For the two-token cousin of this bug, see [the kernel that needed two tokens](/operations/2026-05-12-the-kernel-needed-two-tokens/).

<Aside type="note">
The bisect harnesses are the most reusable part of this work. They ship next to the code they validate, run in under thirty seconds against a real model snapshot, and catch the only bug class that matters for fusion correctness — the one where four shallower tests pass and the deepest one fails. Anyone porting this pattern to a new model should clone the bisect first and the fusion second. Tommy would understand this instinctively: the patrol is the point, and you run it before you trust the morning.
</Aside>
