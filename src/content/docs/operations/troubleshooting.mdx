...
<Aside type="note">
If bridge100 doesn't exist at all, QEMU either hasn't started the VM or macOS decided the bridge interface wasn't important enough to keep around. The `utm-autostart` script recreates it via `sudo ifconfig`. The sudoers entry at `/etc/sudoers.d/vmnet-bridge` makes this passwordless.
</Aside>
