...
The Sanctum schedule is enforced at the network. When the evening ends, the haus cuts the connection and Firewalla confirms it — red on failure, never a fake green. But an iPhone is not only a haus-network device. It has cellular data, downloaded videos, offline games. A network rule cannot reach any of them once the Wi-Fi is gone.

That is what a **mirror** is for. You copy the same times into Apple's own Screen Time **Downtime**, on the device itself. The network enforces from outside; iOS enforces from inside; both agree on the hour. Nothing here replaces the [Sanctum schedule](/parents-guide/schedules-and-holidays/) — this is the second lock on the same door.

## What you are copying

Before touching the iPhone, open the Sanctum app and look at the schedule for the child. Note two things per layer: when the evening ends on a **school night**, and when it ends on a **weekend**. Two numbers. Those are the times you will type into iOS.
...
That is the whole mirror. No pairing. No account linking. No app to install on the child's phone. You typed the same hours in two places; that is all a mirror is. The [Nintendo Switch](/parents-guide/mirrors/nintendo-switch/) and the other consoles get the same treatment — same schedule, copied to where the network cannot follow.
