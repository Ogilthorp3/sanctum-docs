...
Sometimes the answer is yes. The movie has twenty minutes left, homework got finished without a single reminder, the cousins are over and the tournament is tied. Most of this app is built to say no on your behalf — the [Block Now](/parents-guide/block-now/) button, the nightly [budgets](/parents-guide/budgets/), the four [schedule layers](/parents-guide/schedules-and-holidays/). This is the one screen for the other answer. The **Plus de temps** (More time) screen gives you two different ways to say yes, and it matters which one you pick.
...
- **Credits play by [budget rules](/parents-guide/budgets/).** The budget meters real network activity by category, so a credit extends the same honest count the budget already keeps — it does not open a side door around it.
