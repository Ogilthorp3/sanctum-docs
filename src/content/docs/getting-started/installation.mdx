---
title: Installation
description: Install Sanctum on your Mac Mini from the checked-in operator CLI.
---
...
Sanctum's current installer surface is the checked-in `sanctumctl` operator CLI. Clone the repo, then run:
...
That flow creates the local `~/.sanctum` scaffold, drops in the annotated `instance.yaml` example, writes a node identity file, and verifies that the checked-in renderers, runtime calibration layer, and audit suites all agree.

<Aside type="tip">
  Prefer to inspect before writing? Good instinct. `python3 tools/sanctumctl.py bootstrap` previews the filesystem plan without touching your home directory.
</Aside>

The operator CLI will:
1. Create the baseline `~/.sanctum` directory structure
2. Copy the checked-in `instance.yaml.example` into `~/.sanctum/instance.yaml`
3. Derive a `.node_id` from the example topology
4. Check the canonical render/sync surfaces with `render --check`
5. Run the audited verification wall with `doctor` or `verify`
...
## Create the Sanctum Haus Directory
...
2. **Copy the example configuration.**
   ...
   ```bash
   cp ~/Projects/sanctum/sanctum/instance.yaml.example ~/.sanctum/instance.yaml
   ```

   <Aside type="tip">
     The checked-in example now lives in the Sanctum repository at `sanctum/instance.yaml.example`.
   </Aside>
...
<Aside type="tip">
  Always run with `--dry-run` first to verify the output before writing plists. The dry run prints each rendered plist to stdout without touching the filesystem. In this haus, we look before we leap. Usually.
</Aside>
...
1. **Create the VM with QEMU.**

   Use the settings from the [Requirements](/getting-started/requirements/) page. Install Ubuntu 24.04 LTS with SSH enabled.

2. **Configure networking.**

   The Mac needs to set the bridge IP each time the VM starts. This is handled by the VM autostart LaunchAgent, which runs:
