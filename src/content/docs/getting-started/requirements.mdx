...
<Tabs>
  <TabItem label="macOS">
    ...
    |----------|---------|---------|
    | macOS | 26 (Tahoe) — 15 Sequoia minimum | System update |
...
<Aside type="note">
  [Lima](https://lima-vm.io) runs the Ubuntu VM on Apple's Virtualization Framework (`vmType: vz`) — no UTM GUI, no QEMU process. The one thing `vz` does not give you natively is host-only networking, so Sanctum backs the bridge interface with `socket_vmnet` running as root. Two tools, one VM, and the host-only network Apple forgot to ship. (Sanctum ran on QEMU/HVF until May 2026, when an idle-spin bug that pinned a core at 100% on a sleeping VM convinced us to switch. The fan thanked us.)
</Aside>
