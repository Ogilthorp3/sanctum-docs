...
2. **Verify service status.**
   ...
   The root page now opens the Jocasta shell, and the same backend serves the dashboard data. Verify the backend is alive, then use Holocron or the dashboard surfaces to inspect health. Each service tile displays:
...
1. **Check the bridge interface.**
   ...
   You should see `10.10.10.1` assigned to the interface. If the interface does not exist, verify that QEMU is running and the VM is started.
