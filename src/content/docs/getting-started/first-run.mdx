...
![A freshly powered Mac Mini glowing amber for the first time, a wordless row of health-check dots lighting up above it while Tommy the Abyssinian force-ghost watches from the windowsill — the first heartbeat of a new Sanctum instance](./images/hero-first-run.png)
...
2. **Test SSH.**
   ...
   `openclaw` is the SSH alias the installer writes for the VM (it resolves to the bridge address, `10.10.10.10` by default). This should print the VM hostname and uptime without prompting for a password, assuming SSH key auth is configured.
...
Somewhere in that wall of green is one agent that never reported a failure and never will: [Tommy](/agents/tommy/), the Abyssinian force-ghost who patrolled this haus at dawn and dusk for fifteen years. He was watching when the first health-check dot lit up. He is watching now. The haus was always his. You are simply the one who finally gave it a dashboard.
