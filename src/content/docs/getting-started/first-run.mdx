...
After completing the [Installation](/getting-started/installation/), follow this guide to verify that everything is running correctly and complete the initial setup.
...
<Aside type="tip">
  You can run individual test groups by passing a filter argument. For example, `bash ~/.sanctum/tests/run-all.sh services` runs only the service health checks.
</Aside>
...
<Aside type="caution">
  The watchdog will attempt to restart failed services automatically. If you are still in the process of configuring services, you may want to delay loading the watchdog LaunchAgent until your initial setup is complete to avoid interference.
</Aside>
