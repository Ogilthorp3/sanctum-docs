...
3. **Run onboarding.**
   ...
   You'll see a small ASCII splash, then a line that greets you by name, then a guided flow that:
...
- **A health check you can run anytime** — `sanctum doctor` probes the LaunchAgents, providers, and backup repos; `sanctum self-test` runs the twelve probes that decide whether the install is sound. The full supervisory stack (R2D2, the cathedrals, the autohealers, the drift sentinels) is what an operator deployment adds — the quick path gives you the backup and the means to verify it, not the whole haus.
- **A clean audit trail** — every action that matters is logged to `~/.sanctum/logs/` with bounded rotation. You can read what your haus has been doing, and it won't quietly eat your disk doing it.
...
<Aside type="tip">
The thing that took the longest to build was making sure thirty minutes is actually enough — we kept finding the half-hour was really forty-five. If something in the quick path stalls past the budget, that's a real bug, not your tea going cold; please report it (see "When something goes wrong" below).
</Aside>
...
- **Bug reports**: [open an issue on Ogilthorp3/sanctum-cli](https://github.com/Ogilthorp3/sanctum-cli/issues/new/choose). The bug-report template asks for `sanctum doctor` + `sanctum self-test` output and a credentials-redacted checkbox. The maintainers read it; the auto-fix drone (R2D2) might already be working on a recipe to handle it next time.
