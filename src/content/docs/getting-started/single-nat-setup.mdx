...
`sanctum net single-nat` is dry-run by default — bare, it prints the staged plan and writes nothing to any device. `--apply` is what actually fires, and it is **attended-only**: there is no cron path, by design. So you stay in the room. Whole-haus WAN surgery is structurally a thing you watch happen.

<Steps>

1. **Read the plan, then validate the live seams.**

   ```bash
   sanctum net single-nat            # seven staged steps, each rollback-safe
   sanctum net single-nat --check    # read-only, but probes the real hardware
   ```

   `--check` reads the out-of-band path, box preflight, and live WAN address/routes — so you know the seams are honest before anything fires. If it shows a clean public lease already (some ISPs hand one out over PPPoE, no `/1` poison), you may not need the cutover at all.
