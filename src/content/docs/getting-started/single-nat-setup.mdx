...
"Seamlessly" is the right word for once, because the whole point is that there is exactly **one** seam. Today your traffic is translated twice — once by your ISP's hub, once by your own router — which is two seams pretending to be a network. Single-NAT collapses that to one: your Firewalla holds the public address, your hub becomes a dumb bridge, and everything downstream answers to a single policy point.
...
3. **Reverse it any time.**
   ...
   Disables the DMZ, removes the hook, re-leases DHCP, and lands you back on verified-online double-NAT in one command.
...
<Aside type="caution" title="AP mode disables the Netgear app — this is by design">
An Orbi in AP mode shuts off its SOAP control service on port `5000`, and **the Netgear / Orbi app talks to the router over exactly that service.** So the app goes dark: it can't find the router, shows it offline, or refuses to manage it. Nothing is broken. Manage the Orbi from its **web admin** at `http://<orbi-ip>` instead — the full settings surface lives there, app or no app.
</Aside>

One more new-user step that saves a confusing afternoon: set the Orbi's **management IP to DHCP**, then give it a **reservation on the Firewalla**. DHCP means the AP actually receives a gateway and DNS, so it can still reach the internet for cloud login and firmware updates; the reservation means it always lands on the same address, so `http://<orbi-ip>` is a bookmark and not a scavenger hunt. A static IP with no gateway is the classic trap here — the AP works for Wi-Fi but can never update itself.
...
| **Orbi / AP management** | Web admin `http://<orbi-ip>` (app off in AP mode) | Netgear app works (router still NATs) |
