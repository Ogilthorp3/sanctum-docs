...
The voice half talks to **sanctum-tts** on `:8008` — the Rust voice front door, fronting the Chatterbox `cbx-yoda` worker on `:8011` — over its small `{input}` contract: POST the line to speak under an `input` field, get a WAV back. It's the same stack the rest of the haus's Yoda voice already rides on, so the hello inherits a path that's been hardened, not a one-off.
