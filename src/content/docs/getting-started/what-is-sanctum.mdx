...
The hallway lights just turned themselves on. The thermostat thinks it is July. The only thing between you and sleep is a NAS that has decided — philosophically — that it no longer believes in DNS. You did not build a smart haus. You built a part-time job that pays in frustration, and it schedules its own shifts.

Sanctum is the platform that fires you from that job. One Mac Mini in your closet, one config file, one dashboard — haus automation, AI agents, network management, and local voice control, all answering to your hardware and no one's cloud. It is built for the kind of haushold where "reboot the router" is not a punchline but a Tuesday.
...
<CardGrid>
  <Card title="Haus Automation" icon="laptop">
    Home Assistant runs on a dedicated Green appliance on your haus network, with full LAN access to Sonos, HomeKit, and smart devices.
  </Card>
  ...
  <Card title="Voice Control" icon="signal">
    Yoda, the voice agent, speaks in a Chatterbox-MLX cloned voice for local text-to-speech on Apple Silicon GPU. No cloud dependency for voice. Your haus talks to you and only you.
  </Card>
</CardGrid>

## How It Works

At the core of Sanctum is a single YAML configuration file (`instance.yaml`) that describes your entire deployment: which services are enabled, what ports they bind to, which nodes exist, and how secrets are managed. Every script, LaunchAgent, and dashboard panel reads from this one source of truth — the [config system](/architecture/config-system/) is the spine the rest of the platform hangs on.

One file to rule them all — and exactly one file to blame when it's wrong.
...
<Tabs>
  <TabItem label="Hub Node (Mac Mini)">
    ...
    - The Home Assistant Green link (the control plane runs on its own appliance)
    - AI agent gateway and specialized agents
    - Local LLMs via MLX (a council seat and a coder seat, both on Apple Silicon GPU)
    - Sanctum TTS — a Chatterbox-MLX voice synthesis server
    - Cloudflare tunnel for secure remote access
  </TabItem>
  <TabItem label="VM Workloads (Ubuntu)">
    A Lima-managed Ubuntu VM, running on Apple's Virtualization.framework (the `vz` backend, not QEMU), handles workloads that benefit from a Linux environment:
...
Sanctum supports hub, satellite, mobile, and sensor node types. Your primary haus is the hub. A vacation haus runs a satellite. Your laptop is a mobile node. Each node knows its role and adjusts its services accordingly — which is more than can be said for most family members. The [node topology](/architecture/node-topology/) spells out who does what.

### Self-Healing Watchdog

A [watchdog process](/guides/watchdog/) checks every service in the [catalog](/architecture/services/) every 10 minutes. When it detects a failure, it attempts automatic remediation using the service-doctor skill before alerting you. Most issues resolve without human intervention. The rest resolve after human intervention and a glass of something strong.
...
Large language models run locally [via MLX](/architecture/sanctum-mlx/) (Apple Silicon optimized) — a Qwen3.6-35B council seat and a Devstral-Small-2-24B coder seat, each its own server on the GPU. Cloud APIs are available as fallbacks with automatic failover, but the system is designed to operate entirely on-premises when needed. Your conversations with your haus stay in your haus.
