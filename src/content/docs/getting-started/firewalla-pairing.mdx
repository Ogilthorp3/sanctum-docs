...
4. **Restart the bridge.**
   ...
   The live service is the gui-domain watchdog `com.sanctum.firewalla-bridge-watchdog`, which runs as your login user — no `sudo`, and the `gui/$(id -u)/` domain is the one that finds it. The original one-shot `com.sanctum.firewalla` system plist is retired; kickstarting it will tell you it can't find the service.
...
<Aside type="caution">
  The Firewalla MSP REST API — a cloud HTTPS endpoint, reached over your account, not port 8833 — is *not* what Sanctum uses for enforcement. Port 8833 is the local P2P/SDK port the bridge actually speaks over. Probes that pass on the MSP side and fail on the SDK side are not a contradiction — they are two different protocols looking at two different views of your network. Trust the SDK probe. The MSP probe exists for diagnostics.
</Aside>
