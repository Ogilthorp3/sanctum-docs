...
Here's the thing: the Firewalla MSP HTTP API is the loud, well-documented front door, and it straight up lies about which devices it actually knows. So Sanctum doesn't use it. We drive MAC-level enforcement through the [node-firewalla SDK](/architecture/sanctum-firewalla/) instead — the bridge running on your Mac talks to your Firewalla over the SDK's encrypted P2P protocol. The side door. The one that tells the truth, no cap.
...
## The green light means something now

Go back to 21:31. Same curfew, same bedroom door, same iPad. The difference is that `/hosts` answered with a real device count before you walked away — so when the dashboard glows BLOCKED, it glows because the box agrees, not because the front door was being polite. That is the whole gap between a dashboard and a lock, and closing it is the entire reason we took the side door.

The bridge you just paired is the muscle. The [schedules that decide when to flex it](/operations/screen-time-enforcement/) are a story for another page. Tonight, the TikTok stops leaking under the door.
