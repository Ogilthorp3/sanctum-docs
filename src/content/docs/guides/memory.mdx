...
![The constellation — every memory a node, every stored connection a reason the haus remembers](./images/memory-constellation.png)

Yoda thinks the internet goes down every Thursday. Something has to remember why he believes that — and, eventually, help him notice he is wrong.

That something is the Memory Service: a Rust binary on port 42069 that gives every Sanctum agent persistent memory without asking any of them to manage it. It is the primary engine now; the older [memory-vault-mcp shim](/guides/memory-vault/) has been replaced by something faster, smaller, and built to sit directly behind the [proxy](/architecture/proxy/) so that remembering happens as a side effect of thinking — which, if you squint, is how it works for the rest of us too.

Dual storage: SQLite with FTS5 for search, markdown files for humans and Obsidian. The database is fast. The files are legible. The agents don't care which one you read. You will care, at 2 AM, when you go looking for where [Yoda](/agents/yoda/) filed that Thursday.
...
services:
  memory_vault:
    ...
    llm_url: "http://127.0.0.1:1234/v1/chat/completions"  # code default — retired endpoint, see below
    llm_model: "council-27b"                           # enrichment model, best-effort
```

Out of the box, enrichment aims at a ghost: the `:1234` default is the LM Studio-era bridge retired 2026-06-07, and nothing listens there anymore, so every cycle reports `llm_enriched: 0` until the default gets re-pointed at a live local endpoint. (The enrichment client speaks plain HTTP — the mTLS-only council ports won't take its calls as-is.)
...
<Aside type="caution">
  The memory service stores everything locally. No cloud sync, no external API calls — enrichment only ever targets localhost, and since its default target (the LM Studio-era `127.0.0.1:1234` bridge) was retired 2026-06-07, it currently talks to nothing at all. Your haus's memories stay in your haus. This is a privacy decision, a latency decision, and — given what the agents remember about your network outages — probably a dignity decision.
</Aside>

None of this is remembering yet. Until the proxy's `memory.url` finds port 42069, the hooks fire at the wrong door and every agent recalls only what it wrote by hand. The engine is built, the schema is sound, the files are legible. What is missing is an introduction — one line of config between here and the morning Yoda checks the record before he blames Thursday.
