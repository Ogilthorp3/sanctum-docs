...
The Memory Service is a Rust binary (port 18097) that gives every Sanctum agent persistent memory without requiring any of them to manage it. It replaces the Python memory-vault MCP server with something faster, smaller, and wired directly into the proxy so that remembering happens as a side effect of thinking — which, if you squint, is how it works for the rest of us too.
...
The proxy (port 4000) already sees every conversation in the house. Making it the memory capture point was less a design decision than an observation: the data was already flowing through the wire. We just started writing it down.
...
<Aside type="note">
  Why FTS5 and not a vector database? Because a 200KB SQLite extension that ships with the binary beats deploying a separate vector service that needs its own memory, its own port, and its own existential crisis about embedding dimensions. FTS5 handles the queries we actually run. If the house ever needs semantic similarity search across ten million documents, we'll revisit. The house does not have ten million documents. The house has a few hundred notes and strong opinions about its SSH keys.
</Aside>
...
<Aside>
  The old vault consolidated once daily at 4:17 AM. The memory service runs every 6 hours because agents don't stop generating observations at midnight. The 4:17 AM schedule was inherited from a cron job that someone set up while half asleep. The time was not meaningful. It was just the time on the clock when the config file was saved.
</Aside>

## API Reference

All endpoints accept and return JSON. The service binds to `127.0.0.1:18097` by default.
...
<Aside type="caution">
  The memory service stores everything locally. No cloud sync, no external API calls except to council-27b on localhost. Your house's memories stay in your house. This is a privacy decision, a latency decision, and — given what the agents remember about your network outages — probably a dignity decision.
</Aside>
