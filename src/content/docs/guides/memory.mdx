...
![The constellation — every memory a node, every connection a reason to remember](../../../assets/illustrations/memory-constellation.svg)

The Memory Service is a Rust binary (port 42069) that gives every Sanctum agent persistent memory without requiring any of them to manage it. It replaces the Python memory-vault MCP server with something faster, smaller, and wired directly into the proxy so that remembering happens as a side effect of thinking — which, if you squint, is how it works for the rest of us too.
