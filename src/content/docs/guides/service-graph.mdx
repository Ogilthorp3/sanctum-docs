...
When bridge100 went down on March 22, the watchdog dutifully restarted six services that depend on bridge100 — a retry storm that accomplished nothing, because the root was dead and nobody told it to look down. It was like calling an ambulance for every room in a haus that lost power. The problem was the breaker. The watchdog couldn't see the breaker. The watchdog didn't know breakers existed.
...
![The dependency tree — twenty-eight services, one truth](./images/living-force-immune.png)
...
Here's where it gets beautiful, in the way that only directed acyclic graphs can be beautiful, which is to say: extremely beautiful if you're reading this page, and completely invisible to everyone else in your haushold.
