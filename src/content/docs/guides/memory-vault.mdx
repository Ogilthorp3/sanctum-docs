...
The Memory Vault is Sanctum's shared knowledge base, inspired by how human memory works. It uses a three-layer architecture — working memory, short-term, and long-term — with a daily consolidation process that acts like sleep: distilling raw observations into lasting knowledge.
...
Every note has standardized YAML frontmatter:
...
Every note has a computed importance score (0.0 to 1.0) that determines its TTL:
...
Notes accessed 5+ times are protected from expiry regardless of score.

## Consolidation

The consolidation engine runs daily at 4:17 AM (LaunchAgent: `com.sanctum.memory-consolidate`). Like sleep for the brain:
...
To prevent bloat, each layer has a maximum note count:
