...
The doc above describes the system as it is meant to be. Two pieces are not there yet.
...
- **Procedures layer is empty.** The directory exists and is wired into the cap + consolidation logic, but zero notes have been written to it. Procedural memory (runbooks, how-tos) is still living in `sanctum-docs/operations/` and individual repo READMEs. Either the layer earns its keep or it should be retired from the design.
