...
The `SKILL.md` file serves as both human documentation and agent instructions. It describes the skill's purpose, available tools, required parameters, and usage examples. Agents read this file to understand how and when to invoke the skill's tools.
...
The VM has no direct internet access (host-only networking), so skills are synced through the Mac using a two-stage pipeline.
...
<Aside type="caution">
  Because the VM has no GitHub access, any skill changes made directly on the VM will be overwritten on the next sync cycle. Always commit changes through GitHub and let the pipeline propagate them.
</Aside>
...
<Aside type="tip">
  After adding a new skill, restart the gateway for it to be discovered. Skills are loaded at gateway startup, not dynamically at runtime.
</Aside>
