...
![The health dashboard — your haus gets more checkups than you do, and it doesn't even complain about the copay](../../../assets/illustrations/health-monitoring-dashboard.png)
...
iPhone (Health Auto Export)
  │ HTTPS POST (hourly)
  ▼
Cloudflare Tunnel (health.example.net)
...
- Cloudflare tunnel with CF Access configured for `health.example.net`
...
6. Configure the endpoint:
   ...
   | Field | Value |
   |-------|-------|
   | URL | `https://health.example.net/ingest` |
   | Method | POST |
...
curl -s -H "CF-Access-Client-Id: $(security find-generic-password -a sanctum -s cf-access-health-client-id -w)" \
     ...
     https://health.example.net/health
```
