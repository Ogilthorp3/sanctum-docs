...
Home Assistant is Sanctum's home automation hub. It runs as a Docker container on the Mac Mini with bridge networking, integrating Sonos speakers, HomeKit devices, and providing remote access through a Cloudflare tunnel. It used to run on the VM, but the VM's networking had other ideas.

![The robot butler — your home automation concierge](../../../assets/illustrations/pool-robot-butler.png)
...
<Aside type="caution">
  If a Sonos speaker changes IP address (e.g., after a router reboot), it will become unreachable until the `configuration.yaml` is updated with the new IP. Use Firewalla DHCP reservations to keep speaker IPs stable. Ten speakers, ten reservations, zero tolerance for DHCP roulette.
</Aside>
...
<Aside type="danger">
  The `tts.speak` service does **not** dispatch audio to Sonos. If you use it, you'll get silence and confusion. Always use `media_player.play_media` with a TTS media source URI for Sonos announcements. This has been learned the hard way so you don't have to.
</Aside>
