...
The model is Qwen3.6-35B-A3B-4bit — a sparse MoE with about three billion active parameters per token. The base is genuinely strong at reasoning; the Council's voice and refusal posture come from the system prompt each Jedi wears, not from a fine-tune baked into these weights.

<Aside type="note">
The `model` field is asymmetric. You send `qwen3.6-35b-a3b-4bit`; the response (and `/v1/models`) echoes the HuggingFace snapshot hash — a 40-character string like `38740b84…` — because the server names the model after the directory it loaded from, and that directory is `…/snapshots/<hash>`. The slug you sent is not validated against it; any string is accepted. Don't assert on `response.model` until the server learns to wear a friendly name.
</Aside>
