...
description: From a three-file cert envelope to a streamed response against the Council's brain — a Rust MoE inference server on one Mac Mini, mTLS-only, OpenAI-compatible, one request at a time.
---

import { Aside } from '@astrojs/starlight/components';

![A pencil sketch on dark background of a vintage brass switchboard, a single illuminated plug labelled 1337 routing a glowing cable arc into a distant Mac Mini silhouette, teal halo around the plug, a small nameplate reading MLX](./images/hero-sanctum-mlx-beta-quickstart.png)
...
This page is the shortest path from "I have a cert envelope" to "I have a streamed response." For the why — where this brain sits and who decides to call it — read the [Smart Router](/architecture/dynamic-model-routing/) and the [Living Force](/architecture/living-force/).
