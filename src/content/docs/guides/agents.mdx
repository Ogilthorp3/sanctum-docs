...
|------|--------------|----------------|--------|
| `council-brain` | Claude Opus 4.7 | minimax-m27 → qwen35-plus → council-code → council-mlx | Yoda, Jocasta |
| `council-code` | Qwen 2.5 Coder 14B (LM Studio) | lmstudio-qwen → council-mlx → qwen35-plus | Any agent when the request is code-shaped |
| `council-secure` | Qwen 3.5 Plus (OpenRouter) | qwen35-plus → LM Studio | Windu |
| `council-routine` | Qwen 3.5 Plus (OpenRouter) | qwen35-plus → council-mlx | Qui-Gon, Cilghal, Mundi |
| `council-heartbeat` | Council MLX (local) | council-mlx → minimax-m25-free | All (heartbeat/status) |

Claude Code connects directly as `claude-opus-4-7` (not through a tier), with its own fallback chain to qwen35-plus → gemini-25-pro.
...
| Model | Port | Use Case |
|-------|------|----------|
| Qwen 2.5 Coder 14B Instruct | 1234 (LM Studio) | Local coding/debugging tier (`council-code`) |
| Qwen 3.5 35B (A3B) | 1234 (LM Studio) | General reasoning fallback |
| Qwen 3.5 27B 4-bit + LoRA | 1337 (MLX) | Agent-personalized local inference |

The Council MLX MLX server provides LoRA adapters fine-tuned for each of the six agents, enabling personalized responses at the local tier. Every agent gets its own voice. This is either sophisticated engineering or the early stages of a personality disorder. Possibly both.

`council-code` is intentionally not a prompt trick. The proxy owns that routing decision. When the request is clearly about code, tests, shell, or config surgery, the infrastructure can send it to the local coder tier and log the handoff like any other policy decision.

The current local serving split is simple when you stop lying to yourself about it:
- `1234` is LM Studio, including the `council-code` lane.
- `1337` is the Sanctum-served Council MLX front-end.

One port for the coding specialist, one port for the local council brain. Fewer onions.
...
|----------|-------------|-------|----------|---------------------|
| Mac      | DenchClaw   | 1977 | LAN      | `~/.openclaw/`      |
...
```
Yoda → SSH to operator@10.10.10.1 →
...
| Plugin         | Description                                          |
|----------------|------------------------------------------------------|
| **sanctum-memory**| Persistent memory across conversations               |
| **Neo4j KG**   | Knowledge graph via Graphiti (port 31416)            |
...
The Holocron provides a simplified interface for haushold members to interact with the agent system without needing direct gateway access. Because explaining SSH to your family is a different kind of support ticket.
