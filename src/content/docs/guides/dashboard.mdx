---
title: Command Center Dashboard
description: Real-time monitoring dashboard for Sanctum services, infrastructure, and network status.
---

import { Aside, Steps, Tabs, TabItem, Card, CardGrid } from '@astrojs/starlight/components';

Every haus has a thermostat. This haus has a mission control. The command center dashboard is the primary interface for monitoring and managing a Sanctum instance — real-time visibility into service health, VM status, Docker containers, network peers, and AI model availability, all rendered in a browser window that your spouse will never voluntarily open.

The root shell at `http://localhost:1111/` is the Jocasta chat interface. The operational panels live behind the same backend on `/api/*` and the packaged Holocron shell. Sanctum insists on making the librarian the front desk.

If you want the same core surfaces in a native macOS shell instead of a browser tab, see the [Holocron App](/guides/holocron-app/). Same Sanctum, less tab sprawl, slightly higher expectations.

![The monitoring wall — every service, every status, one screen](./images/monitoring-wall.png)

## Architecture

The dashboard runs a **Vite + React** frontend served by an **Express** backend on port 1111. A reverse proxy LaunchDaemon forwards port 80 to 1111, enabling clean browser access via the instance hostname. The number of layers involved in showing you a green dot is, frankly, unreasonable.

```
Browser → http://sanctum-hub (port 80)
       → socat LaunchDaemon (port 80 → 1111)
       → Express backend (port 1111)
       → Vite/React frontend
```

<Aside type="tip">
  The browser alias relies on Firewalla local DNS and an mDNS broadcast LaunchAgent. Both must be running for hostname resolution on the LAN. If neither is running, you can always fall back to the IP like an animal.
</Aside>

## Dashboard Panels

The dashboard renders conditionally based on services enabled in `instance.yaml`. Each panel polls its data source independently. It's like having seven tiny air traffic controllers, each watching one runway and none of them talking to each other.

### Service Health

Displays the status of every LaunchAgent and systemd unit tracked by the health subsystem. Services are grouped by category (core, AI, network, monitoring) with color-coded indicators:

- **Green** — running, passed last health check
- **Yellow** — running, degraded (e.g., high memory usage)
- **Red** — stopped or failing health checks
- **Grey** — disabled in configuration

You will develop a Pavlovian relationship with these colors.

### VM Status

Shows the Ubuntu VM state as reported by QEMU, including CPU core count, memory allocation, disk usage, and network connectivity on the bridge100 interface.

### Docker Containers

Lists all Docker containers with their image, status, port mappings, and resource consumption. The Home Assistant container is highlighted when present.

### LaunchAgents

Enumerates all Sanctum-managed LaunchAgents with their loaded/running state, PID, last exit status, and any recent restart events.

### Tailscale Peers

Renders the Tailscale network topology showing all connected devices, their IP addresses, online status, and last seen timestamps.

### LM Studio Models

Queries LM Studio on port 1234 for loaded and available models, displaying model name, parameter count, quantization level, and VRAM usage.

### Alerts

Shows active and recent alerts from the watchdog system with severity, source, message, and timestamp. Alerts can be acknowledged directly from the panel.

<Aside type="danger">
  Acknowledging an alert does not fix the underlying problem. It just makes the dashboard stop yelling at you. These are different things, and confusing them is how outages happen at 2 AM.
</Aside>

## API Endpoints

The Express backend exposes several endpoints consumed by the frontend and available for external tooling.

### GET /api/health/status

Returns the current health status of all monitored services.

```json
{
  "status": "degraded",
  "services": {
    "openclaw-gateway": { "status": "running", "pid": 1234, "uptime": "4d 12h" },
    "home-assistant": { "status": "running", "pid": 5678, "uptime": "2d 8h" },
    "qemu-vm": { "status": "running", "cpu": "4 cores", "memory": "8GB" }
  },
  "timestamp": "2026-03-19T14:30:00Z"
}
```

### GET /api/config

Serves the instance configuration from `instance.yaml` with secrets excluded. Used by the frontend to determine which panels to render and how to label the instance.

### GET /api/alerts

Returns active alerts with optional query parameters for filtering by severity and time range.

| Parameter  | Type   | Description                        |
|------------|--------|------------------------------------|
| `severity` | string | Filter: `critical`, `warning`, `info` |
| `since`    | string | ISO 8601 timestamp for lower bound |
| `limit`    | number | Maximum results (default: 50)      |

### GET /api/health/tests

Runs the full health test suite on demand and returns results. This is the same suite executed by the watchdog on its periodic schedule. Think of it as pulling the fire alarm to see if the sprinklers work. Useful, but don't do it during dinner.

## Proxied Services

The dashboard backend proxies requests to services that are not directly accessible from the browser. This is the polite way of saying "some of these services were never designed to talk to a web browser, so we put a translator in the middle."

### Firewalla

Firewalla API calls are proxied through the Express backend, which handles the cloud authentication handshake and local P2P connection on port 8833. The frontend never communicates with Firewalla directly.

### Home Assistant

HA API requests are forwarded to `localhost:8123` through the backend, which injects long-lived access tokens stored in the Keychain. This avoids exposing HA tokens to the browser.

## Configuration

Dashboard behavior is driven by `instance.yaml`:

```yaml
services:
  command-center:
    enabled: true
    port: 1111
    bind: lan
```

The `bind: lan` directive restricts the Express server to the LAN interface. The socat LaunchDaemon handles port 80 forwarding separately.

## LaunchAgent

The dashboard runs as a persistent LaunchAgent:

| Property       | Value                          |
|----------------|--------------------------------|
| Label          | `com.sanctum.dashboard`    |
| RunAtLoad      | true                           |
| KeepAlive      | false                          |
| Port           | 1111                           |

<Aside type="note">
  The reverse proxy from port 80 to 1111 runs as a **LaunchDaemon** (`com.sanctum.dench-proxy`) because binding to port 80 requires root privileges. One of the few places in this system where we admit the operating system is still in charge.
</Aside>

For the packaged desktop shell that embeds this dashboard alongside command and health surfaces, see [Holocron App](/guides/holocron-app/).
