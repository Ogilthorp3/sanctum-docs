...
The loopback shell at `http://127.0.0.1:1701/` serves the Command Center React app directly — the page title is `… Command Center`, not a chat window. On the tailnet, reach the same app through the PQ terminator at `https://manoir.tail…:1111/` (WebAuthn passkey). The operational data lives behind `/api/*` on the same process.

Mutating API calls also need **[parent unlock](/guides/parent-unlock/)** (Touch ID or PIN) when that policy is enabled — passkey is the front door; biometrics open the write path.
...
The dashboard runs a **Vite + React** frontend served by an **Express** backend on port **1701** (NCC-1701), loopback by default. Tailnet clients use **`:1111`** (post-quantum terminator → `:1701`). The number of layers involved in showing you a green dot is, frankly, unreasonable.
...
```
Local    → http://127.0.0.1:1701     → Express + React (Command Center)
Tailnet  → https://manoir…:1111      → PQ terminator → :1701
...
<Aside type="tip">
  The only mDNS broadcaster installed (`com.sanctum.mdns-docs`) advertises `sanctum.local` for the docs site on `:4321` — not a dashboard hostname. Prefer `127.0.0.1:1701` on manoir or the MagicDNS `:1111` front door from the admin laptop.
</Aside>
